function splitBatchContent(result) {
	...
	for ( var i = 0; i < result.length; i++) {
		...
		$("#textSaleRate" + i).val(result[i].purchaseRate);
...
function getProductByBatch(productId) {
	var storeName=$("#mainStore").val();
	jQuery.ajax({
		async : true,
		type : "GET",
		data : {
			"productId" : productId,"validStore":storeName
		},
...
function setCreditBillContents(content) {
    ...
    var cellsrenderer = function (row, columnfield, value, defaulthtml, columnproperties, rowdata) {
        ...
        if (value ==0) {
            return '<span style="margin: 4px; float: ' + columnproperties.cellsalign + '; color: #ff0000;">Cash Return</span>';
        }
        else {
            return '<span style="margin: 4px; float: ' + columnproperties.cellsalign + '; color: #008000;">Credit Return</span>';
        }
    };
