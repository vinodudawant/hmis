...
function saveProductBelowLevel() {
	var favorite = [];
	fetchProductDetails();
	...
	if ($('#hiddenVendorId').val() != "0")
		...
		{
		alert("Please Enter Proper Vendor Name");
		$("#txtPartyName").focus();
		return false;
		}
	...
	for ( var i = 0; i < favorite.length; i++) {
		...
		if ($('#textMrp' + favorite[i]).val() != null
				&& $('#textMrp' + favorite[i]).val() != "")
			mrp = $("#textMrp" + favorite[i]).val();
		else
			{
			alert("Enter MRP");
			$("#textMrp" + favorite[i]).focus();
			return false;
		...
		if ($('#textQty' + favorite[i]).val() != null
		...
		$("#textPurchaseRate" + favorite[i]).focus();
		return false;
		}

		if ($('#textAmt' + favorite[i]).val() != null
	...
	jQuery
			.ajax({
				...
				type : "POST",
				data : str,
				url : "../productMinLevel/saveProductBelowMinLevel",
				catche : false,
				error : function() {
				...
				success : function(r) {
					...
					if (r[0] == 'error') {
						...
						window.open(
								"/EhatEnterprise/pharmacy/productMinLevel/printView?poId="
										+ r[0], '_blank');
...
function calculateAmount(value) {
	...
	/*
	 * calculateTotal(value); calculateVat(value);
	 */
...
function calculateTotal() {
	...
	for ( var i = 0; i < favorite.length; i++) {
		if ($('#textAmt' + favorite[i]).val() != ''
				&& $('#textAmt' + favorite[i]).val().length > 0)
...
function deleteProductBelowMinLevel(purchaseOrderId) {
...
function productBelowMinLevelPrint(partywisePoId) {
	window.open("/EhatEnterprise/pharmacy/productMinLevel/printView?poId="
			+ partywisePoId + "");
...
function splitProductBelowMinLevel(content) {
...
function searchPurchaseOrder(id) {
	...
	jQuery.ajax({
		...
		type : "GET",
		data : str + "&reqType=AJAX",
		url : "/EhatEnterprise/pharmacy/productMinLevel/getPObyVendorID",
		timeout : 1000 * 60 * 5,
		catche : false,
		error : function() {
			alert("error");
...
function setTableContent(result) {
	...
	for ( var i = 0; i < r.length; i++) {
		...
		divContent = divContent
				 ...
				 * r[i].poId + "' class='btn btn-xs btn-success' onclick='edit(" +
				 * r[i].poId + ")' value='PRINT'> <i class='fa fa-edit'></i>
				 * </button> </td>"
				 */

				/*
				 * + "<td class='col-md-2 center'> <a id='btnEdit" + r[i].poId + "'
				 * class='btn btn-xs btn-success' value='EDIT'
				 * href='/EhatEnterprise/pharmacy/po/edit-view?poId=" +
				 * r[i].poId + "'>" + "<i class='fa fa-edit'></i> </a></td>"
				 */
