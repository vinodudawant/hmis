...
<body>
	<%
		...
		PdfPTable HeaderTable6 = new PdfPTable(7);
				int[] headerwidth6 = { 5,20,8,8,25,10,5  };
				...
				if (order_comp_drugesli.size() != 0) {
					...
					if (order_comp_drugesli.size() != 0) {
						...
						HeaderTable6.addCell(new Phrase("Sr.No", subheader));
						HeaderTable6.addCell(new Phrase(" Drug Name", subheader));
						HeaderTable6.addCell(new Phrase("Route", subheader));
						HeaderTable6.addCell(new Phrase("Dosage Freq.", subheader));
						HeaderTable6.addCell(new Phrase("Instruction", subheader));
						...
						//HeaderTable6.flushContent();
			 			...
			 			for(int i=0;i<order_comp_drugesli.size();i++){
							...
							int Night = Integer.parseInt(day[3]);
								...
								String drugName="";
								if(order_comp_drugesli.get(i).getNutracalProductFlag() == 0)
									drugName=order_comp_drugesli.get(i).getDrugName();
 
 							HeaderTable6.addCell(new Phrase(""+(i+1), tabletext));
 						
 							//if(order_comp_drugesli.get(i).getNutracalProductFlag() == 0)
							//HeaderTable6.addCell(new Phrase(""+order_comp_drugesli.get(i).getMedicineName()+". "+ "("+drugName+")" , tabletext));
 						  //else
 							// HeaderTable6.addCell(new Phrase(""+order_comp_drugesli.get(i).getMedicineName(), tabletext));
 						  
 						  
 						 Phrase pp = new Phrase();
						  pp.add(new Chunk("" + order_comp_drugesli.get(i).getMedicineName() , subheader));
							if(order_comp_drugesli.get(i).getNutracalProductFlag() ==0){
								pp.add(new Chunk("\n" + "("+drugName.trim()+")", tabletext8));
							}
							
							PdfPCell testNameCell = new PdfPCell(pp);
							//testNameCell.setBorder(Rectangle.NO_BORDER);
							HeaderTable6.addCell(testNameCell); 
 							  
 						  HeaderTable6.addCell(new Phrase(""+order_comp_drugesli.get(i).getRouteName(),tabletext ));
 						//  HeaderTable6.addCell(new Phrase(""+order_comp_drugesli.get(i).getRouteUnicode(),tabletext ));
