<%@page import="com.hms.canteen.dto.CanteenPurDto"%>
 ...
 */
			...
			Document document = new Document(PageSize.A4);//SMALL_PAPERBACK//POSTCARD//PENGUIN_SMALL_PAPERBACK
			...
			int purId=Integer.parseInt(request.getParameter("canteenId"));
			CanteenService canteenService=(ApplicationContextUtils.getApplicationContext()).getBean(CanteenService.class);
			List<CanteenPurDto> ltcanteen = new ArrayList<CanteenPurDto>();
			ltcanteen=canteenService.getpurlistbyId(purId);
			...
			HeaderTable2.addCell(new Phrase("       CANTEEN PURCHASE PRINT ", header));
			HeaderTable2.addCell(new Phrase("", subheader));
			HeaderTable2.addCell(new Phrase("", subheader));
			...
			HeaderTable3.getDefaultCell().setBorder(Rectangle.NO_BORDER);
				...
				int billNo =ltcanteen.get(0).getPurchId();
				String refbillNo =ltcanteen.get(0).getRefBillNo();
				int canteenNo=ltcanteen.get(0).getPurchId();
				String customerName =ltcanteen.get(0).getPurchaseby();
				int tokenNo =0;	
				...
				HeaderTable3.addCell(new Phrase("Ref Bill No ",subheader));
				HeaderTable3.addCell(new Phrase(" : "+refbillNo,tabletext));
	            
				HeaderTable3.addCell(new Phrase("Purchase By ",subheader));
			...
			HeaderTable5.addCell(new Phrase("", tabletext));	
				...
				HeaderTable5.addCell(new Phrase("UOM", subheader));
			...
			HeaderTable5.addCell(new Phrase("", tabletext));		
				...
				for (int i = 0; i < ltcanteen.get(0).getLtCanteenSlave().size(); i++) {
					...
					String SubName=ltcanteen.get(0).getLtCanteenSlave().get(i).getItemName();
					...
					String umo  = ltcanteen.get(0).getLtCanteenSlave().get(i).getUom();
					...
					HeaderTable5.addCell(new Phrase(""+umo,tabletext));
