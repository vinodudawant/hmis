...
<body>
	...
	//	List<HospitalDetails> arrHospitalDetails = hs.getListHospitalDetails().getListHospitalDetails();
		...
		//Integer gender =ltRegMasterDto.get(0).getGender();
					...
					HeaderTableH.addCell(new Phrase(" Service Advice Info:", headerTitle));
					 ...
					 HeaderTableSpace.addCell(new Phrase("", tabletext));
						...
						for(int i=0;i < lstserviceObj.size();i++){
						 ...
						 Date dateformatter1=lstserviceObj.get(i).getCreated_date_time();
					     String datetime1=dateformatter.format(dateformatter1);
						...
						/*HeaderTableServiceAdvice.addCell(new Phrase(""+lstserviceObj.get(i).getCreated_date_time(), tabletext));*/		
							HeaderTableServiceAdvice.addCell(new Phrase(""+datetime1, tabletext));
			...
			if(listPrescriptionsSP.size()>0){	
				...
				// START : General Instruction
					...
					/* for (int i = 0; i < generalInstructionList.size(); i = i + 2) {
							...
							Table3.addCell(new Phrase("" + (i + 1) + ".",
									tabletext));
							 Table3.addCell(new Phrase(""+generalInstructionList.get(i),
									tabletext));
							 Table3.addCell(new Phrase("", tabletext));
							 if (generalInstructionList.get(i + 1) != null
								...
								Table3.addCell(new Phrase("" + (i + 2) + ".",
										tabletext));
								Table3.addCell(new Phrase(generalInstructionList.get(i + 1),tabletext)); 
							} 
							document.add(Table3);
							Table3.flushContent();
						} */
		       ...
		       List<OPDDietMasterDTO> lstmaster1 = dietService.getOPDDietListByTreatmentId(treatmentId);
  		...
  		HeaderTable5.flushContent();
				...
				if(clinicalObj !=null){
					...
					HeaderTableSpace.flushContent();
				     ...
				     String dm2=clinicalObj.getClinicalEvaltemplateData();
				     java.util.List<Element> ie1 = HTMLWorker.parseToList(new StringReader(dm2), styleSheet);
				     
				     if(dm2.equals("") || dm2.equals("NULL")){
				         for (Element element : ie1) {
				             if (element instanceof PdfPTable)
				             {
				                 PdfPTable htmlTable = new PdfPTable(1);
				  	   ...
				  	   java.util.List<Element> ie3 = HTMLWorker.parseToList(new StringReader(dm2), styleSheet);
				         for (Element element : ie3) {
				             if (element instanceof PdfPTable)
				             {
				                 PdfPTable htmlTable = new PdfPTable(1);
