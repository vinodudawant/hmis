...
<%@page import="com.hms.ipdbill.dto.IpdBillRefundMasterDTO"%>
...
<body>
	...
	try {
		...
		if(rtd.getChargesMasterSlaveId() > 0)
			PType = "Sponsor";
    // start for registration 1	
		for (int i = 0; i < listServiceIpdDto.size(); i++) {
			
		  if(listServiceIpdDto.get(i).getServiceId() == 1){
    ...
    //start of bed 2
       for (int i = 0; i < listServiceIpdDto.size(); i++) {
		  ...
		  if(listServiceIpdDto.get(i).getServiceId() == 3){
    ...
    //start of consultation 3
       for (int i = 0; i < listServiceIpdDto.size(); i++) {
		  ...
		  if(listServiceIpdDto.get(i).getServiceId() == 2){
			...
			for (int k = 0; k < listBillDetail.size(); k++) {
				...
				if(listBillDetail.get(k).getCancle().equals("N") && listBillDetail.get(k).getPaidByCashFlag().equals("N")){
					...
					if(serId==2){
						...
						String sname=sponsorCustom.getSponsorCustomTestName(rtd.getChargesMasterSlaveId(), listBillDetail.get(k).getServiceId(), listBillDetail.get(k).getSubServiceId());
    ...
    //start of Bed Side Procedures 4
       for (int i = 0; i < listServiceIpdDto.size(); i++) {
			
		  if(listServiceIpdDto.get(i).getServiceId() == 6){
    ...
    //start of pathology 5
       for (int i = 0; i < listServiceIpdDto.size(); i++) {
			
		  if(listServiceIpdDto.get(i).getServiceId() == 11){
			...
			for (int k = 0; k < listBillDetail.size(); k++) {
				...
				if(listBillDetail.get(k).getCancle().equals("N") && listBillDetail.get(k).getPaidByCashFlag().equals("N")){
					...
					if(serId==2){
						...
						//otProcedure = listBillDetail.get(k).getOtProcedure();
						//subServName = listBillDetail.get(k).getCategoryName();	
						subServName=sponsorCustom.getSponsorCustomTestName(rtd.getChargesMasterSlaveId(), listBillDetail.get(k).getServiceId(), listBillDetail.get(k).getSubServiceId());
						 if(subServName.equalsIgnoreCase("")){
							  subServName = listBillDetail.get(k).getCategoryName();	
						  }
						/* if(otProcedure!=null){
							subServName = subServName + cghsId +"("+otProcedure+")";
    ...
    // end of pathology
     ...
     //start of Administrative Charges 6
       for (int i = 0; i < listServiceIpdDto.size(); i++) {
			
		  if(listServiceIpdDto.get(i).getServiceId() == 15){
    ...
    // end of Administrative Charges
    
    //start of canteen Charges by sandip
       for (int i = 0; i < listServiceIpdDto.size(); i++) {
			
		  if(listServiceIpdDto.get(i).getServiceId() == 17){
    ...
    // end of canteen Charges
    
     //start of Investigation 7
       for (int i = 0; i < listServiceIpdDto.size(); i++) {
			
		  if(listServiceIpdDto.get(i).getServiceId() == 12){
    ...
    // end of Investigation
     ...
     //start of IPD 8
       for (int i = 0; i < listServiceIpdDto.size(); i++) {
		  ...
		  if(listServiceIpdDto.get(i).getServiceId() == 40){
    ...
    // end of IPD
     ...
     //start of other remaining services  
       for (int i = 0; i < listServiceIpdDto.size(); i++) {
			
		  if( listServiceIpdDto.get(i).getServiceId() != 1 && listServiceIpdDto.get(i).getServiceId() != 2 && listServiceIpdDto.get(i).getServiceId() != 3
				  && listServiceIpdDto.get(i).getServiceId() != 6 && listServiceIpdDto.get(i).getServiceId() != 11 && listServiceIpdDto.get(i).getServiceId() != 12
						  && listServiceIpdDto.get(i).getServiceId() != 15 && listServiceIpdDto.get(i).getServiceId() != 17 && listServiceIpdDto.get(i).getServiceId() != 40 ){
    ...
    // end of other remaining services
	...
	//	HeaderTable9.setSpacingBefore(10f);
		...
		// START for Refund purpose shradhha specific
			IpdBillRefundMasterDTO billReceiptMasterObj=new IpdBillRefundMasterDTO();
			billReceiptMasterObj.setBillId(billId);
			billReceiptMasterObj.setTreatmentId(treatmentId);
			billReceiptMasterObj.setReceiptOf("general");
			billReceiptMasterObj.setCreatedBy(userId);
			IpdBillRefundMasterDTO billRefundObj = ipdBillService.getBillRefundDetailsIpd(billReceiptMasterObj,"refund");
			
			if(billRefundObj.getListBillRefundMaster().size()>0){
				...
				HeaderTable6.addCell(new Phrase("Refund Details :", subheader));
				HeaderTable6.addCell(new Phrase("", subheader));
				HeaderTable6.addCell(new Phrase("", tabletext));
				HeaderTable6.addCell(new Phrase("", subheader));
				HeaderTable6.addCell(new Phrase("", subheader));

		for (int i = 0; i < billRefundObj.getListBillRefundMaster().size(); i++) {

			int refundRecId = billRefundObj.getListBillRefundMaster().get(i).getBillRefundId();

			List<BillReceiptMasterDTO> lstPojo = new ArrayList<BillReceiptMasterDTO>();
			String callFrom = "refundIpd";
			lstPojo = hm.getOpdRecDetails(billId, treatmentId, patId, refundRecId, callFrom);
			int recCount = lstPojo.get(0).getReceiptCount();

			//irfan khan 11-jan-2018 multi pay mode list
			List<MultiBillReceiptMasterDTO> listMultiPay = new ArrayList<MultiBillReceiptMasterDTO>();
			listMultiPay = hm.getMultiRecDetails(billId, treatmentId, patId, refundRecId, departmentId);
			...
			HeaderTable6.getDefaultCell().setBorder(Rectangle.TOP);
			...
			HeaderTable6.addCell(new Phrase(
					"" + currencyName + "." + (EnglishNumberToWords.convert(finalam2).toUpperCase()) + "ONLY",
					tabletext));
			...
			// END for Refund purpose shradhha specific
		}
			}
