<%@page import="com.hms.administrator.dao.impl.AdminCashVoucherDaoImpl"%>
<%@page import="com.hms.dto.Doctor"%>
<%@page import="com.hms.administrator.dto.AdminCashVoucherDTO"%>
<%@page import="com.hms.utility.ApplicationContextUtils"%>
<%@page import="com.hms.administrator.controller.AdminCashVoucherController"%>
<%@page import="com.hms.administrator.service.HospitalDetailAdminService"%>
<%@page import="com.hms.administrator.service.AdminCashVoucherService"%>
...
<title>General Voucher Print</title>
</head>
<body>
<%
	try
	{
		...
		String voucherIDs = request.getParameter("voucherID");	
		int voucherID = Integer.parseInt(voucherIDs);
		...
		AdminCashVoucherService AdCashVoucherCont = (AdminCashVoucherService) (ApplicationContextUtils.getApplicationContext()).getBean(AdminCashVoucherService.class);
		List<AdminCashVoucherDTO> lstGeneralVouchers = new ArrayList<AdminCashVoucherDTO>();
		lstGeneralVouchers = AdCashVoucherCont.fetchVoucherForPrint(voucherID,request);
		
		String payTo="";
		String account="";
		String narration="";
		String autherisedById="";
		String autherisedBy="";
		String GrpName = "";
		String ledgerhead = "";
		double amount=0.0;
		Date createdDate = null; 
		for(int i=0;i < lstGeneralVouchers.size();i++ )
		{
		payTo= lstGeneralVouchers.get(i).getPayTo();
		System.out.println("payTo "+payTo);
		}
		for(AdminCashVoucherDTO lst:lstGeneralVouchers)
		{
		//	 payTo=lst.getPayTo();
			 ...
			 narration=lst.getNarration();
			 amount=lst.getAmount();
			 autherisedById=lst.getAuthorisedBy();
			 createdDate =(Date) lst.getCreatedDate();
			 autherisedBy=lst.getAuthByName();
			 GrpName = lst.getGroupName();
			 ledgerhead = lst.getLedgerHead();
		...
		HeaderTable4.addCell(new Phrase("Paid To",subheader));	
		HeaderTable4.addCell(new Phrase(" : "+payTo,tabletext));
		...
		HeaderTable4.addCell(new Phrase(" : "+narration,tabletext));
		HeaderTable4.addCell(new Phrase("Group Name",subheader));		
		HeaderTable4.addCell(new Phrase(" : "+GrpName,tabletext));
		HeaderTable4.addCell(new Phrase("Ledger Heads Name",subheader));		
		...
		HeaderTable10.addCell(new Phrase("Auth.By : "+autherisedBy,tabletext));
