...
(function(jQuery, window, undefined) {
	...
	jQuery.fn.init = function(selector, context, rootjQuery) {
		...
		if (selector && typeof selector === "string"
			...
			// (#9521)
			// Note that this may break jQuery 1.6.x code that otherwise would
			// work.
			if (match[0].charAt(0) === "#") {
			...
			if (jQuery.parseHTML) {
				return oldInit.call(this, jQuery.parseHTML(match[2], context,
						true), context, rootjQuery);
			}
		}
...
// function to hide munu at load
// run when the DOM is ready
/*$(function() {
	var inputs = [];
	inputs.push('action=fetchUserAccessView');
	inputs.push('callfrom=' + 'jquery');
	var str = inputs.join('&');
	jQuery.ajax({
		async : true,
		type : "POST",
		...
		success : function(r) {
			...
			if (pobj1.liua.length != 0) {
				...
				var arrayVisibleModules = [];
				arrayVisibleModules = pobj1.liua[0].modli.split(",");

				for ( var int = 0; int <= (arrayVisibleModules.length); int++) {
					$("#ehatMod" + (arrayVisibleModules[int])).show();
				}

				// var uaccess = "";
				// for ( var k = 0; k < arr.length; k++) {
				// uaccess = uaccess + "U" + arr[k] + ',';
				// }
				//
				// for ( var j = 1; j <= 13; j++) {
				//
				// var n = uaccess.indexOf("U" + j + ",");
				// if (n == -1) {
				// $("#ehatMod" + j).hide();
				// }
				// }
...
$(function() {
	$('#fName').addClass('capitalise');
	$('#mName').addClass('capitalise');
	$('#lName').addClass('capitalise');
	$('#conAdd1').addClass('capitalise');
	$('#conAdd2').addClass('capitalise');
	$('#conAdd3').addClass('capitalise');
	$('#conAdd4').addClass('capitalise');
});
$(function() {
	...
	jQuery.ajax({
		...
		type : "POST",
		data : "&reqType=AJAX",
		url : "ehat/syncronizedtokencontroller/syncronizedtoken",
		timeout : 1000 * 60 * 5,
		cache : false,
		error : function() {
			alert('error');
...
String.prototype.escapeSpecialChars = function() {
	return this.replace(/&quot;/g, "\"").replace(/&#39;/g, "\'").replace(
			...
			.replace(/&lt;/g, "<").replace(/&gt;/g, ">").replace(/PP/g, "%");
};
String.prototype.decodeSpecialChars = function() {
	return this.replace(/&amp;/g, "&");
};
/** ***********Key board Shortcut code****************** */
