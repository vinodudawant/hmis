...
<body>
	<%
		try {
			// TempEventHandlerLISPDF2 ooo = new TempEventHandlerLISPDF2();
			...
			response.setHeader("Content-Disposition", "inline; filename = OPD Receipt Print"); //added by sandip for Tag name
			...
			Font newHeader = new Font(Font.FontFamily.HELVETICA, 8, Font.BOLD);
		...
		/* 	p.add(new Chunk(" \nCIN: "+cinNo+", GST : "+gstNo , tabletext));	
			...
			HeaderTable2.addCell(new Phrase("     OPD BILL CUM RECEIPT ", header));
			...
			//addded by vishant
			Integer createdUserId = lstPojo.get(0)
			.getCreatedBy();
			...
			if (labId == 1) {
				...
				for (int i = 0; i < lstPojo.get(0)
				    ...
				    if(rtd.getChargesMasterSlaveId() > 0){
				    	sname=sponsorCustom.getSponsorCustomTestName(rtd.getChargesMasterSlaveId(), lstPojo.get(0).getListBillReceiptSlave()
								.get(i).getServiceId(), lstPojo.get(0).getListBillReceiptSlave().get(i).getSubServiceId());
				    }
					...
					HeaderTable5.addCell(new Phrase("" + compName,tabletext));
						...
						HeaderTable5.addCell(new Phrase("" + sname,tabletext));
					}
			...
			if (labId == 1 || sponsorSlave > 0) {
				...
				if(user_name != null)
				{
					HeaderTable7.addCell(new Phrase(" " +createdUserName , tabletext));
				}
				else
				{
					HeaderTable7.addCell(new Phrase(" " , tabletext));
				...
				if(user_name != null)
				{
					HeaderTable4.addCell(new Phrase(
							"          User :" + createdUserName, tabletext));
				}
				else
				{
					HeaderTable4.addCell(new Phrase(
