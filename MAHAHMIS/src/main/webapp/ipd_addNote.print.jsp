...
//start dignosis info
			         ...
			         if(lstdignoObj.size() > 0){
			        	  for(DiagonosisMasterDto dobj:lstdignoObj ){
			        		  RegistrationDto  pobj =dobj.getPatientObj();
			        		List<TreatmentDto>  listTreat=pobj.getListTreatment();
			        		      for(TreatmentDto tobj :listTreat){
			        		    	  Integer treatId=tobj.getTreatmentId();
			        		    	  System.err.println(treatmentId +"treat====="+treatId);
			        		    	  
			        		    	   if(Integer.toString(treatmentId).equalsIgnoreCase(Integer.toString(treatId))){
			        		    		   System.err.println(treatmentId);
			        		    		  System.err.println(treatId);

			        					
			        						subHeaderTable.addCell(new Phrase("Admission Notes",
			        								subheader));
			        					...
			        					/* 	
			        						...
			        						java.util.List<Element> ie = HTMLWorker.parseToList(
			        								new StringReader("              "+ tobj.getNotes()),
			        								styleSheet);
