<%@page import="com.hms.pharmacy.pojo.IndentSlave"%>
<%@page import="com.hms.pharmacy.pojo.IndentMaster"%>
...
<body>
	<%
		...
		//int unitId= request.getParameter("unitId");
	...
	IndentService indentService=(ApplicationContextUtils.getApplicationContext()).getBean(IndentService.class);
	IndentMaster indentMaster=indentService.getIndentDataById(indentId);
	
	PdfPTable HeaderTableclinicalStaging = new PdfPTable(3);
		int[] headerwidthclinicalStaging = {5,10,10 };
		...
		HeaderTableclinicalStaging.getDefaultCell().setBorder(Rectangle.BOX);
			...
			HeaderTableH.addCell(new Phrase("Generated Indent Information:", subheader));
		  	 HeaderTableH.addCell(new Phrase("", tabletext));
		  	HeaderTableH.addCell(new Phrase(" ", subheader));
		  	// HeaderTableH.addCell(new Phrase("", tabletext));
		  	 
		  	 
		  	//HeaderTableH.addCell(new Phrase("Store Name:", subheader));
		  	// HeaderTableH.addCell(new Phrase("", tabletext));
		  	//HeaderTableH.addCell(new Phrase(" ", subheader));
		  	HeaderTableH.addCell(new Phrase(" Store Name: " +indentMaster.getIndentStoreName(), subheader));
			...
			if(indentMaster.getLtIndentSlave().size() > 0){
				List<IndentSlave> indentSlaveList = indentMaster.getLtIndentSlave();
			
			
			HeaderTableclinicalStaging.addCell(new Phrase("Sr.No ", subheader));
			HeaderTableclinicalStaging.addCell(new Phrase("Product Name  ", subheader));
			HeaderTableclinicalStaging.addCell(new Phrase("Required QTY", subheader));
			
			int icount=1;
			for(int i=0;i<indentSlaveList.size() ;i++){
				
				
				int productId = indentSlaveList.get(i).getIndentProductId();
				String productName = indentService.getProductNameByProductId(productId);
				
				HeaderTableclinicalStaging.addCell(new Phrase(" "+icount, tabletext));
				HeaderTableclinicalStaging.addCell(new Phrase(" "+productName, tabletext));
				HeaderTableclinicalStaging.addCell(new Phrase(" "+indentSlaveList.get(i).getIndentSlaveRequireQty(), tabletext));
