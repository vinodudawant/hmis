<%@page import="com.hms.TempEventColorRender"%>
...
<body>
	<%
		...
		Font tableheader14 = new Font(Font.HELVETICA, 10,Font.BOLD);
		...
		//document.newPage();
			...
			List<PathologySampleWiseMaster> 	labProfileTestCompDTO=phleboService.getTestComponentListByProfileId(list.get(pro).getProfileId());
						...
						int[] HeaderWidth = { 30,10, 10,10,35};
						...
						//cells24.setColspan(3);
						//cells24.setBorder(Rectangle.NO_BORDER);
						Headertable.addCell(cells24);
						
						PdfPCell cells27 = new PdfPCell(new Phrase("Observed Values", tableheader14));
						cells27.setHorizontalAlignment(Element.ALIGN_CENTER);
						//cells27.setBorder(Rectangle.NO_BORDER);
						cells27.setColspan(3);
						Headertable.addCell(cells27);
						
						
						
						PdfPCell cells29 = new PdfPCell(new Phrase("Reference  Values", tableheader14));
						cells29.setHorizontalAlignment(Element.ALIGN_LEFT);
						//cells29.setBorder(Rectangle.NO_BORDER);
						Headertable.addCell(cells29);
						
						
				 	Headertable.getDefaultCell().setBorder(Rectangle.LEFT | Rectangle.RIGHT );
						
						Headertable.addCell(new Phrase("", tabletext));
						
						PdfPCell lowCell = new PdfPCell(new Phrase("LOW",subheader));
						lowCell.setHorizontalAlignment(Element.ALIGN_CENTER);
						  lowCell.setBackgroundColor(Color.GREEN);
							
						PdfPCell normalCell = new PdfPCell(new Phrase("Normal",subheader));
						normalCell.setHorizontalAlignment(Element.ALIGN_CENTER);
						
							PdfPCell highCell = new PdfPCell(new Phrase("High",	subheader));
							highCell.setHorizontalAlignment(Element.ALIGN_CENTER);
							highCell.setBackgroundColor(Color.ORANGE);

					            // Set default cell for the table
					            for (int i = 0; i < Headertable.getNumberOfColumns(); i++) {
                                              //   Cell cell11 = Headertable.get
                                            		  if(i==2){
					            	                     //Headertable.getDefaultCell().setBackgroundColor(Color.GREEN);
                                            		 // Headertable.addCell(lowCell);
                                            			  
                                            		  }
                                      }
					       
							Headertable.addCell(lowCell);
							Headertable.addCell(normalCell);
							Headertable.addCell(highCell);
						 ...
						 for (PathologySampleWiseMaster labCompObj : labProfileTestCompDTO) {
								...
								if( labCompObj.getTestIdd().intValue() > 0){
									...
									if(labCompObj.getTreadanalysisFlag().equalsIgnoreCase("Y")){
										...
										String unitTread=labNObj.getUnitname();
								    	...
								    	java.awt.Font font4 = new java.awt.Font("FreeMono", java.awt.Font.BOLD, 17);
								    	TextTitle title = my2DChart.getTitle();
								    	title.setFont(font4);
								    	
								    	CategoryPlot plot = my2DChart.getCategoryPlot();
								    	CategoryAxis axis = plot.getDomainAxis();
								    	java.awt.Font font5 = new java.awt.Font("FreeMono", java.awt.Font.BOLD, 18);
						  ...
						  for (PathologySampleWiseMaster labCompObj : labProfileTestCompDTO) {
				             ...
				             if( labCompObj.getTestIdd().intValue() == 0){
				            	  String headName = labCompObj.getHeadingname();
							         Headertable.getDefaultCell().setBorder(Rectangle.RIGHT | Rectangle.LEFT);
				            	...
				            	// PathologySampleWiseMaster labNObj =phleboService.getLabTestNormalValueDetialsForPrint(patientType, patientAgeType, patientAge,labNormalvalueTest);
									PathologySampleWiseMaster labNObj =phleboService.getLabNormalValuesForPrint(patientType, patientAgeType, patientAge,labCompObj.getTestIdd().intValue());
								...
								PathologySampleWiseSlave pslaveObj= pathoService.getPathologySamplewiseSlaveDetailsForPrint(labCompObj.getTestIdd().intValue(), profileId, treatmentId,list.get(pro).getSampleWiseMasterId());
									...
									//String unitNameGenaral=labUnitObj.getUnitName();
								...
								Units = "-";
									...
									if (TestName != null) {
										    	 ...
										    	 // Headertable.addCell(new Phrase(""+TestName, subheader)); 
										    	  Phrase p = new Phrase();
													...
													testNameCell.setBorder(Rectangle.LEFT | Rectangle.RIGHT );
									...
									if (TestResult == null || TestResult.equalsIgnoreCase("null")|| TestResult.equalsIgnoreCase("") || TestResult.equalsIgnoreCase("-")) {
										...
										cells26.setBorder(Rectangle.LEFT | Rectangle.RIGHT );
										...
										if(flagmarkResult.equalsIgnoreCase("L")){
											...
											cells26.setHorizontalAlignment(Element.ALIGN_LEFT);
											cells26.setBorder(Rectangle.LEFT | Rectangle.RIGHT );
											cells26.setBackgroundColor(Color.GREEN);
											Headertable.addCell(cells26);
											Headertable.addCell("");
											Headertable.addCell("");
											...
											cells26.setHorizontalAlignment(Element.ALIGN_LEFT);
											cells26.setBorder(Rectangle.LEFT | Rectangle.RIGHT );
											cells26.setBackgroundColor(Color.ORANGE);
											Headertable.addCell("");
											Headertable.addCell("");
											Headertable.addCell(cells26);
									...
									// start normal range and units
									String normalvalueUnit="";
									if(  labCompObj.getTestIdd().intValue() > 0 ){
										if(quantitative.equalsIgnoreCase("Y"))
												{
													if(biologicalReferenceChk.equalsIgnoreCase("Y")){
														...
														if (biologicalReferenceWithNormal == null || biologicalReferenceWithNormal.equalsIgnoreCase("null")|| biologicalReferenceWithNormal.equalsIgnoreCase("") ||  biologicalReferenceWithNormal.equalsIgnoreCase("null-") ||  biologicalReferenceWithNormal.equalsIgnoreCase("-null")) {
															   ...
															   normalvalueUnit=biologicalReferenceWithNormal;
													...
													if(biologicalReferenceChk.equalsIgnoreCase("Y")){
														...
														if(valueType.equalsIgnoreCase("individual"))
														{
																...
																normalvalueUnit="";
																   ...
																   normalvalueUnit=biologicalReferenceWithGeneral;
																	}
														}
									...
									PdfPCell cellsNormalUnits = new PdfPCell(new Phrase(normalvalueUnit+"   "+nunits , tabletext));
									cellsNormalUnits.setHorizontalAlignment(Element.ALIGN_LEFT);
									cellsNormalUnits.setBorder(Rectangle.LEFT | Rectangle.RIGHT );
								
									Headertable.addCell(cellsNormalUnits);
								
									
									//end normal range and units
