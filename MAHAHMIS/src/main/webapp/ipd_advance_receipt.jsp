...
<body>
	<%
		try {
			...
			response.setHeader("Content-Disposition", "inline; filename = IPD Advance Receipt");
			...
			if(lstPojo.size()>0){
				...
				userId = lstPojo.get(0).getCreatedBy();//ltpayModeDto.get(0).getCreatedBy();
		...
		/* 	p.add(new Chunk(" \nCIN: "+cinNo+", GST : "+gstNo , tabletext));	
			...
			HeaderTable2.addCell(new Phrase("Print Date:"+dtTime, subheader));
			...
			HeaderTable2.addCell(new Phrase("                   IPD ADVANCE RECEIPT ", subheader));	//jitendra IPD ADVANCE RECEIPT
			HeaderTable2.addCell(new Phrase("Print By: " + user_name, subheader));
