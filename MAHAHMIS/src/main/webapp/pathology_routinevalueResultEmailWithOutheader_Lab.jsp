		...
		Headertable3.getDefaultCell().setBorder(Rectangle.NO_BORDER);	
		   ...
		   for (int pro = 0; pro < list.size(); pro++) {	
			...
			Headertable1.flushContent();
				...
				Headertable.getDefaultCell().setBorder(Rectangle.BOTTOM);
				Headertable.addCell(new Phrase("", tabletext));
				Headertable.addCell(new Phrase("Test Done", tableheader));
				Headertable.addCell(new Phrase("Test Result", tableheader));
				Headertable.addCell(new Phrase("Units", tableheader));
				Headertable.addCell(new Phrase("Biological Reference Interval", tableheader));
			...
			for (int i = 0; i < list.get(pro).getTestli().size(); i++) {
				         ...
				         if((Integer)list.get(pro).getTestli().get(i).getTestId() == 0){
					         ...
					         Headertable.getDefaultCell().setBorder(Rectangle.NO_BORDER);
					         Headertable.addCell(new Phrase("", tableheader));
					 		 Headertable.addCell(new Phrase(""+headName, tableheader));
					 		 Headertable.addCell(new Phrase("", tabletext));
					 		 Headertable.addCell(new Phrase("", tabletext));
					 		 Headertable.addCell(new Phrase("", tabletext));
			...
			Headertable.addCell(new Phrase("" + TestName, tableheader));
			    ...
			    try {
			     ...
			     if(numeric){
			        	if (Double.parseDouble(TestResult) < Double.parseDouble(LowValues)|| Double.parseDouble(TestResult) > Double.parseDouble(HighValues)) {
							...
							//Headertable.addCell(new Phrase("" + TestResult + "*", tableheader));
							Headertable.addCell(new Phrase("" + TestResult, tableheader)); // Here * removed by vinod
						}else{
				        	Headertable.addCell(new Phrase("" + TestResult, tabletext));
				        }
			        }else{
			...
			Headertable11.addCell(new Phrase("", tabletext));
			 ...
			 if (list.get(pro).getComments().equalsIgnoreCase("-")) {
				 Headertable11.addCell(new Phrase(" ", tableheader));	
			 	 Headertable11.addCell(new Phrase(" ", tabletext));
			
			 }else
			 {
				 Headertable11.addCell(new Phrase("Comments :-", tableheader));
			...
			if (list.get(pro).getInterpretation().equalsIgnoreCase("-")) {	            	
				...
				Headertable11.addCell(new Phrase("Interpretation :-", tableheader));
			...
			Headertable11.flushContent();
			    ...
			    Headertable2.addCell(new Phrase("  ", footer));
				   ...
				   }else
				   {
					...
					Headertable2.writeSelectedRows(0, -1, document.left(document.leftMargin()), Headertable2.getTotalHeight() + document.bottom(document.bottomMargin() + 10), writer.getDirectContent());		
	        ...
	        Authenticator auth = new Authenticator() {
	         ...
	         Session mailSession = Session.getInstance(properties, auth);       
		        ...
		        // InternetAddress[] toAddresses = { new InternetAddress(mailTo) };
		         ...
		         msg.setSentDate(new Date());       	              // creates message part
