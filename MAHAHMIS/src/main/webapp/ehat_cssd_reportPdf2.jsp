...
<%@ page import="com.hms.dto.CssdMasterDTO"%>
...
<body>
	<%
		try {
			...
			Font subheader = new Font(Font.FontFamily.HELVETICA, 12, Font.BOLD);
			Font footer = new Font(Font.FontFamily.HELVETICA, 8, Font.BOLD);
			header.setColor(10, 4, 2);

			Font tableheader = new Font(Font.FontFamily.HELVETICA, 10, Font.BOLD);
			Font tabletext = new Font(Font.FontFamily.HELVETICA, 12, Font.NORMAL);
			...
			InventoryService inventoryNewService = (ApplicationContextUtils.getApplicationContext()).getBean(InventoryService.class);
			List<CssdMasterDTO> lstPojo1 = inventoryNewService.getCssdReport2(fromDate,toDate,subDept);
			...
			PdfPCell cells113 = new PdfPCell(new Phrase(""+subDept+" To CSSD",bold));
           ...
           HeaderTable15.flushContent();
			...
			int[] headerwidth5 = {4,8,8,7,7,7,7,7,7,7};
			...
			HeaderTable5.addCell(new Phrase("Name of Drum/Tary", subheader));				
			HeaderTable5.addCell(new Phrase("Qty", subheader));
			HeaderTable5.addCell(new Phrase("Content Checked", subheader));
			HeaderTable5.addCell(new Phrase("Issuer", subheader));
			HeaderTable5.addCell(new Phrase("Receiver", subheader));
			HeaderTable5.addCell(new Phrase("Dispached Date", subheader));
			HeaderTable5.addCell(new Phrase("Dispached By", subheader));
			HeaderTable5.addCell(new Phrase("Received By", subheader));
...
/* 			HeaderTable5.addCell(new Phrase("", tabletext));
			...
			// System.err.println("--------------spon---Count------------"+lst);
				...
				Date datetime=lstPojo1.get(j).getCreatedDate();
				String raisedBy=lstPojo1.get(j).getRaisedBy();
				String receivedBy=lstPojo1.get(j).getReceivedBy();
				Date datetime2=lstPojo1.get(j).getReutrnDate();
				int mrnStatus=lstPojo1.get(j).getMrnStatus();
		        ...
		        String strDate2 = dateFormat3.format(datetime);
		        String dispachedBy="";
		        String receivedBySubDep="";
				if(mrnStatus>=4)
				{
		        ...
		        for (int i = 0; i < lstPojo1.get(j).getLtCssdSlave().size(); i++) {
						...
						HeaderTable5.addCell(new Phrase(""+strDate2 ,tabletext));
						HeaderTable5.addCell(new Phrase(""+ lstPojo1.get(j).getLtCssdSlave().get(i).getItemName(),tabletext));	
						
						HeaderTable5.addCell(new Phrase(""+ lstPojo1.get(j).getLtCssdSlave().get(i).getSendQty(),tabletext));
						HeaderTable5.addCell(new Phrase("          ",tabletext));
						HeaderTable5.addCell(new Phrase(""+raisedBy,tabletext));
						HeaderTable5.addCell(new Phrase(""+receivedBy,tabletext));
						if(datetime2==null)
						{
							HeaderTable5.addCell(new Phrase("       ",tabletext));
						}else
						{
							   DateFormat dateFormat2 = new SimpleDateFormat("yyyy-MM-dd");
						        ...
						        //to convert Date to String, use format method of SimpleDateFormat class.
						        String strDate = dateFormat2.format(datetime2);
							HeaderTable5.addCell(new Phrase(""+strDate,tabletext));
						}
					
						
						HeaderTable5.addCell(new Phrase(""+dispachedBy,tabletext));
						HeaderTable5.addCell(new Phrase(""+receivedBySubDep,tabletext));
						
					/* 	HeaderTable5.addCell(new Phrase(""+lstPojo1.get(j).getLtCssdSlave().get(i).getPendingQty(),tabletext));
						HeaderTable5.addCell(new Phrase("          ",tabletext));
						
						HeaderTable5.addCell(new Phrase(""+lstPojo1.get(j).getLtCssdSlave().get(i).getRecievedQtySubDept(),tabletext));
						HeaderTable5.addCell(new Phrase("          ",tabletext)); */
