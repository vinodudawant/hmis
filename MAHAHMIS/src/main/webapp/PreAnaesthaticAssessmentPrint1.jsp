...
<body>
	 ...
	 */					
		...
		if(chartReportVitals != null )
		{
			...
			PdfPTable HeaderTable55 = new PdfPTable(10);
			int[] headerwidth55 = { 6,12,12,10,11,11,10,10,15,15};
			...
			/* HeaderTable55.addCell(new Phrase("Sr.No", subheader));
			 ...
			 HeaderTable55.addCell(new Phrase(""+bsl, subheader));*/
			
			 
			 List<ComplaintsDTO> otSlaveList = new ArrayList<ComplaintsDTO>();
			 otSlaveList =chartReportVitals.getListChartSlave(); 
			 
			 Map<String,Integer> map=new HashMap();
			 //Map<String,Integer> maptime=new HashMap();
			 Map<String,Integer> maptime=new LinkedHashMap();
			 
			 //System.err.println("otSlaveList======"+otSlaveList);
			 if(otSlaveList.size() > 0){
				 
				 String[] data;
				 for(int k=0; k < otSlaveList.size();k++){
				 ComplaintsDTO objChartReport = new ComplaintsDTO();
				 objChartReport = otSlaveList.get(k);
				 
			      if(map.containsKey(objChartReport.getDate())){
			    	  
			      }else{
			    	  map.put(objChartReport.getDate(), 1);
			      }
			      
			      if(maptime.containsKey(objChartReport.getTime())){
			    	  
		    	  }
		    	  else{	
		    		  maptime.put(objChartReport.getTime(),1 );
		    	  }
			   ...
			   Set<String> keySettime = maptime.keySet();  
			   ArrayList<String> maptimelist1=new ArrayList(keySettime);
	...
	for(int i=0;i<list.size();i++){
		...
		for(int j=0;j<maptimelist1.size();j++){
			
			int countSize=1;
			String time1=maptimelist1.get(j);
	...
	if(objChartReport.getDate().equalsIgnoreCase(date) && objChartReport.getTime().equalsIgnoreCase(time1)){
		
	//	if(objChartReport.getLoginUserId() ==  14)
	//	  {
			  temp1 = objChartReport.getStatus();
	//	  }
	//else if(objChartReport.getLoginUserId() ==  15){
			  bloodpre = objChartReport.getStatus();
		//  }else if(objChartReport.getLoginUserId() ==  16){
			  plusrate = objChartReport.getStatus();
		  //}else if(objChartReport.getLoginUserId() ==  17){
			  resprate = objChartReport.getStatus();
		  //}else if(objChartReport.getLoginUserId() ==  18){
			  oxygen = objChartReport.getStatus();
		 // }else if(objChartReport.getLoginUserId() ==  24){
			  bsl1 = objChartReport.getStatus();
		  // }
		  type = objChartReport.getCommentType();
		  time = objChartReport.getTime();
