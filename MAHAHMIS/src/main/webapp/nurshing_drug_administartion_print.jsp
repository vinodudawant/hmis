<%@page import="com.hms.ipd.nurshing.dto.NurshingDrugAdministartionDTO"%>
...
<body>
	...
	//	List<HospitalDetails> arrHospitalDetails = hs.getListHospitalDetails().getListHospitalDetails();
		...
		String dateDrug = request.getParameter("dateDrug");
		String checkDatePresent = request.getParameter("checkDate");
		...
		PdfPTable Headertable1 = new PdfPTable(3);
			...
			//document.newPage();
				...
				List<NurshingDrugAdministartionDTO> nObj = null;
				if(checkDatePresent.equalsIgnoreCase("dateWiseDrug"))
				nObj =  uss2.getNurshingDrugAdministartionlist(treatmentId, unitId,dateDrug);
				else
				nObj= uss2.getNurshingDrugAdministartionlist(treatmentId,unitId);
				 ...
				 PdfPTable HeaderTableSpace = new PdfPTable(1);
					...
					PdfPTable HeaderTableChem = new PdfPTable(8);
					int[] headerwidthChemo = {10,20,20,20,20,20,20,20 };
					HeaderTableChem.setWidths(headerwidthChemo);
					HeaderTableChem.setWidthPercentage(95f);
					HeaderTableChem.getDefaultCell().setBorder(Rectangle.BOX);
							
					HeaderTableChem.addCell(new Phrase("Sr.No", subheader));
					HeaderTableChem.addCell(new Phrase("Time", subheader));
					HeaderTableChem.addCell(new Phrase("Drug Name", subheader));
					HeaderTableChem.addCell(new Phrase("Strength", subheader));
					HeaderTableChem.addCell(new Phrase("Dose", subheader));
					HeaderTableChem.addCell(new Phrase("Quantity", subheader));
					HeaderTableChem.addCell(new Phrase("Route", subheader));
					HeaderTableChem.addCell(new Phrase("Frequency", subheader));
				         ...
				         if(nObj !=null){
				        	 ...
				        	 for(int i=0; i < nObj.size() ;i++){
				        		 ...
				        		 HeaderTableChem.addCell(new Phrase(""+nObj.get(i).getTime(), subheader));
				        		 HeaderTableChem.addCell(new Phrase(""+nObj.get(i).getDrugName(), subheader));
				        		 HeaderTableChem.addCell(new Phrase(""+nObj.get(i).getStrength(), subheader));
				        		 HeaderTableChem.addCell(new Phrase(""+nObj.get(i).getDose(), subheader));
				        		 HeaderTableChem.addCell(new Phrase(""+nObj.get(i).getQuantity(), subheader));
				        		 HeaderTableChem.addCell(new Phrase(""+nObj.get(i).getRoute(), subheader));
				        		 HeaderTableChem.addCell(new Phrase(""+nObj.get(i).getFrequency(), subheader));
				        		 index++;
				        			 }
				         		}
				         }
