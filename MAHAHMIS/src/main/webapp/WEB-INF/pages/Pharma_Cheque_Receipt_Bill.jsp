...
<body>
	...
	<c:forEach items="${chequeReceiptData}" var="row" varStatus="count">

         <c:set var="billId" value="${row.chequeReceiptId }" />

		<c:set var="billNumber" value="${row.chequeReceiptNo }" />

		<c:set var="saleDate" value="${row.chequeReceiptDate }" />

		<c:set var="vendorName" value="${row.vendorMaster.vendorName }" />
		
		<c:set var="vendorMobile" value="${row.vendorMaster.vendorMobileNumber }" />

   	<c:set var="vendorBank" value="${row.bankMaster.bankName }" />
   	
   		<c:set var="vendorBranch" value="${row.branchMaster.branchName}" />

			<c:set var="less" value="${row.chequeReceiptAmt}" />
		  
		  <c:set var="entrymadeby" value="${row.chequeReceiptMadeBy}" />
			...
			<%
					...
					String vendorBank = "";
					if(pageContext.getAttribute("vendorBank")
							.toString()==null)
					{
						vendorBank = "";
					}
					...
					{
						vendorBank =vendorBank + (String) pageContext.getAttribute("vendorBank")
								.toString();
					}
					
					
					String vendorBranch = "";
					if(pageContext.getAttribute("vendorBranch")
							.toString()==null)
					{
						vendorBranch = "";
					}
					...
					{
						vendorBranch =vendorBranch + (String) pageContext.getAttribute("vendorBranch")
								.toString();
					}
					...
					PdfPCell text = new PdfPCell(new Phrase("CHEQUE RECEIPT",
					subheader));
					...
					HeaderTable2.getDefaultCell().setBorder(Rectangle.BOTTOM);
					HeaderTable2
							.addCell(new Phrase("Cheque No:- " +billNumber, subheader));
		...
		<%
				...
				/* 	HeaderTable7.addCell(new Phrase("" , subheader)); */
					...
					HeaderTable7.addCell(new Phrase("", subheader));
					/* HeaderTable7.addCell(new Phrase("" , subheader)); */
					HeaderTable7.addCell(new Phrase("Vendor bank", subheader));
					HeaderTable7.addCell(new Phrase(vendorBank, subheader));
					
					HeaderTable7.addCell(new Phrase("", subheader));
					HeaderTable7.addCell(new Phrase("" , tabletext));  
					HeaderTable7.addCell(new Phrase("", subheader));
					/* HeaderTable7.addCell(new Phrase("" , subheader)); */
					HeaderTable7.addCell(new Phrase("Vendor branch", subheader));
					HeaderTable7.addCell(new Phrase(vendorBranch, subheader));
