...
<script type="text/javascript">
	function load(currentVal, id) {
		if ($('#textNo' + currentVal).val() != ""
			...
			setTimeout(function() {
				$('#textNo' + currentVal).focus();
			}, 00);
		}

	}

	function setValuesToAutocomplete(key,tcnt) {
		$("#tkr").val(tcnt);
		...
		jQuery
				.ajax({
					...
					success : function(r) {
						...
						$(".typeheadCounterPo"+rowCount).html(template);
						$(".typeheadCounterPo"+rowCount).show();

						setTimeout(
								function() {
									$('.typeheadCounterPo'+rowCount).typeahead({
										source : resultData,
										displayField : 'Name',
										valueField : 'ID',
									...
									$(".typeheadCounterPo"+rowCount).data('typeahead').source = resultData;
								}, 100);
					}
				});
	}
	...
	function loadCounterBatchPopUp(productId) {
		//getCounterByBatch(productId);
		getCounterByBatch2(productId);
	...
	function displayResult(item) {
		...
		$('#txtPre').val(content[9]);

		if (content[8] == '1') {
			$("#divSchH1")
					.html(
							"<font color=red size=4><strong>Sch H1 Product Billing Must</strong></font>");
		} else {
			$("#divSchH1").html("<font><strong></strong></font>");
		}
		/* loadCounterBatchPopUp(content[0]); */

		if (($("#pharmaFetchStockOptionForCounterSale").val()) != "FIFOExpiry")
			
	loadCounterBatchPopUp(content[0]);
		else
			getBatchDetailsByFIFOForCounterSale(content[0]);

	}

	function getBatchDetailsByFIFOForCounterSale(productId) {
		var storeName = "store";
		jQuery.ajax({
			async : true,
			type : "GET",
	...
	function setPopUpValues(number) {
		...
		$('#txtVat').val(selectedRowData.vat);
					...
					<div class="col-md-12-1" style="margin-top: 9px;">
						<div class="col-md-12-1" style="margin-top: 9px;">
							<div class="col-md-4-1" style="margin-top: 0px;">
								<div class="form-group" class="typeahead">
									<label for="product">Product</label> <input type="hidden"
										id="hiddenProductId" value="0" /> <input type="hidden"
										id="txtRateForPrint" /> <input type="text" id="particulars"
										name="txtProductName"
										class="form-control input-SmallText typeheadCounterPo "
										placeholder="Product" tabindex="1" maxlength="25"
										autocomplete="off"
								...
								<div class="col-md-3-1" style="margin-top: 0px;">
									<div class="form-group">
										<label for="product"> Quantity</label> <input type="text"
											...
											maxlength="25" required
											onblur="isNumber('txtQty',0,10),calculateCounterAmount('qty')"
											onblur='checkQtyforStock()'>
								...
								<div class="col-md-3-1" style="margin-top: 0px;">
									<div class="form-group">
										<label for="product">Amount</label> <input type="text"
											id="txtAmt" name="txtAmt" readonly txtAmt
								...
								<div class="col-md-8-1" style="margin-top: 20px; float: right;">
									...
									<div class="col-md-4-1" style="margin-top: -2px;">
										<input type="text" id="txtVat" readonly name="txtVat"
											...
											maxlength="25" required>
											<input type="hidden" id="txtVatid" readonly name="txtVat"
											class="form-control input-SmallText" placeholder="GST"
											maxlength="25" required>
									</div>
									<div class="col-md-1-1 center"
										style="margin-top: -7px; margin-left: -166px; color: red;">
										<b> *</b>
									</div>
								</div>
							</div>
...
<input type='hidden' value='0' id='txtPre' /> 
