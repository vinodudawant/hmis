...
<body>
	...
	<c:forEach items="${cashPaidData}" var="row" varStatus="count">

    <c:set var="billId" value="${row.cashPaidId }" />
 
		<c:set var="billNumber" value="${row.cashPaidDocId }" />

		<c:set var="saleDate" value="${row.cashPaidDate }" />
		...
		<c:set var="less" value="${row.cashPaidAmt}" />

		<c:set var="grossAmt" value="${row.cashPaidMadeBy}" />
		...
		<c:forEach items="${row.cashPaidSlaves}" var="vendor" varStatus="count">
          ...
          <c:set var="counter" value="${(count.index)+1}" />
			...
			<c:set var="billDate" value="${vendor.purchaseMaster.purBillDate }" />
