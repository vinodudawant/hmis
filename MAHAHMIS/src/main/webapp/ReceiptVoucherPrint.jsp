...
<%@page import="com.hms.administrator.dto.IPDReceiptVoucherDTO"%>
...
<title>Receipt Voucher Print</title>
</head>
<body>
<%
	try
	{
       ...
       //List<HospitalDetails> arrHospitalDetails = hs.getListHospitalDetails().getListHospitalDetails();
		...
		VouchersService AdCashVoucherCont = (VouchersService) (ApplicationContextUtils.getApplicationContext()).getBean(VouchersService.class);
		List<IPDReceiptVoucherDTO> lstGeneralVouchers = new ArrayList<IPDReceiptVoucherDTO>();
		lstGeneralVouchers = AdCashVoucherCont.fetchReceiptVoucherForPrint(voucherID,request);
		...
		{
		CompanyName= lstGeneralVouchers.get(i).getCompanyName();
		System.out.println("CompanyName "+CompanyName);
		}
		for(IPDReceiptVoucherDTO lst:lstGeneralVouchers)
		{
		//	 payTo=lst.getPayTo();
			...
			//  CashMode= lst.getPaymentMode();
			 	 GroupName=lst.getGroupName();
				 LedgerHead=lst.getLedgerHeadName();
				 RefTo=lst.getReceivedFrom();
				 amount=lst.getAmount();
				 PaidAmount = lst.getAmountPaid();
				 createdDate =(Date) lst.getInsertDateTime();
				 System.out.println("created date on jsp--> "+createdDate);
				 paymentMode = lst.getPaymentMode();
		...
		HeaderTable4.addCell(new Phrase("Payment Mode",subheader));		
		HeaderTable4.addCell(new Phrase(" : "+paymentMode1,tabletext));
		HeaderTable4.addCell(new Phrase("Group Name",subheader));		
		HeaderTable4.addCell(new Phrase(" : "+GroupName,tabletext));
		HeaderTable4.addCell(new Phrase("Ledger Head Name",subheader));		
		HeaderTable4.addCell(new Phrase(" : "+LedgerHead,tabletext));
		HeaderTable4.addCell(new Phrase("Refer To",subheader));		
		HeaderTable4.addCell(new Phrase(" : "+RefTo,tabletext));
