 ...
 *@code		:autosuggestion
 ***********/
function setallservautocompleteOnBillingIVF(inputID) {
	//var	listofunit=[];
	//var resultData = [];
	...
	jQuery.ajax({
		...
		success : function(r) {
			...
			autoCompDoctorDeskOnBillingIVF(r,inputID);
 ...
 ***********/
function autoCompDoctorDeskOnBillingIVF(response,id) {
 ...
 * **/
function setallchargesConfigOnBillingOPDIVF(inputID) {
	...
	if(sugVal != findingNames){
		...
		jQuery.ajax({
			...
			data : {
				...
				"serviceid" : serviceid,
				"categoryName" : encodeURIComponent(findingName)
			},
			url : "ehat/testautosuggest/getSponsorTestAutosuggestion",
			success : function(r) {
				autoCompConfigurationBillingIVF(r,inputID);
			}
		});
 ...
 ***********/
function autoCompConfigurationBillingIVF(response,id) {
...
function  closePatientTreatment() {
	...
	if (r == true) {
	   ...
	   "patientId" : pId,
		 ...
		 success : function(r) {
			 if(r==1){
		     		alertify.success("Close Treatment Successfully");
		     		window.location.reload(true);
			 }else{
				 alertify.error("Network Issuess...");
			 }
...
function autosuggetionForDefaultIVF(inputID) {
	var findingName = $("#" + inputID).val();
	var sugVal = $("#inputAuto").val();
	...
	if(sugVal != findingName){
		...
		if ( sponsorId >= 1 && chargesSlaveId > 0) {
			...
			setallservautocompleteOnBillingIVF(inputID);
			$("#inputAuto").val(findingName);
		}
	}	
}
...
function getBillAmountDetails(callFrom) {
	...
	jQuery.ajax({
		...
		url : "ehat/ivfbill/getAllAmountDetails",
		success : function(r) {
...
function  getIvfTreatIdByNormalTreatmentId() {
	 var treatId=$("#treatId").val();
	
		jQuery.ajax({
			type : "POST",
			url  : "ehat/ivfbill/getIvfTreatIdByNormalTreatmentId",
			data : {
	   "treatmentId" : treatId
			},
	     timeout : 1000 * 60 * 5,
