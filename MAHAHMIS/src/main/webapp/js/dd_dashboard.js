function fetchDoctorDeskDeshboard(callFrom,pageNumber) {
	...
	jQuery.ajax({
		...
		success : function(r) {
			if (callFrom == 'OPDLIST') {
				setOpdDoctorDeskDeshboard(r,pageNumber);
			} else if (callFrom == 'IPDLIST') {
				setIpdDoctorDeskDeshboard(r,pageNumber);
			} else if (callFrom == 'ERLIST') {
				setErDoctorDeskDeshboard(r,pageNumber);
			} else if (callFrom == 'CLOSEDLIST') {
				setClosedDoctorDeskDeshboard(r,pageNumber);
			}
		}
	});
}
function setOpdDoctorDeskDeshboard(r,pageNumber) {
	var pNo = 0;
	if (pageNumber != 0) {
		pNo = (pageNumber - 1);
	}
	...
	var index = 1;
		...
		// Added By Annapurna
		//var opdcount1=$("#unitCountOpddoctordesk").text();
	...
	for (var i = 0; i < r.listRegTreBillDto.length; i++) {
	    ...
	    date = date.getDate()+'-' +date.getMonth()+'-' +date.getFullYear() + ' ' + ("00" + date.getHours()).slice(-2) + ":"   + ("00" + date.getMinutes()).slice(-2) + ":" + ("00" + date.getSeconds()).slice(-2); ;*/
                var date=new Date(r.listRegTreBillDto[i].createdDateTime).toLocaleString();
     		    var createdDateTime = (date.split(",")[0]).split('/');
		...
		htm = htm
				...
				+ r.listRegTreBillDto[i].centerPatientId
				+ "</td>"
		...
		/*htm = htm+ "<td class='col-md-1-1 center' style='width:5%;'>"
				...
				htm = htm+ "<td class='col-md-1-1 center' style='width:5%;'>"
				+ "<button onclick=getOpdDoctorStation('"+ r.listRegTreBillDto[i].treatmentId+"','"+ r.listRegTreBillDto[i].patientId+"','OPDID'); type='button' class='btn btn-xs btn-success'><i class='fa fa-eye View'></i></button>"
				+ "</td>" 
				
				var abdmFlow2 = $('#abdmFLow').val()
				
				if(abdmFlow2=='on'){
				  if(r.listRegTreBillDto[i].healthId==null || r.listRegTreBillDto[i].healthId=="0"||
					  r.listRegTreBillDto[i].healthId==""){
					htm = htm+ "<td class='col-md-1-1 center' style='width:5%;'>"
					+ "<button  onclick=addCareContext('"+ r.listRegTreBillDto[i].patientId+"','OPDID'); type='button' class='btn btn-xs btn-success' disabled><i class='fa fa-plus-circle'></i></button>"
					+ "</td>" 
				}else
				{
					htm = htm+ "<td class='col-md-1-1 center' style='width:5%;'>"
					+ "<button  onclick=addCareContext('"+ r.listRegTreBillDto[i].patientId+"','OPDID'); type='button' class='btn btn-xs btn-success'><i class='fa fa-plus-circle'></i></button>"
					+ "</td>" 
				}
					}
				+ '</tr>';
...
function setIpdDoctorDeskDeshboard(r,pageNumber) {
	...
	for (var i = 0; i < r.listRegTreBillDto1.length; i++) {
		...
		var srNo = Number(noConcat+(i+1));
			//var date = new Date(r.listRegTreBillDto1[i].createdDateTime).toLocaleString();
		var date= new Date(r.listRegTreBillDto1[i].createdDateTime);
	    date = date.getDate()+'-' +date.getMonth()+'-' +date.getFullYear() + ' ' + ("00" + date.getHours()).slice(-2) + ":" 
		...
		htm = htm
				...
				+ r.listRegTreBillDto1[i].centerPatientId
				+ "</td>"
				+ " <td class='col-md-2-1 center'>"
		...
		htm = htm+ "<td class='col-md-1-1 center'>"
				...
				var abdmFlow2 = $('#abdmFLow').val()
				if(abdmFlow2=='on'){
					if(r.listRegTreBillDto1[i].healthId==null || r.listRegTreBillDto1[i].healthId=="0"
				||r.listRegTreBillDto1[i].healthId==""){	
				htm = htm+ "<td class='col-md-1-1 center' style='width:5%;'>"
				+ "<button  onclick=addCareContext('"+ r.listRegTreBillDto1[i].patientId+"','IPDID'); type='button' class='btn btn-xs btn-success' disabled><i class='fa fa-plus-circle'></i></button>"
				+ "</td>"
				}
				else{
					htm = htm+ "<td class='col-md-1-1 center' style='width:5%;'>"
					+ "<button  onclick=addCareContext('"+ r.listRegTreBillDto1[i].patientId+"','IPDID'); type='button' class='btn btn-xs btn-success'><i class='fa fa-plus-circle'></i></button>"
					+ "</td>"
				}
				}
				+ '</tr>';
...
function setErDoctorDeskDeshboard(r,pageNumber) {
...
function setClosedDoctorDeskDeshboard(r,pageNumber) {
	var pNo = 0;
	if (pageNumber != 0) {
		pNo = (pageNumber - 1);
	}
...
function serachDoctorDeskDeshboard(value, callFrom) {
	...
	if (callFrom == 'OPDLIST') {
		...
		$("#patientipd").attr("minlength", "10");
		$("#patientipd").attr("maxlength", "10");
		if(value == 'patientipd' )
		{
		...
		$("#patienter").attr("minlength", "10");
		$("#patienter").attr("maxlength", "10");
		if(value == 'patienter' )
		{
		...
		$("#patientclosed").attr("minlength", "10");
		$("#patientclosed").attr("maxlength", "10");
	...
	jQuery.ajax({
		...
		success : function(r) {
			if (callFrom == 'OPDLIST') {
				...
				setClosedDoctorDeskDeshboard(r,"0");
			}
		}
	});
}
...
function actionDashBoard(id,pid,callfrom)
{
	...
	window.location.href = "IPD_DoctorStation2.jsp?tid="+id+"&callfrom="+callfrom+"&pid="+pid;
}

function getOpdDoctorStation(id,pid,callfrom)
{
	window.location.href = "OPD_DoctorStation.jsp?tid="+id+"&callfrom="+callfrom+"&pid="+pid;
}

//redirect to sandbox add care context page
function addCareContext(pid,callfrom)
{
	window.location.href = "sandbox_add_care_context_by_mobile_otp.jsp?callfrom="+callfrom+"&pid="+pid;
}
...
/*//Added By Annapurna
function doctorDeskPatientOpdCount()
{
	var unitId=$("#unitId").val();
	var inputs = [];
	inputs.push('unitId=' + unitId);
	...
	jQuery.ajax({
		...
		type : "POST",
		data : str + "&reqType=AJAX",
		url : "ehat/doctordeskqueuecontroller1/doctorDeskPatientOpdCount",
		timeout : 1000 * 60 * 5,
		cache : false,
		error : function() {
			alert('error');
		...
		//	$("#unitCountOpd").text(r.opdcount);
			$("#unitCountOpddoctordesk").text(r.opdcount1);
