...
function assignBmwRequisition() {
	...
	alert("Are You Sure Want To Assigned Requisition.. ");
	
	 if($("input[name='fmdocid']:checked").length == 0){
		 alert("Plz Select At Least One Record to Assigned");
		 return false;
	 }
	...
	jQuery.ajax({
		type : "GET",
		url : "ehat/bmwRequisition/assignBmwRequisition",
		...
		if (CallFrom == "All") {
			for ( var i = 0; i < r.lstBmwRequisitionDetails.length; i++) {
				...
				htm = htm
					...
					//	+ ' <td class="col-md-1 center">'
						+r.lstBmwRequisitionDetails[i].assign
						+ '</td>'
						+ ' <td class="col-md-1 center">'
