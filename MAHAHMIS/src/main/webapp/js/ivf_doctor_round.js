...
function temForIvfDoctorround() {
	...
	if(prevtr == "previousIvfTreatment"){
		document.getElementById("iddrr").disabled = "true";
	}
}
...
function saveIvfDoctorRoundDetails() {
	...
	var ivf_treatmentId = $("#IVFTreatmentId").val();
	...
	for (var i = 1; i <= rows; i++) {
		...
		setListDoctorRoundIvf(DoctorRoundDtoDetails, treatmentId,ivf_treatmentId, patientId, doctorRoundid, time, tmName, clinicalNotes,
...
function setListDoctorRoundIvf(DoctorRoundDtoDetails, treatmentId,ivf_treatmentId, patientId,
	...
	DoctorRoundDtoDetails.listDoctorRound.push({
		...
		ivf_treatmentId : ivf_treatmentId,
		time : time,
		tmName : tmName,		
...
function checkFreshListSequeneceDR(tableId) {
	//alert('checkFreshListSequeneceDR.idt....'+tableId)
	var trLength = $('#' + tableId).find("tr:first th").length;

trLength = trLength + 1;

obj = $('#' + tableId + ' tbody tr td').find('input,span,td,select,textarea');
...
$.each(obj, function(key, value) {
	...
	if(idText == "time"){
		...
		$('#'+replaceById).attr('onClick','setTimeForDoctorRoundIvf('+idIndex+')');
	}
	
if(idText == "tmName"){
		
		$('#'+replaceById).attr('onchange','getTemplateDataByTempTdForIvfDoctorRound('+idIndex+')');
	}
...
function getDoctorListForEmbryoTransperIvfDoctor(callfrom) { 
	...
	jQuery.ajax({
		...
		for (var i = 0; i < r.listDoctorDetails.length; i++) {
			unitlistTemp = unitlistTemp + "<option value="
					+ r.listDoctorDetails[i].Doctor_ID + " ' data-name=" + r.listDoctorDetails[i].doc_name + "'>"
					+  r.listDoctorDetails[i].doc_name + "</option>";
		}
	...
	setTimeout(
			function() {
					...
					window .open(("ivf_doctor_round_Print.jsp?" + "patID="
							...
							+ "&ivftreatmentId=" + encodeURIComponent(ivftreatmentId) ));
			}, 300);
	return;	
}
...
function setPrescriptionTempForIvf(id) { 
	...
	$("#" + id).addClass("active");
         ...
         getUnitTypeListForIvfDoctorRound();   	
         getPreperationsListForIvfDoctorRound();       
         getAllPresInstructionsForIvfDoctorRound(); 
...
function getUnitTypeListForIvfDoctorRound() {
...
function getPreperationsListForIvfDoctorRound() { 
...
function getAllPresInstructionsForIvfDoctorRound() {
	
	var depType="IPD";
	 
	var inputs = [];
	inputs.push('depType=' + depType);
	...
	jQuery.ajax({
		...
		success : function(r) {  
			...
			for (var i = 0; i < r.prescriptionInstructionList.length; i++) {  
				
				unitlistTemp = unitlistTemp + "<option value="
						+ r.prescriptionInstructionList[i].presciptionInstructionId + " ' data-name=" +r.prescriptionInstructionList[i].englishInstruction/r.prescriptionInstructionList[i].hindiInstruction/r.prescriptionInstructionList[i].marathiInstruction + "'>"
						+ r.prescriptionInstructionList[i].englishInstruction+"/"+r.prescriptionInstructionList[i].hindiInstruction+"/"+r.prescriptionInstructionList[i].marathiInstruction + "</option>";
			}
			$('#instruction').html(unitlistTemp);
...
function temForIvfPrescription() { 
	...
	var temp = '<div class="tab-pane active" id="OrderForm">'
			...
			+ '<div class="col-md-2-1"><button id="saveTempIvf" class="btn btn-xs btn-success" data-toggle="modal"  style="margin-top: 10px;margin-left: -13px;"'
			...
			+ '<div class="col-md-5-1"><button id="savePrescriptionIvf" style="line-height: 1.2;float: right;" onclick="savePrescriptionIvf()" class="btn btn-xs btn-success editUserAccess">'
	...
	if(prevtr == "previousIvfTreatment"){
		//alert(prevtr);
		document.getElementById("savePrescriptionIvf").disabled = "true";
		document.getElementById("saveTempIvf").disabled = "true";
	}
...
function temForNewIvfHistory(id) {
	...
	if(prevtr == "previousIvfTreatment"){
		//alert(prevtr);
		document.getElementById("saveAddIpdHistory").disabled = "true";
	}
...
function  createDivForIvfHistory() {
	...
	+"</td>"	
		...
		+ rowCount
		+ '"   id="qty'
		+ rowCount +'"  " value="0" onkeyup="changeSlider('+rowCount+')" />'
		+'</td>'
...
function saveIvfHistory() {
	...
	if (r == true) { 
		...
		for ( var i = 1; i <= rowCount; i++) {
				...
				setlistIvfHisCompObj(listIvfHisCompObj,history_slave_id,chfdur,idAdddHisComp,duration,days_month_year,patientId,treatmentId,ivf_treatmentId,userId, unitId);				
		}
...
function setlistIvfHisCompObj(listIvfHisCompObj,history_slave_id,chfdur,idAdddHisComp,duration,days_month_year,patientId,treatmentId,ivf_treatmentId,userId, unitId) {
 ...
 * @codeFor	: Save/Delete voucher
 ...
 function ivfsaveGeneralVoucher() {
	...
	/* var userId = $("#userId").val();
	 ...
	 if(parseInt(txtAmount)<parseInt(totalPaidAmount))
		 {
		 alert("total paid amount should be less than Amount")
		 $("#totalPaidAmount").val('');
 ...
 function getNextGeneralVoucherId() {  
		...
		jQuery.ajax({
			...
			success : function(r) {
				...
				var generalVoucherId1=r;
				$("#txtVoucherid").val(generalVoucherId1);
