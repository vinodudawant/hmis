	...
	jQuery
			.ajax({
				...
				success : function(ajaxResponse) {
					...
					var followUpPatientTemplate1 = ' <tbody>{#foreach $T.liapp as liapp}{#if $T.liapp.aptyId=="FollowUp"}{#if $T.liapp.appdt=="'+ date +'"} '
						+ ' <tr class="gradeX" id="existing{$T.liapp.apid}"> '
						+ ' <td style="width: 5%">{ $T.liapp.apid} <div class="divide-10"></div></td>'
					    + ' <td style="width: 22%">{$T.liapp.title}{$T.liapp.patNm} {$T.liapp.lastName}<div class="divide-10"></div></td> '				    
					    + ' <td style="width: 10%">{ $T.liapp.mobNo} <div class="divide-10"></div></td>'
					    + ' <td style="width: 22%">{ $T.liapp.docNm} <div class="divide-10"></div></td>'
					    + ' <td class="center" style="width: 20%">{ $T.liapp.appdt}{ $T.liapp.aptf}<div class="divide-10"></div></td> '					    
					    + ' <td style="width: 20%"><button class="btn btn-xs btn-primary" type="submit" data-toggle="modal" data-target="" onclick="setFollowUpAppointmentType(),changeAppointmentOfPatient({$T.liapp.apid})" >change</button> '
					    ...
					    + '	<button class="btn btn-xs btn-success" onclick="registerPatient({$T.liapp.apid},\'mark\')" data-toggle="modal"	data-target="#MarkPatient">Mark Visit</button></td></tr>{#/if}{#/if}{#/for}';
					...
					var followUpPatientTemplate2 = temphtmlDate	+ '{#foreach $T.liapp as liapp} '
							+ ' <tr class="gradeX" id="existing{$T.liapp.apid}"> '
							+ ' <td style="width: 5%">{ $T.liapp.apid} <div class="divide-10"></div></td>'
							+ ' <td style="width: 22%">{$T.liapp.title}{$T.liapp.patNm} {$T.liapp.lastName}<div class="divide-10"></div></td>'
							+ ' <td style="width: 10%">{ $T.liapp.mobNo} <div class="divide-10"></div></td>'
							+ ' <td style="width: 22%">{$T.liapp.docNm}</td> '
							+ ' <td class="center" style="width: 20%">{ $T.liapp.appdt}</td> '
							+ ' <td style="width: 20%"><button class="btn btn-xs btn-primary" type="submit" data-toggle="modal" data-target="" onclick="getPatientDetailsFollowUp({$T.liapp.apid})" >Schedule Appointment</button> '
							+ ' <button class="btn btn-xs btn-danger" type="submit" data-toggle="modal"	data-target="" onclick="cancleFollowAppointment({$T.liapp.apid})">cancel</button></td></tr>{#/for} <tbody>';
					
					$("#followUpPatientList").setTemplate(followUpPatientTemplate2);
			...
			.ajax({
				...
				success : function(ajaxResponse) {
					...
					var followUpPatientTemplate1 = '<tbody>{#foreach $T.liapp as liapp}'
							...
							+ '{#if $T.liapp.patid=="0"}'
							+ '<button class="btn btn-xs btn-primary" type="submit" data-toggle="modal" data-target="" onclick="setReScheduleAppointmentType(),changeAppointmentOfNewPatient({$T.liapp.apid})" >change</button>&nbsp'
							+ '{#else}'
							+ '<button class="btn btn-xs btn-primary" type="submit" data-toggle="modal" data-target="" onclick="setReScheduleAppointmentType(),changeAppointmentOfPatient({$T.liapp.apid})" >change</button>&nbsp'
							+ '{#/if}'
							+ '{#else}'
							+ '<button class="btn btn-xs btn-primary" type="submit" data-toggle="modal" data-target="" onclick="setReScheduleAppointmentType(),changeAppointmentOfPatient({$T.liapp.apid})" >change</button>&nbsp'
							+ '{#/if}'
							
							+ '<button class="btn btn-xs btn-danger" type="submit" data-toggle="modal"	data-target="" onclick="cancelAppointment({ $T.liapp.apid})">cancel</button>&nbsp'
							+ '{#if $T.liapp.regType=="new"}'
								+ '{#if $T.liapp.patid=="0"}'
...
function registerPatient(appoid,callFrom) {
	...
	jQuery.ajax({
		...
		success : function(r) {
			...
			if(callFrom=="newReg"){
			window.location.href ="ehat_reg.jsp?" +"apid="
				+ encodeURIComponent(appoid);
			}
			if(callFrom=="mark"){
				window.location.href ="ehat_reg.jsp?" +"ptid="
				+ encodeURIComponent(patid) +"&apid="+ encodeURIComponent(appoid);
			}
...
function setAppTemp(r) {
	...
	$('#prefix').select2('val',r.lstAppointment[0].title);
	...
	setGender('ehat_patient');
  	...
  	/*$('#dob').val(r.lstAppointment[0].dob);
	$('#year').val(r.lstAppointment[0].age);
	$('#month').val(r.lstAppointment[0].ageMonths);
	$('#days').val(r.lstAppointment[0].ageDays);*/	
  	
  	$('#mName').val(r.lstAppointment[0].mName);
  	$('#dob').val(r.lstAppointment[0].dob);
	$('#year').val(r.lstAppointment[0].age);
	$('#month').val(r.lstAppointment[0].ageMonths);
	$('#days').val(r.lstAppointment[0].ageDays);
  	$('#talukaId').val(r.lstAppointment[0].talukaId);//locality
  	
	$('#townId').val(r.lstAppointment[0].townId);
	$('#districtId').val(r.lstAppointment[0].districtId);//district
	$('#stateId').val(r.lstAppointment[0].stateId);
	$('#country').val(r.lstAppointment[0].countryId);
	$('#areaCode').val(r.lstAppointment[0].areaCode);
	$('#addressText').val(r.lstAppointment[0].address);
	$('#peraddressText').val(r.lstAppointment[0].perAddress);
  	$('#pertalukaId').val(r.lstAppointment[0].pertalukaId);
	$('#pertownId').val(r.lstAppointment[0].pertownId);
 	$('#perdistrictId').val(r.lstAppointment[0].perdistrictId); 	
 	$('#perstateId').val(r.lstAppointment[0].perstateId);
 	$('#percountry').val(r.lstAppointment[0].percountryId);
 	$('#perareaCode').val(r.lstAppointment[0].perareaCode);
}
 /*
...
function updateAppointments() {
	...
	var title = $("#prefix").val();
	var patName = $("#fName").val();
	var patLastName = $("#lName").val();
	var patMob = $("#mobile").text();
...
function getDoctorNameList(callFrom) {
	...
	if (callFrom == "New") {
		inputs.push('departmentId=' + $("#selHosDeptNew").val());
	}else if (callFrom == "change") {
		
		var deptId = $("#chgDeptId").val();	
		inputs.push('departmentId=' + deptId);
	} else {
		inputs.push('departmentId=' + $("#selHosDept").val());
	}
			...
			.ajax({
				...
				success : function(r) {
					...
					if (callFrom == "New") {
						$("#selDoctorNameNew")
								.setTemplate(
										"<option value='0'>-Select-</option>{#foreach $T.listDoctor as listDoctor}<option value='{$T.listDoctor.di}'>{$T.listDoctor.dn}</option>{#/for}");
						$("#selDoctorNameNew").processTemplate(pobj1);
					} else {
						$("#selDoctorName")
								.setTemplate(
										"<option value='0'>-Select-</option>{#foreach $T.listDoctor as listDoctor}<option value='{$T.listDoctor.di}'>{$T.listDoctor.dn}</option>{#/for}");
						$("#selDoctorName").processTemplate(pobj1);
					}
				}
			});
}
	...
	setTimeout(
			function() {
				...
				var patName = patientDetails.tit + patientDetails.fn + " "
						+ patientDetails.ln;
				var templatePatientDetails = '<div class="panel panel-default"><div class="panel-body"><div class="divide-10"></div><table id="" cellpadding="0" cellspacing="0" border="0" class="datatable table table-striped table-bordered table-hover"><thead><tr><th>Patient ID: '
						+ patientDetails.pi
...
function scheduleAppointmentOfPatient(callFrom) {
	...
	var patMob;
	var noOfToken;	
	var queryType = $("#appQueryType").val();
	...
	jQuery
			.ajax({
				...
				success : function(r) {
					...
					//alert(pobj.liapp[5].aptyId);
					
					/*var existingPatientTemplate = '<tbody>{#foreach $T.liapp as liapp}{#if $T.liapp.aptyId=="Existing"}{#if $T.liapp.appdt=="'+ date+ '"} '
					   + ' <tr class="gradeX" id="existing{$T.liapp.apid}"> '
					   + ' <td style="width: 5%">{ $T.liapp.apid}<div class="divide-10"></div></td>'
					   + ' <td style="width: 22%">{$T.liapp.title}{$T.liapp.patNm} {$T.liapp.lastName}<div class="divide-10"></div></td>'
					   + ' <td style="width: 10%">{ $T.liapp.mobNo}<div class="divide-10"></div></td>'
					   + ' <td style="width: 22%">{ $T.liapp.docNm}<div class="divide-10"></div></td>'
					   + ' <td class="center" style="width: 20%">{ $T.liapp.appdt}{ $T.liapp.aptf}<div class="divide-10"></div></td>'					   
					   + ' <td style="width: 20%"> <button class="btn btn-xs btn-primary"	type="submit" data-toggle="modal" data-target="" onclick="setExistingAppointmentType(),changeAppointmentOfPatient({$T.liapp.apid})" >change</button>'
					   + ' <button class="btn btn-xs btn-danger" type="submit" data-toggle="modal"	data-target="" onclick="cancelAppointment({ $T.liapp.apid})">cancel</button>'
					   + ' <button class="btn btn-xs btn-success" onclick="registerPatient({$T.liapp.apid},\'mark\')" data-toggle="modal"	data-target="#MarkPatient">Mark Visit</button></td></tr>{#/if}{#/if}{#/for} <tbody>';
					   
					$("#existingPatientList").setTemplate(existingPatientTemplate);
					$("#existingPatientList").processTemplate(pobj);*/

					var newPatientTemplate1 = '<tbody>{#foreach $T.liapp as liapp}{#if $T.liapp.aptyId=="New"}{#if $T.liapp.patid=="0"}{#if $T.liapp.appdt=="'+ date + '"}'
					 + ' <tr class="gradeX" id="new{$T.liapp.apid}"> '
					 + ' <td style="width: 5%">{ $T.liapp.apid}<div class="divide-10"></div></td>'
					 + ' <td style="width: 22%">{$T.liapp.title}{$T.liapp.patNm} {$T.liapp.lastName}<div class="divide-10"></div></td> '
					 + ' <td style="width: 10%">{ $T.liapp.mobNo}<div class="divide-10"></div></td>'
					 + ' <td style="width: 22%">{ $T.liapp.docNm}<div class="divide-10"></div></td> '
					 + ' <td class="center" style="width: 20%">{ $T.liapp.appdt}{ $T.liapp.aptf}<div class="divide-10"></div></td> '									 
					 + ' <td style="width: 20%"><button class="btn btn-xs btn-primary"	type="submit" data-toggle="modal" data-target="" onclick="changeAppointmentOfNewPatient({$T.liapp.apid})" >change</button> '
					 + ' <button class="btn btn-xs btn-danger" type="submit" data-toggle="modal" data-target="" onclick="cancelAppointment({ $T.liapp.apid})">cancel</button> '
					...
					// + ' <button class="btn btn-xs btn-success"	type="submit" data-toggle="modal" data-target="" onclick="registerPatient({$T.liapp.apid},\'newReg\')" >Register</button></td></tr>{#/if}{#/if}{#/if}{#/for} <tbody>';
	...
	if ($("#appointmentType").val() == "New") {
		...
		for ( var j = 0; j < patientListArray.length; j++) {
			...
			newPatientTemplate1 = newPatientTemplate1
					...
					+ '<td style="width: 25%">' + patientListArray[j].title + ' ' + patientListArray[j].patNm + ' ' + patientListArray[j].lastName + '<div class="divide-10"></div></td>'
					+ '<td style="width: 25%">' + patientListArray[j].docNm + ''
					+ '<div class="divide-10"></div></td>'
					+ '<td class="center" style="width: 20%">' + patientListArray[j].appdt	+ '' + patientListArray[j].aptf + ''
					+ '<div class="divide-10"></div></td>'
					+ '<td style="width: 30%"> <button class="btn btn-xs btn-primary" type="submit"	data-toggle="modal" data-target="" onclick="changeAppointmentOfNewPatient('+ patientListArray[j].apid + ')">change</button>'
					+ '	<button class="btn btn-xs btn-danger" type="submit"	data-toggle="modal" data-target="" onclick="cancelAppointment('+ patientListArray[j].apid + ')">Cancel</button>'
					+ '	<button class="btn btn-xs btn-success" type="submit" data-toggle="modal" data-target="" onclick="registerPatient(' + patientListArray[j].apid +',\'newReg\')">Register</button>'
					+ '</td></tr>';
			/*
		...
		for ( var j = 0; j < patientListArray.length; j++) {
			existingPatientTemplate1 = existingPatientTemplate1
					...
					+ '<div class="divide-10"></div> </td><td style="width: 25%">'					
					+ patientListArray[j].docNm
					+ '<div class="divide-10"></div> </td><td class="center" style="width: 20%">'
					+ patientListArray[j].appdt
					+ ''
					+ patientListArray[j].aptf
					+ '<div class="divide-10"></div> <td style="width: 30%">'
					+ ' <button class="btn btn-xs btn-primary" type="submit" data-toggle="modal" data-target="" onclick="setExistingAppointmentType(),changeAppointmentOfPatient('+ patientListArray[j].apid +')" >change</button>'
					+ ' <button class="btn btn-xs btn-danger" type="submit" data-toggle="modal"	data-target="" onclick="cancelAppointment({ $T.liapp.apid})">cancel</button>'
				    + '	<button class="btn btn-xs btn-success"	onclick="registerPatient('+ patientListArray[j].apid + ',\'mark\')" data-toggle="modal"	data-target="#MarkPatient">Mark Visit</button></td></tr>';
		}
		...
		for ( var j = 0; j < patientListArray.length; j++) {
			followUpPatientTemplate1 = followUpPatientTemplate1
					+ '<tr class="gradeX" id="existing'	+ patientListArray[j].apid + '">'
					+ '<td style="width: 25%">'+ patientListArray[j].title	+ ' ' + patientListArray[j].patNm + ' '	+ patientListArray[j].lastName +'<div class="divide-10"></div>'
					+ '<td style="width: 25%">'+patientListArray[j].docNm+ '<div class="divide-10"></div>'
					+ '<td class="center" style="width: 20%">'+patientListArray[j].appdt+ ' '+ patientListArray[j].aptf +'<div class="divide-10"></div>'
					+ '<td style="width: 30%"><button class="btn btn-xs btn-primary" type="submit" data-toggle="modal" data-target="" onclick="setExistingAppointmentType(),changeAppointmentOfPatient('+ patientListArray[j].apid+ ')" >change</button>'
					+ '<button class="btn btn-xs btn-danger" type="submit" data-toggle="modal"	data-target="" onclick="cancelAppointment({ $T.liapp.apid})">cancel</button>'
					+ '<button class="btn btn-xs btn-success" onclick="registerPatient('+ patientListArray[j].apid + ',\"mark\")" data-toggle="modal"	data-target="#MarkPatient">Mark Visit</button></td></tr>';
		}
		...
		for ( var j = 0; j < patientListArrayFollowUp.length; j++) {
			followUpPatientTemplate1 = followUpPatientTemplate1
					+ '<tr class="gradeX" id="existing'+ patientListArrayFollowUp[j].apid + '">'
					+ '<td style="width: 25%">'+ patientListArrayFollowUp[j].patNm	+ '<div class="divide-10"></div> '
					+ '<td style="width: 25%">'+ patientListArrayFollowUp[j].docNm	+'<div class="divide-10"></div>	'
					+ '<td style="width: 20%">'+ patientListArrayFollowUp[j].appdt + '</td><td class="center">'+ patientListArrayFollowUp[j].appdt
					+ '<td style="width: 30%"><button class="btn btn-xs btn-primary" type="submit" data-toggle="modal" data-target="" onclick="getPatientDetailsFollowUp('+ patientListArrayFollowUp[j].apid+ ')" >Schedule Appointment</button>'
					+ '<button class="btn btn-xs btn-danger" type="submit" data-toggle="modal"	data-target="" onclick="cancelAppointment({ $T.liapp.apid})">cancel</button>'
					+ '</td></tr>';
		}
		...
		for ( var j = 0; j < patientListArrayReSchedule.length; j++) {
			reSchedulePatientTemplate1 = reSchedulePatientTemplate1
					...
					+ '<div class="divide-10"></div></td><td style="width: 30%">';				
					...
					if(patientListArrayReSchedule[j].aptyId == "New"){
						if(patientListArrayReSchedule[j].patid == "0"){
							...
							+ patientListArrayReSchedule[j].apid
							+ ')" >change</button> &nbsp&nbsp&nbsp&nbsp';
						}else{
							reSchedulePatientTemplate1 = reSchedulePatientTemplate1
					...
					reSchedulePatientTemplate1 = reSchedulePatientTemplate1 
						+ '<button class="btn btn-xs btn-danger" type="submit" data-toggle="modal"	data-target="" onclick="cancelAppointment('+ patientListArrayReSchedule[j].apid + ')">cancel</button> &nbsp&nbsp&nbsp&nbsp';
...
function changeAppointmentOfNewPatient(appointmentId) {
	...
	$("#hidpatId").val(myObj.patid);
	$("#appointmentId").val(myObj.apid);	
	$("#chgDeptId").val(myObj.bid);
	
	getDoctorNameList("change");
		
	setTimeout(function() {
		$("#selHosDept").val(myObj.bid);
		
		$("#prefix").val(myObj.title);
		$("#fName").val(myObj.patNm);
		$("#lName").val(myObj.lastName);
		$("#mobile").val(myObj.mobNo);
		setGender('ehat_patient');
		$("#selDoctorName").val(myObj.docid);
		setTimeout(function() {
			getDoctorTimeList();
	...
	$("#"+ id).mcautocomplete({
	    ...
	    select: function (event, ui) {
	       ...
	       if( ui.item.fn !='No Record Found'){
	    	   $('#results').text(ui.item.fn ? 'Selected: ' + ui.item.fn : 'Nothing selected, input was ' + this.value);
