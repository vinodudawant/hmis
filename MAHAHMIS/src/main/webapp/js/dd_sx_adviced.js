function getSexAdviceTemplate(id){
	...
	temp = temp
			...
			+ '<div class="divide-10"></div>'
			+ '<div class="divide-20"></div>'
			+ '<input class="btn btn-xs btn-success editUserAccess" type="button" onclick="saveAdviceOPD()" value="Save">'
...
function temForRadioTherapyOPD(id) {
	var temp = '<div id="Advices" class="tab-pane active">'
			...
			+ '<li id="RadioTherapyList" class="active" onclick=getOPDRadioTheropyListByTreatmentId();getRadionTechnique();getRadioTheropyCheckBox();setNewtempAdviceOPD(this.id);>'
			...
			+ '<select id="radiationId" class="TextFont" style="width:80%" name="">'
			+ '</select>'
			+ '</div>'
			+ '</div>'
			+ '</div>'
			...
			+ '<button class="btn btn-xs btn-primary" data-keyboard="false" data-backdrop="static" data-target="#iPackage" data-toggle="modal" style="width:80%" onclick="getIndivisualInstructionListOnRadioTheropy();" type="button">Add Instruction</button>'
			+ '</div>'
			...
			+ '<button id="cancel" class="btn btn-xs btn-danger" onclick="" data-dismiss="modal" style="margin-left: 790px;">Cancel</button>'
			+ '</div>'
			+ '</div>'
			+ '<div class="divide-10"></div>'
...
function temForChemoTherapyOPD(id) {
	var temp = '<div id="Advices" class="tab-pane active">'
			...
			/*+ '<a data-toggle="tab" href="#Chemotherapy" onclick=temForChemoTherapyOPD("id"),editOPDChemoByTreatmentIdAndDate("onload");setNewtempAdviceOPD(this.id);>Chemotherapy</a>'*/
			+ '<a data-toggle="tab" href="#Chemotherapy" onclick=editOPDChemoByTreatmentIdAndDate("onload");setNewtempAdviceOPD(this.id);>Chemotherapy</a>'
			+ '</li>'
			+ '<li id="PalliativeCareList">'
			+ '<a data-toggle="tab" href="#PalliativeCare" onclick=temForCareAdvicesOPD("careAdvices");editOPDCareAdvice();setNewtempAdviceOPD(this.id);>Care Advices</a>'
			+ '</li>'
...
function temForCareAdvicesOPD(id) {
	var temp = '<div id="Advices" class="tab-pane active">'
			...
			+ '<li id="PalliativeCareList" class="active" onclick=;editOPDCareAdvice();setNewtempAdviceOPD(this.id);>'
			...
			+ '<button style="margin-left: 20px" class="btn btn-xs btn-success" '
...
function temForPlanOfAdviceOPD(id) {
	var temp = '<div id="Advices" class="tab-pane active">'
			...
			+ '<li id="PlanOFTreatmentList" class="active" onclick=getPlanOfTreatmentCheckBox();getOPDPlanOfTreatmentListByTreatmentId();setNewtempAdviceOPD(this.id);>'
...
function setSxAdviceListForOPDDoctorStation(res){
	...
	if (res.getListOfOPDSxAdviceDTO.length > 0) {
		...
		//	alert("res.getListOfOPDSxAdviceDTO.length"+res.getListOfOPDSxAdviceDTO.length)
			...
			+ "<button class='btn btn-xs btn-success editUserAccess' onclick=editOPDSxAdvice("+res.getListOfOPDSxAdviceDTO[i].sxAdviceMasterId+") > <i class='fa fa-edit'></i> </button>"
           + '</td> '

			+ '	<td class="col-md-1-1 center">' 
			+ "<button class='btn btn-xs btn-danger editUserAccess' onclick=deleteOPDSxAdvice("+res.getListOfOPDSxAdviceDTO[i].sxAdviceMasterId+") > <i class='fa fa-trash-o'></i> </button>"
           + '</td> '
...
function setRadioTheropyListForOPDDoctorStation(res){
	...
	if (res.getListOfOPDTheropyDTO.length > 0) {
		...
		for (var i = 0; i < res.getListOfOPDTheropyDTO.length; i++) {
			...
			+ '	<td class="col-md-1-1 center">' 
			+ "<button class='btn btn-xs btn-success editUserAccess' onclick=editOPDRadioTheropy("+res.getListOfOPDTheropyDTO[i].radioTheropyMasterId+") > <i class='fa fa-edit'></i> </button>"
           + '</td> '

			+ '	<td class="col-md-1-1 center">' 
			+ "<button class='btn btn-xs btn-danger editUserAccess' onclick=deleteOPDRadioTheropy("+res.getListOfOPDTheropyDTO[i].radioTheropyMasterId+") > <i class='fa fa-trash-o'></i> </button>"
           + '</td> '
...
//	var billId=$("#billNo").text();    
	...
	var chemoDate = $("#chemoDate").val();
	 ...
	 var printTitle="CHEMOTHERAPY DETAILS";
  ...
  ///  var pendFlag = $("#pendingFlag").val();
    ...
    window.open("chemotheropy_print.jsp?treatmentId="+treatId+"&chemoDate="+chemoDate+"&printTitle="+printTitle+"&patientName="+patientName);
