...
function saveModule(){
	...
	var landPage = $('#landPage').val();
	var ModSequence = $('#ModSequence').val();
	var checkModule = $('#checkModule').val();
	if ($('[name="checkModule"]').is(':checked')) {
		checkModule = "Y";
	} else {
		checkModule = "N";
	}
...
function getModuleByModuleId(moduleId){
	jQuery.ajax({
		...
		success : function(response) {
			...
			$('#landPage').val(response.landPage);
			$('#ModSequence').val(response.ModSequence);
			
			var checkModule = response.checkModule;
			if (checkModule == "Y") {
				 $('#checkModule').prop('checked',true);
			} else {
				$('#checkModule').prop('checked',false);
			}
		}
	});
}
	...
	jQuery.ajax({
		...
		success : function(response) {
			...
			if(subModuleId!="" && subModuleId!=null){
				...
				$('#subModuleList').select2('val',subModuleId);
			}
		}
	});
	}
...
function getAllModule(){
	jQuery.ajax({
		...
		success : function(response) {
			...
			var printModuleBody="";
			for(var i=0;i<response.length;i++){
				tableBody = tableBody + "<tr><td style='width:10%'>"+index+""
				+"</td><td style='width:10%'>"+response[i].moduleId+"</td><td style='width:30%'>"+response[i].moduleName+"</td><td style='width:10%'>"
				+"<button class='btn btn-xs btn-success editUserAccess' onclick='getModuleByModuleId("+response[i].moduleId+")' ><i class='fa fa-edit'></i>"
				+"</button><td style='width:10%'><button class='btn btn-xs btn-danger deleteUserAccess' onclick='deleteModule("+response[i].moduleId+")' ><i class='fa fa-trash-o'></i></button></td></tr>";
				index++;
			...
			else if(currentPage=="user_access_sub_module_master" || currentPage=="general_access_print_master" || currentPage=="generalForm" || currentPage=="admin_general_form"){
...
function saveSubModule(){
	...
	var jspPageName = $('#jspPageName').val();
	var subSequence = $('#subSequence').val();
	var checksub = $('#checksub').val();
	if ($('[name="checksub"]').is(':checked')) {
		checksub = "Y";
	} else {
		checksub = "N";
	}
...
function getSubModuleBySubModuleId(subModuleId){
	...
	jQuery.ajax({
		...
		success : function(response) {
			...
			$('#moduleList').select2('val',response.moduleId);
			$('#subModuleType').val(response.subModuleType);
			$('#jspPageName').val(response.jspPageName);
			$('#subSequence').val(response.subSequence);
			
			if (response.checksub == 'Y') {
				$('#checksub').prop('checked',true);
			} else {
				$('#checksub').prop('checked',false);
			}
			$('#checksub').val(response.checksub);
...
function getAllProfile(){
	jQuery.ajax({
		...
		success : function(response) {
			...
			for(var i=0;i<response.length;i++){
				tableBody = tableBody + "<tr><td style='width:10%'>"+index+""
				+"</td><td style='width:10%'>"+response[i].profileId+"</td><td style='width:30%'>"+response[i].profileName+"</td><td style='width:10%'>"
				+"<button class='btn btn-xs btn-success editUserAccess' onclick='getProfileByProfileId("+response[i].profileId+")' ><i class='fa fa-edit'></i>"
				+"</button><td style='width:10%'><button class='btn btn-xs btn-danger deleteUserAccess' onclick='deleteProfile("+response[i].profileId+")'><i class='fa fa-trash-o'></i></button></td></tr>";
				index++;
			...
			if(currentPage=="user_access_role_master"){
				...
				$("#profileList_chosen").css("width","500px");
			}
...
function getRoleByRoleId(roleId){
	...
	jQuery.ajax({
		...
		success : function(response) {
			...
			$('#privilege_'+response.privilegesType).prop("checked","checked");
			$('#moduleBody :checkbox').map(function(){
				$('#'+this.id).prop('checked', false);
			});
...
function getAllRole(){
	jQuery.ajax({
		...
		success : function(response) {
			...
			for(var i=0;i<response.length;i++){
				tableBody = tableBody + "<tr><td style='width:10%;'>"+index+""
				+"</td><td style='width:10%;'>"+response[i].roleId+"</td><td style='width:30%;'>"+response[i].roleName+"</td><td style='width:10%;'>"
				+"<button class='btn btn-xs btn-success editUserAccess' onclick='getRoleByRoleId("+response[i].roleId+")' ><i class='fa fa-edit'></i>"
				+"</button><td style='width:10%;'><button id='deleteRole_"+index+"' class='btn btn-xs btn-danger deleteUserAccess' onclick='deleteRole("+response[i].roleId+")'><i class='fa fa-trash-o'></i></button></td></tr>";
				index++;
			...
			if(currentPage=="user_access"){
				...
				$(function() {
			        $('.chosen-select').chosen();
			        $('.chosen-select-deselect').chosen({ allow_single_deselect: true });
			        $("#roleList_chosen").css("width","500px");
			    });
...
/*	var inputDate=dateSelected.split("/");
	...
	jQuery.ajax({
		type : "POST",
		url : "useraccess/getLoginHistoryDateWise",
		timeout : 1000 * 60 * 5,
		cache : false,
		data : {
			"inputDate" : inputDate,
