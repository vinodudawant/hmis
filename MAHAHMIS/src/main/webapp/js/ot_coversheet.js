...
function setTemForcoversheet(id) {
	...
	var temp = '<div style="margin: 0px;" class="col-md-12-3" id="row1">'
			...
			+ '<i class="fa fa-eye View"></i>View All</label> <label style="cursor: pointer;display:none;" id="newVitals" onclick="showVitalsOnPopUp1()">'
	...
	if (r.listChartType.length > 0) {
		...
		for ( var i = 0; i < r.listChartType.length; i++) {
			...
			htm = htm
					...
					+"<td class='col-md-2-1 TextFont'> "+todays_date+"  </td>" 
		...
		success : function(r) {
			...
			for(var i=0; i< r.lstChartReport.length;i++){
				var neo_id = r.lstChartReport[i].am1;
				 ...
				 $("#vitalResult_"+i).text( r.lstChartReport[i].am8);
			}
									...
									if (r.litreatment.length > 0) {
										...
										for (var i = 0; i < r.litreatment.length; i++) {  
											...
											htm = htm
													...
													+ "<td class='col-md-1-1 center TextFont'> <span id='snum"
													+ rowCount
													+ "'>"
													+ rowCount
													...
													+ "<td class='col-md-3-1 center TextFont' >"
