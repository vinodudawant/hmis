...
function addDynamicRecordsToContactInfoTable(id) {
	...
	htm = htm
			...
			+ id + '" onclick="deletePartyMasterSlaveOnPO(' + id
			+ ',\'deleteContact\')" '
 ...
 */
function deletePartyMasterSlaveOnPO(id, callFrom) {
...
function setTableValuesToPurchaseOrderItemInfo(totalCheckboxes, totalRow,
	...
	var venderState= $("#hiddenVenderStatePO").val();
	if (totalRow >= 1) {
 ...
 * @param response
 */
function setPartyContactInfoOnPurchaseOrder(response) {
 ...
 * @author Rohit Sandbhor
 * @since 05112019
 * @comment Below js code save Purchase Order and slaves details w.r.t purchase
 *          order id
 ******************************************************************************/
function savePurchaseOrder() {
	...
	var getQuotation = $("#getQuotationId").val();
	...
	var purchaseOrderItemSlaveDetails = {
		lstPurchaseOrderItemSlaveDto : []
	};
	var purchaseOrderPartyContactDetails = {
		partyMasterContactInfoDto : []
	};
	var purchaseOrderPartyAddressDetails = {
		partyMasterAddressInfoDto : []
	};

	var rowsPurchaseOrderItemSlave = $('#ItemInfoTablePO tbody tr.newAdded').length;
	var rowsPurchaseOrderPartyContactSlave = $('#contactInfoTablePurchaseOrder tbody tr.newAdded').length;
	var rowsPurchaseOrderPartyAddressSlave = $('#addressInfoTablePurchaseOrder tbody tr.newAdded').length;
	...
	var totalIgstAmount1 = 0;
	var totalGstAMt1 = 0;
	for ( var i = 1; i <= rowsPurchaseOrderItemSlave; i++) {
		...
		totalGstAMt1 = $("#totalGstAmt").val();
		if(totalGstAMt1 == "NaN"){
			totalGstAMt1 = 0;
		}
		$("#totalGstAmt").val(parseInt(totalGstAMt1) + parseInt(txtPurchaseOrderTaxAmtinRs));
		// below is the item wise IGST amount value
		var igstAmountValue = $("#igstAmtId" + i).val();
		
		totalIgstAmount1  = $("#totaliGstAmt").val();	
		if(totalIgstAmount1 == "NaN"){
			totalIgstAmount1 = 0;
		}
		$("#totaliGstAmt").val(parseInt(igstAmountValue)+parseInt(totalIgstAmount1)); 
		...
		// this is added by Rohit for unit factor name
		...
		setPurchaseOrderItemSlaveList(purchaseOrderItemSlaveDetails,
	...
	for ( var i = 1; i <= rowsPurchaseOrderPartyContactSlave; i++) {
		var userId = $("#userId").val();
		var unitId = $("#unitId").val();
		var contactInfoId = $("#contactInfoId" + i).html();
		...
		setPurchaseOrderPartyContactSlaveList(purchaseOrderPartyContactDetails,
	...
	for ( var i = 1; i <= rowsPurchaseOrderPartyAddressSlave; i++) {
		...
		setPurchaseOrderPartyAddressSlaveList(purchaseOrderPartyAddressDetails,
	...
	purchaseOrderItemSlaveDetails = JSON
			.stringify(purchaseOrderItemSlaveDetails);
	purchaseOrderPartyContactDetails = JSON
			.stringify(purchaseOrderPartyContactDetails);
	purchaseOrderPartyAddressDetails = JSON
			.stringify(purchaseOrderPartyAddressDetails);
	...
	inputs.push('getQuotation=' + getQuotation);
	...
	// item slave details
	inputs.push('purchaseOrderItemSlaveDetails='
			+ encodeURIComponent(purchaseOrderItemSlaveDetails));
	// party contact details
	inputs.push('purchaseOrderPartyContactDetails='
			+ encodeURIComponent(purchaseOrderPartyContactDetails));
	// party address details
	inputs.push('purchaseOrderPartyAddressDetails='
			+ encodeURIComponent(purchaseOrderPartyAddressDetails));
	...
	jQuery.ajax({
		...
		type : "POST",
		data : str + "&reqType=AJAX",
		url : "ehat/inventoryPurchaseOrder/savePurchaseOrderModule",
		cache : false,
		success : function(r) {
			if (r[0] == 1) {
 ...
 * @author Rohit Sandbhor
 * @since 27-12-2019
 * @comment created below function to set purchase order item slave list details
 *          to pojo
 ...
 * @returns
 */
function setPurchaseOrderItemSlaveList(purchaseOrderItemSlaveDetails,
		itemSlaveId, itemMasterId, txtPurchaseQuotationItemName,
		txtPurchaseQuotationDocQuantity, txtPurchaseQuotationUnitPrice,
		txtPurchaseQuotationTrdeDiscountPercentage,
		txtPurchaseQuotationTrdeDiscountInRupess,
		...
		assetItemManufacture,assetAmcVal,assetPmVal,assetAmcYear,assetPmYear,assetProductWarranty,assetProductCategory,
		labEquipmentItemStatus) {
	purchaseOrderItemSlaveDetails.lstPurchaseOrderItemSlaveDto.push({
		id : itemSlaveId,
		...
		itemPurchaseFactorUom1 : (txtPurchaseQuotationFactorOne != 'undefined' && txtPurchaseQuotationFactorOne != null && txtPurchaseQuotationFactorOne != '' && txtPurchaseQuotationFactorOne !='null') ? txtPurchaseQuotationFactorOne : 0,
		itemPurchaseFactorUom2 : (txtPurchaseQuotationFactorTwo != 'undefined' && txtPurchaseQuotationFactorTwo != null && txtPurchaseQuotationFactorTwo != '' && txtPurchaseQuotationFactorTwo !='null') ? txtPurchaseQuotationFactorTwo : 0,
		itemPurchaseFactorUom3 : (txtPurchaseQuotationFactorThree != 'undefined' && txtPurchaseQuotationFactorThree != null && txtPurchaseQuotationFactorThree != '' && txtPurchaseQuotationFactorThree !='null') ? txtPurchaseQuotationFactorThree : 0,
		itemPurchaseFactorUom4 : (txtPurchaseQuotationFactorFour != 'undefined' && txtPurchaseQuotationFactorFour != null && txtPurchaseQuotationFactorFour != '' && txtPurchaseQuotationFactorFour !='null') ? txtPurchaseQuotationFactorFour : 0,
		...
		uomUnitFactorOneName : (uomUnitFactorOneNameId != 'undefined' && uomUnitFactorOneNameId != null && uomUnitFactorOneNameId != '' && uomUnitFactorOneNameId !='null') ? uomUnitFactorOneNameId : 0,
		uomUnitFactorTwoName : (uomUnitFactorTwoNameId != 'undefined' && uomUnitFactorTwoNameId != null && uomUnitFactorTwoNameId != '' && uomUnitFactorTwoNameId !='null') ? uomUnitFactorTwoNameId : 0,
		uomUnitFactorThreeName : (uomUnitFactorThreeNameId != 'undefined' && uomUnitFactorThreeNameId != null && uomUnitFactorThreeNameId != '' && uomUnitFactorThreeNameId !='null') ? uomUnitFactorThreeNameId : 0,
		uomUnitFactorFourName : (uomUnitFactorFourNameId != 'undefined' && uomUnitFactorFourNameId != null && uomUnitFactorFourNameId != '' && uomUnitFactorFourNameId !='null') ? uomUnitFactorFourNameId : 0,
		itemReagentStatus : (reagentItemStatus != 'undefined' && reagentItemStatus != null && reagentItemStatus != '') ? reagentItemStatus : 0,
		itemAssetStatus : (assetItemStatusId != 'undefined' && assetItemStatusId != null && assetItemStatusId != '') ? assetItemStatusId : 0,
		isItemBatchWise : (isItemBatchWiseId != 'undefined' && isItemBatchWiseId != null && isItemBatchWiseId !="") ? isItemBatchWiseId : "NO",
		manufactureName : (assetItemManufacture != 'undefined' && assetItemManufacture != null && assetItemManufacture !="") ? assetItemManufacture : 'NA',
		amcVal : (assetAmcVal != 'undefined' && assetAmcVal != null && assetAmcVal!="") ? assetAmcVal : 0,
		pmVal : (assetPmVal != 'undefined' && assetPmVal != null && assetPmVal !="") ? assetPmVal : 0,
		amcYear : (assetAmcYear != 'undefined' && assetAmcYear != null && assetAmcYear !="") ? assetAmcYear : 'NA',
		pmYear : (assetPmYear != 'undefined' && assetPmYear != null && assetPmYear!="") ? assetPmYear: 'NA',
		productWarranty : (assetProductWarranty != 'undefined' && assetProductWarranty != null && assetProductWarranty !="") ? assetProductWarranty : 'NA',
		productCategory : (assetProductCategory != 'undefined' && assetProductCategory != null && assetProductCategory!="") ? assetProductCategory : 'NA',
 ...
 * @author Rohit Sandbhor
 * @since 27-12-2019
 * @comment created below function to set purchase order party contact slave
 *          list to pojo
 ...
 * @param contactMail
 */
function setPurchaseOrderPartyContactSlaveList(
		purchaseOrderPartyContactDetails, contactPersonId,
	...
	purchaseOrderPartyContactDetails.partyMasterContactInfoDto.push({
		id : contactInfoId,
 ...
 *          pojo
 * @param purchaseOrderPartyAddressDetails
 * @param companyName
 * @param cityId
 * @param companyStreet
 * @param companyPin
 ...
 */
function setPurchaseOrderPartyAddressSlaveList(
		purchaseOrderPartyAddressDetails, addressInfoId, companyAddressType,
	...
	purchaseOrderPartyAddressDetails.partyMasterAddressInfoDto.push({
		id : addressInfoId,
...
function addDynamicRecordsToTermsAndConditionTableOnPurchaseOrder(id) {
	...
	htm = htm
			...
			+ ' <td class="col-md-1 center">'
			+ '	<button type="button" class="btn btn-xs btn-danger editBodyPartMaster" isNew="true" id ="deleteTermsAndCondition'
			+ id + '" onclick="deletePartyMasterSlaveOnPO('
 ...
 * @author Rohit Sandbhor
 ******************************************************************************/
/*function getAllPurchaseOrderRecordsDetails() {
	jQuery.ajax({
		async : true,
		type : "GET",
		url : "ehat/inventoryPurchaseOrder/getAllPurchaseOrderRecordsDetails",
		error : function() {
			alert('Network Issue..!!');
		},
		success : function(r) {
			setPurchaseOrderDataToTable(r);
 ...
 * @param r
 ******************************************************************************/
function setPurchaseOrderDataToTable(r) {
	var htm = "";
	var index = 1;
	for ( var i = 0; i < r.purchaseOrderDtos.length; i++) {
		htm = htm
				+ '<tr> '
				+ ' <td class="col-md-1 center">'
				+ index
				+ '</td>'
				+ ' <td class="col-md-1 center">'
				+ getDateTimePurchaseOrder(r.purchaseOrderDtos[i].createdDateTime)
				+ '</td>'
				+ ' <td class="col-md-1 center">'
				+ r.purchaseOrderDtos[i].id
				+ '</td>'
				+ ' <td class="col-md-1 center">'
				+ r.purchaseOrderDtos[i].supplierName
				+ '</td>'
				if(r.purchaseOrderDtos[i].isPoUsed == "N"){
					htm = htm	+ ' <td class="col-md-1 center">'
					+ '	<button class="btn btn-xs btn-success editBodyPartMaster"   data-toggle="modal" data-target="#purchaseOrderModuleModal"  onclick=editPurchaseOrder('
					+ r.purchaseOrderDtos[i].id
					+ ')><i class="fa fa-edit"></i></button></td>'
				}
				else{
					htm = htm	+ ' <td class="col-md-1 center">'
					+ '	<button class="btn btn-xs btn-success editBodyPartMaster" disabled data-toggle="modal" data-target="#purchaseOrderModuleModal"  onclick=editPurchaseOrder('
					+ r.purchaseOrderDtos[i].id
					+ ')><i class="fa fa-edit"></i></button></td>'
				}
				htm = htm + ' <td class="col-md-1 center">'
				+ '	<button class="btn btn-xs btn-danger editBodyPartMaster" onclick=deletePurchaseOrder('
				+ r.purchaseOrderDtos[i].id
				+ ')><i class="fa fa-trash-o"></i></button></td>'
				+ ' <td class="col-md-1 center">'
				+ '	<button class="btn btn-xs btn-success editBodyPartMaster" data-toggle="modal" data-target="#itemMasterModal" onclick=printPurchaseOrderMaster('
				+ r.purchaseOrderDtos[i].id
	...
	$("#purchaseOrderTableBodyId").html(htm);
}

/*******************************************************************************
 * @since 08-11-2019
 * @comment below js function to edit the purchase order data details
 * @author Rohit Sandbhor
 * @param id
 ******************************************************************************/
	...
	jQuery
			.ajax({
				...
				success : function(r) {
					...
					setParyMasterStateToEditPurchaseOrder(r.partyMasterDtos);
					$('#supplierStateId').select2('val', r.supplierStateId);
					$("#hiddenVenderStatePO").val(r.supplierStateId);
					...
					$("#getQuotationId").val(r.getQuotation);
					...
					//added  by rohit to set the total table size of PO item slave table
					$("#totaltblsize").val(r.purchaseOrderItemSlaveDto.length);
					for ( var i = 0; i < r.purchaseOrderItemSlaveDto.length; i++) {
						htm = htm
								...
								+ " <td style='display:none'><div><input type='hidden' value='"+ r.purchaseOrderItemSlaveDto[i].id+"' id='itemSlaveId"+ index+ "'></div></td> "
								...
								+ r.purchaseOrderItemSlaveDto[i].itemName
								...
								+ index
								+ "' value='"
								+ r.purchaseOrderItemSlaveDto[i].item_master_id
								+ "'/></div></td> "

								// added HSN ID here
								+ "<td style='display:none'><input type='text' disabled='disabled' value="
								+ r.purchaseOrderItemSlaveDto[i].hsnName
								...
								+ r.purchaseOrderItemSlaveDto[i].hsnNameValue
								...
								+ r.purchaseOrderItemSlaveDto[i].itemQuantity
						...
						// it
						if (r.purchaseOrderItemSlaveDto[i].itemTradeDiscountRupees == 0) {
							htm = htm
									+ "<td><input type='text' class='form-control input-SmallText' disabled value="
									+ r.purchaseOrderItemSlaveDto[i].itemTradeDiscountRupees
						...
						htm = htm
								+ " <td><input type='text' class='form-control input-SmallText' value="
								+ r.purchaseOrderItemSlaveDto[i].itemTradeDiscountAmount
								+ "  id='txtPurchaseQuotationTrdeDiscountAmt"
								+ index
								+ "' onkeypress='return validateNumbers(event);'   style='width:60px;'></td>"
								+ "<td><input type='text' class='form-control input-SmallText' value="
								+ r.purchaseOrderItemSlaveDto[i].itemTradeBaseAmount
								...
								+ r.purchaseOrderItemSlaveDto[i].gst
						...
						if (r.purchaseOrderItemSlaveDto[i].igst == 0) {
							htm = htm
									...
									+ r.purchaseOrderItemSlaveDto[i].igst
						...
						htm = htm
								+ "<td><input type='text' class='form-control input-SmallText' value="
								+ r.purchaseOrderItemSlaveDto[i].gstAmountValue
								+ " style='width:100px;' id='txtPurchaseOrderTaxAmtinRs"
								+ index + "'   ></td> "
						// below is the IGST total amount
						// if igst amount is '0' we disabled it
						if (r.purchaseOrderItemSlaveDto[i].igstAmountValue == 0) {
							htm = htm
									+ "<td><input type='text' class='form-control input-SmallText' disabled value="
									+ r.purchaseOrderItemSlaveDto[i].igstAmountValue
						...
						htm = htm
								+ "<td><input type='text' class='form-control input-SmallText' value="
								+ r.purchaseOrderItemSlaveDto[i].totalAmount
								...
								+ r.purchaseOrderItemSlaveDto[i].itemPurchaseFactorUom1
								...
								+ index
								+ "'>"
								+ r.purchaseOrderItemSlaveDto[i].uomUnitFactorOneName
								+ " </lable></td> "
								+ "<td><input type='text' class='form-control input-SmallText' onkeypress='return validateNumbers(event);' value="
								+ r.purchaseOrderItemSlaveDto[i].itemPurchaseFactorUom2
								...
								+ index
								+ "'>"
								+ r.purchaseOrderItemSlaveDto[i].uomUnitFactorFourName
								+ " </lable></td> "
								+ " <td><input type='text' class='form-control input-SmallText' value="
								+ r.purchaseOrderItemSlaveDto[i].itemActualQuantity
								+ " id='txtPurchaseQuotationActualQuantity"
								+ index
								+ "' onkeypress='return validateNumbers(event);' style='width:60px;'></td> "
								+ "<td style='display:none'><input type='text' class='form-control input-SmallText'  value="
								+ r.purchaseOrderItemSlaveDto[i].itemPendingQuantity
								...
								+ "<td style='display:none;' ><input type='text' class='form-control input-SmallText'  value="
								+ r.purchaseOrderItemSlaveDto[i].itemAssetStatus
								+ "  id='assetItemStatusId"
								+ index
								+ "'' style='width:60px;'></td> "
								
								+ "<td style='display:none;' ><input type='text' class='form-control input-SmallText'  value="
								+ r.purchaseOrderItemSlaveDto[i].isItemBatchWise
								...
								//added reagent type by rohit
								+ "<td style='display:none;' ><input type='text' class='form-control input-SmallText'  value="+ r.purchaseOrderItemSlaveDto[i].itemReagentStatus + "  id='reagentItemStatusId" + index + "' style='width:60px;'></td> "
								
								//added by rohit on 12-08-2020 asset manufacture name 
								+ "<td class='col-md-6 col-xs-12 col-sm-6 center' style='display:none;'><input type='text'  value="+ r.purchaseOrderItemSlaveDto[i].manufactureName +"   id='assetItemManufactureId"
								+ index
								+ "' class='form-control input-SmallText'> </td>"
								+
								
								//added by rohit on 12-08-2020 asset amc value
								"<td class='col-md-6 col-xs-12 col-sm-6 center' style='display:none;'><input type='text'  value="+ r.purchaseOrderItemSlaveDto[i].amcVal +" id='assetAmcValId"
								+ index
								+ "' class='form-control input-SmallText'> </td>"
								+
								
								//added by rohit on 12-08-2020 asset pm value
								"<td class='col-md-6 col-xs-12 col-sm-6 center' style='display:none;'><input type='text'  value="+ r.purchaseOrderItemSlaveDto[i].pmVal +" id='assetPmValId"
								+ index
								+ "' class='form-control input-SmallText'> </td>"
								+
								
								//added by rohit on 12-08-2020 asset amc year value
								"<td class='col-md-6 col-xs-12 col-sm-6 center' style='display:none;'><input type='text'  value="+ r.purchaseOrderItemSlaveDto[i].amcYear +" id='assetAmcYearId"
								+ index
								+ "' class='form-control input-SmallText'> </td>"
								+
								
								//added by rohit on 12-08-2020 asset pm year value
								"<td class='col-md-6 col-xs-12 col-sm-6 center' style='display:none;'><input type='text'  value="+ r.purchaseOrderItemSlaveDto[i].pmYear +" id='assetPmYearId"
								+ index
								+ "' class='form-control input-SmallText'> </td>"
								+
								
								//added by rohit on 12-08-2020 asset product warranty value
								"<td class='col-md-6 col-xs-12 col-sm-6 center' style='display:none;'><input type='text'  value="+ r.purchaseOrderItemSlaveDto[i].productWarranty +" id='assetProductWarrantyId"
								+ index
								+ "' class='form-control input-SmallText'> </td>"+
								
								//added by Rohit on 12-08-2020 to store product category type
								"<td class='col-md-6 col-xs-12 col-sm-6 center' style='display:none;'><input type='text'  value="+ r.purchaseOrderItemSlaveDto[i].productCategory +" id='assetProductCategory"
								+ index
								+ "' class='form-control input-SmallText'> </td>"+
								
								//added by rohit on 12-08-2020 lab equipment item type
								"<td class='col-md-6 col-xs-12 col-sm-6 center' style='display:none;'><input type='text'  value="+ r.purchaseOrderItemSlaveDto[i].itemLabEquipmentStatus +" id='labEquipmentItemStatusId"
 ...
 * @returns {Boolean}
 ******************************************************************************/
function fetchPurchaseOrderDetails(inputID) {
	var resultData = [];
	...
	jQuery.ajax({
		...
		type : "POST",
		data : str + "&reqType=AJAX",
		url : "ehat/inventoryPurchaseOrder/autoFillSearchPurchaseOrder",
		cache : false,
		success : function(r) {

			for ( var j = 0; j < r.purchaseOrderDtos.length; j++) {

				var arrValue = r.purchaseOrderDtos[j].id + "-"
						+ r.purchaseOrderDtos[j].supplierName;
				var id = r.purchaseOrderDtos[j].id;
				var supplierName = r.purchaseOrderDtos[j].supplierName;
				resultData.push({
					ID : id,
					Name : supplierName,
				});
			...
			setTimeout(function() {
				...
				$("input#" + inputID).typeahead({
					...
					displayField : 'Name',
					valueField : 'ID',
					onSelect : displayPurchaseOrderSearchResult,
					scrollBar : true
				});
	...
	function displayPurchaseOrderSearchResult(item) {
		var res = item.text.split('-');
		var purchaseOrderId = res[0];
		var supplierName = res[1];
		getPurchaseOrderDetailsBySupplierName(supplierName);
 ...
 *            Name
 ******************************************************************************/
function getPurchaseOrderDetailsBySupplierName(supplierName) {
	var inputs = [];
	inputs.push('supplierName=' + supplierName);
	var str = inputs.join('&');
	jQuery.ajax({
...
function deletePurchaseOrder(id) {
	...
	if (r == true) {
		jQuery.ajax({
			type : "POST",
			url : "ehat/inventoryPurchaseOrder/deletePurchaseOrder",
			data : {
				"id" : id
			},
			timeout : 1000 * 60 * 5,
 ...
 ******************************************************************************/
function getPurchaseQuotationOnPurchaseOrder(call) {
	var unitId = $("#unitId").val();
 ...
 * @codeFor : getAllPurchaseQuotationMaster()
 ******************************************************************************/
function getPurchaseQuotationMasterOnPurchaseOrder(pQId) {
	
	if(pQId == "0"){
		alertify.error("Please Select Purchase Quotation...");
		return false;
	}
	var unitId = $("#unitId").val();
	var inputs = [];
	...
	jQuery.ajax({
		...
		success : function(r) {
			...
			$("#hiddenSupplierNameId").val(r.supplierName);
			//setParyMasterStateToPurchaseOrder(r.partymasterdto);
			$("#referenceNoId").val(r.referenceNumber);
			if(r.quotationStatus == 1){
				$("#orderStatusId").val("Open"); 
			}else if(r.quotationStatus == 2){
				$("#orderStatusId").val("Closed"); 
			}else if(r.quotationStatus == 3){
				$("#orderStatusId").val("Hold"); 
			}else if(r.quotationStatus == 4){
				$("#orderStatusId").val("Cancelled"); 
			}else{
				$("#orderStatusId").val(0); 
			}
			
			$('#supplierStateId').select2('val', r.supplierState);
			$("#supplierMobileNoId").val(r.mobileNumber);
			$("#supplierAddressId").val(r.supplierAddress);

			$("#lessSpecialDiscountId").val(r.specialDiscount);
			$("#lessDebitAmountId").val(r.debitAmount);
			$("#lessCDPercent1Id").val(r.cdPercentage);
			$("#lessCDPercent2Id").val(r.cdAmount);
			$("#addOctroiId").val(r.octroi);
			$("#addSurchargeId").val(r.surcharge);
			$("#addCreditAmountId").val(r.creditAmount);
			$("#addFreightId").val(r.freight);
			$("#taxVatId").val(r.vat);
			$("#taxLBTId").val(r.lbt);
			$("#taxCSTId").val(r.cst);
			$("#taxExVatId").val(r.exVat);
			$("#taxTotalTaxesId").val(r.totalTax);
			$("#grossAmountId").val(r.grossAmount);
			$("#grossLessAmountId").val(r.less);
			$("#grossAddAmountId").val(r.addAmount);
			$("#grossTaxesId").val(r.igstTotalAmount);
			$("#grossNetAmountId").val(r.grossTotalAmount);
			$("#taxIgstId").val(r.totalIgstAmount);
			setPurchaseQuotationOnPurchaseOrder(r);
		}
	});
 ...
 * @author Rohit Sandbhor
 * @since 27-12-2019
 * @comment created below function to set purchase quotation on purchase order
 * @param response
 */
function setPurchaseQuotationOnPurchaseOrder(response) {

	var length = 0;
	if (response.lstpurchaseitemInfoDto.length != 0
			&& response.lstpurchaseitemInfoDto != null
			&& response.lstpurchaseitemInfoDto != "") {
		...
		for ( var i = 0; i < length; i++) {
			...
			htm = htm
					...
					+ id
					+ "'>"
					+ "<td><input type='checkbox' class='chkMrnItem' value='"
					+ id
					+ "' id='checkbox"
					+ id
					...
					+ id
					+ "</td>"
					+ " <td style='display:none'><input type='hidden'  id='itemMasterId"
					+ id
					+ "' value='"+response.lstpurchaseitemInfoDto[i].itemMasterId+"'/><div><input type='hidden' value='0' id='itemSlaveId"+ id +"'></div></td> "
					+ "<td><input type='text' style='width: 100px;' id='txtPurchaseQuotationItemName_"
					+ id
					+ "' class='form-control input-SmallText' value='"
					+ response.lstpurchaseitemInfoDto[i].itemName
					+ "'>"
					+ "</td>"

					// added HSN ID here
					+ "<td style='display:none'><input type='text' disabled='disabled' value="
					+ response.lstpurchaseitemInfoDto[i].hsnName
					+ " class='form-control input-SmallText' id='hsnCodePurchaseOrder"
					+ id
					+ "'  style='width:90px;'></td>"
					...
					// added HSN Name here
					+ "<td><input type='text' disabled='disabled' value="
					+ response.lstpurchaseitemInfoDto[i].hsnNameValue
					+ " class='form-control input-SmallText' id='hsnCodeNamePurchaseOrder"
					+ id
					+ "'  style='width:90px;'></td>"

					+ "<td><input type='text' style='width: 80px;' onkeypress='return validateNumbers(event)'  onkeyup='totalAmount(this.id,"
					+ id
					+ ")' id='txtPurchaseQuotationDocQuantity"
					+ id
					+ "' class='form-control input-SmallText' value='"
					+ response.lstpurchaseitemInfoDto[i].itemQuantity
					+ "'>"
					+ "</td>"
					+ "<td><input type='text' style='width: 80px;' id='txtPurchaseQuotationUnitPrice"
					+ id
					+ "' class='form-control input-SmallText' value='"
					+ response.lstpurchaseitemInfoDto[i].itemUnitPrice
					+ "'>"
					+ "</td>"
					+ "<td><input type='text' style='width: 80px;'  onkeyup='chkTradAmtorPercentage(this.id,"
					+ id
					+ ")' onblur='calculTradeDis(this.id,"
					+ id
					+ ")' id='txtPurchaseQuotationTrdeDiscountPercentage"
					+ id
					+ "' class='form-control input-SmallText' value='"
					+ response.lstpurchaseitemInfoDto[i].itemDiscountPer
					+ "'>"
					+ "</td>"
					+ "<td><input type='text' style='width: 80px;'  onkeypress='return validateNumbers(event)' onkeyup ='chKTradAmt(this.id,"
					+ id
					+ ")' id='txtPurchaseQuotationTrdeDiscountInRupess"
					+ id
					+ "' class='form-control input-SmallText' value='"
					+ response.lstpurchaseitemInfoDto[i].itemDiscountRs
					+ "'>"
					+ "</td>"
					+ "<td><input type='text' style='width: 80px;' disabled id='txtPurchaseQuotationTrdeDiscountAmt"
					+ id
					+ "' onkeypress='return validateNumbers(event)' class='form-control input-SmallText' value='"
					+ response.lstpurchaseitemInfoDto[i].itemDiscountAmt
					+ "'>"
					+ "</td>"
					+ "<td><input type='text' style='width: 80px;' id='txtPurchaseQuotationBaseAmount"
					+ id
					+ "' onkeypress='return validateNumbers(event)' class='form-control input-SmallText' value='"
					+ response.lstpurchaseitemInfoDto[i].itemBaseAmt
					+ "'>"
					+ "</td>";
				...
				if(supplierState == userState){
					// gst value
					htm = htm
					+ "<td><input type='text' style='width: 80px;' id='txtPurchaseQuotationTaxCodePO_"
					+ id
					+ "' onkeyup='calculateGSTWisePurchaseOrder(this.id,"
					+ id
					+ ");calculateTotalItemAmountPurchaseOrder("
					...
					+ ");calculateTotalItemgstAmountPurchaseOrder("
					+ id
					+ ");autotaxCodeforItemGstPurchaseOrder("+ id + "," + id + ",this.id);' class='form-control input-SmallText' value='"
					+ response.lstpurchaseitemInfoDto[i].itemGst
					+ "'>"
					+ "</td>"
					// igst value
					+ "<td><input type='text' style='width: 80px;' disabled onkeyup='calculateIGSTWisePurchaseOrder(this.id,"
					+ id
					+ ");calculateTotalItemAmountPurchaseOrder("
					+ id
					+ ");calculateTotalItemigstAmountPurchaseOrder("
					+ id
					+ ");autotaxCodeforItemigstPurchaseOrder(" + id + "," + id + ",this.id);' value='0' id='txtPurchaseQuotationTaxAmount_"
					+ id
					+ "' class='form-control input-SmallText' value='"
					+ response.lstpurchaseitemInfoDto[i].itemIgst
					+ "'>"
					+ "</td>";
				}else {
					htm = htm
					+ "<td><input type='text' disabled style='width: 80px;' id='txtPurchaseQuotationTaxCodePO_"
					+ id
					+ "' onkeyup='calculateGSTWisePurchaseOrder(this.id,"
					+ id
					+ ");calculateTotalItemAmountPurchaseOrder("
					...
					+ "</td>"
					// igst value
					+ "<td><input type='text' style='width: 80px;' onkeyup='calculateIGSTWisePurchaseOrder(this.id,"
					+ id
					+ ");calculateTotalItemAmountPurchaseOrder("
					+ id
					+ ");calculateTotalItemigstAmountPurchaseOrder("
					...
					// gst amount in RS
				htm = htm	+ "<td><input type='text' disabled style='width: 80px;' id='txtPurchaseOrderTaxAmtinRs"
					+ id
					+ "' class='form-control input-SmallText' value='"
					+ response.lstpurchaseitemInfoDto[i].itemGstAmt
					+ "' >"
					+ "</td>"
					// added by rohit
					// igst amount in RS
					+ "<td><input type='text' disabled style='width: 80px;' id='igstAmtId"
					+ id
					+ "' class='form-control input-SmallText' value='"
					+ response.lstpurchaseitemInfoDto[i].itemToatIgstAmt
					+ "' >"
					+ "</td>"
					+ "<td><input type='text' style='width: 80px;' id='txtPurchaseQuotationRowAmount1"
					+ id
					+ "' class='form-control input-SmallText' value='"
					+ response.lstpurchaseitemInfoDto[i].itemTotalAmt
					+ "'>"
					+ "</td>"
					+ "<td><input type='text' style='width: 80px;' id='txtPurchaseQuotationFactorOne"
					+ id
					+ "' class='form-control input-SmallText' value='"
					+ response.lstpurchaseitemInfoDto[i].itemFactor1
					+ "'>"
					+ "</td>"
					+ "<td><input type='text' style='width: 80px;' id='txtPurchaseQuotationFactoTwo"
					+ id
					+ "' class='form-control input-SmallText' value='"
					+ response.lstpurchaseitemInfoDto[i].itemFactor2
					+ "'>"
					+ "</td>"
					+ "<td><input type='text' style='width: 80px;' id='txtPurchaseQuotationFactorThree"
					+ id
					+ "' class='form-control input-SmallText' value='"
					+ response.lstpurchaseitemInfoDto[i].itemFactor3
					+ "'>"
					+ "</td>"
					+ "<td><input type='text' style='width: 80px;' id='txtPurchaseQuotationFactorFour"
					+ id
					+ "' class='form-control input-SmallText' value='"
					+ response.lstpurchaseitemInfoDto[i].itemFactor4
					+ "'>"
					+ "</td>"
					+ "<td><input type='text' style='width: 80px;' id='txtPurchaseQuotationActualQuantity"
					+ id
					+ "' class='form-control input-SmallText' value='"
					+ response.lstpurchaseitemInfoDto[i].itemOrderQty
					+ "'>"
					+ "</td>"
					+ "<td style='display:none'><input type='text' style='width: 80px;' id='txtPurchaseQuotationPendingQuantity"
					+ id
					+ "' class='form-control input-SmallText' value='"
					+ response.lstpurchaseitemInfoDto[i].itemPendingQty
					+ "'>"
					+ "</td>"

					+ "<td style='display:none;' ><input type='text' style='width: 80px;' id='batchId"
					+ id
					+ "' class='form-control input-SmallText' value='"
					+ response.lstpurchaseitemInfoDto[i].itemBatchNo
					+ "'>"
					+ "</td>"
					...
					+ response.lstpurchaseitemInfoDto[i].itemId + ">" + "</td>"
					+ "</tr>";
		}
		$("#rowCountId").val(id);
		var totaltblsize = $("#rowCountId").val();
		$("#totaltblsize").val(totaltblsize);
		document.getElementById("ItemInfoTablePO").deleteRow(1);
		$("#ItemInfoTablePO").append(htm);
	}
	if (response.partymasterdto.partyMasterContactInfoDto.length != 0
		...
		for ( var i = 0; i < response.partymasterdto.partyMasterContactInfoDto.length; i++) {
			...
			htm = htm
					...
					+ ',\'fromdb\')"><i class="fa fa-edit"></i></button></td>'
					+ ' <td class="col-md-1 center">'
					+ '	<button type="button" class="btn btn-xs btn-danger editBodyPartMaster" isNew="false" id="deletepurchasecontact'
					+ response.partymasterdto.partyMasterContactInfoDto[i].id
					+ '" onclick="deletePurchaseQuotationSlaveInfo('
					+ response.partymasterdto.partyMasterContactInfoDto[i].id
					+ ',\'deletecontact\')"><i class="fa fa-trash-o"></i></button></td>'
					+ '</tr>';
			$("#PartyContactTableInfoList").html(htm);
		}
	...
	if (response.partymasterdto.partyMasterAddressInfoDto.length != 0
		...
		for ( var i = 0; i < response.partymasterdto.partyMasterAddressInfoDto.length; i++) {
			...
			htm = htm
					...
					+ ' <td class="col-md-1 center"><button type="button" class="btn btn-xs btn-success editBodyPartMaster" onclick="editAddressPurchaseQuotationMaster('
					+ response.partymasterdto.partyMasterAddressInfoDto[i].id
					+ ',\'fromdb\')"><i class="fa fa-edit"></i></button></td>'
					+ ' <td class="col-md-1 center">'
					+ '	<button type="button" class="btn btn-xs btn-danger editBodyPartMaster" isNew="false"  id="deletepurchaseaddress'
					+ response.partymasterdto.partyMasterAddressInfoDto[i].id
					+ '" onclick="deletePurchaseQuotationSlaveInfo('
					+ response.partymasterdto.partyMasterAddressInfoDto[i].id
					+ ',\'deleteaddress\')"><i class="fa fa-trash-o"></i></button></td>'
					+ '</tr>';
			$("#PartyAddressTableInfoList").html(htm);
		}
	}
 ...
 * @param pQId
 */
function printPurchaseOrderMaster(purchaseOrderId) {
	window.open("inv_purchase_order_master_print.jsp?purchaseOrderId="
			+ purchaseOrderId);
}
...
function showCharges() {
	...
	getAllChargeMasterOnPurchaseOrder();
}
 ...
 ******************************************************************************/
function getAllChargeMasterOnPurchaseOrder() {
	var unitId = $("#unitId").val();
	var inputs = [];
	inputs.push('unitId=' + unitId);
	var str = inputs.join('&');
...
function getPurchaseOrderItemInfoBody(id) {
	...
	tbody = tbody
			...
			+ id
			+ "'  style='width:60px;' ></td>"
			+ " <td><input type='text' class='form-control input-SmallText' disabled id='txtPurchaseQuotationTrdeDiscountAmt"
			+ id
			+ "' onkeypress='return validateNumbers(event);'   style='width:60px;'></td>"
 ...
 * 
 */
function showpartyMasterDetailsModalOnPurchaseOrder() {
 ...
 * @param response
 */
function setPartyModalAddressInfoToPurchaseOrder(response) {
	var count = 0;
	var htm = "";
 ...
 ******************************************************************************/
function setPartyModalContactInfoToPurchaseOrder(response) {
	...
	jQuery
			.ajax({
				...
				success : function(response) {
					...
					setParyMasterStateToPurchaseOrder(response);
					for ( var i = 0; i < response.partyMasterAddressInfoDto.length; i++) {
...
function getVenderStateOnPurchaseOrder() {
	...
	$("#hiddenVenderStatePO").val(supplierState);
	for ( var i = 1; i <= rows; i++) {
		if (supplierState == userState) {
 ...
 * @author : Rohit Sandbhor
 * @date : 23-03-2020
 * @codeFor : to Set Row Template To Body Of Item Info Table on purchase order
 *          module on plus button click
 ******************************************************************************/
function getPurchaseOrderItemInfoBodyByPlusButton(id) {
	...
	if (mainTableLength == 1) {
		...
		// GST And ISGT on purchase Order
		if (supplierState == userState) {
			tbody = tbody
					...
					+ id
					+ ");autotaxCodeforItemGstPurchaseOrder("+ id + "," + id + ",this.id);'  style='width:80px;'></td>"
					+ " <td><input type='text' class='typeahead form-control input-SmallText' value='0' disabled='disabled' onkeyup='calculateIGSTWisePurchaseOrder(this.id,"
					+ id + ");calculateTotalItemAmountPurchaseOrder(" + id
					+ ");calculateTotalItemigstAmountPurchaseOrder(" + id
					+ ");autotaxCodeforItemigstPurchaseOrder(" + id + "," + id + ",this.id);' id='txtPurchaseQuotationTaxAmount_" + id
					+ "'   style='width:80px;'  autocomplete='off' ></td> "
		}
		...
		// GST And ISGT on purchase Order
			...
			tbody = tbody
					+ "<td><input type='text' class='typeahead form-control input-SmallText'  autocomplete='off'  disabled='disabled' id='txtPurchaseQuotationTaxCodePO_"
					+ id
					+ "' onkeyup='calculateGSTWisePurchaseOrder(this.id,"
					+ id
					+ ");calculateTotalItemAmountPurchaseOrder("
		...
		tbody = tbody
				...
				// added HSN ID here
				+ "<td style='display:none'><input type='text' class='form-control input-SmallText' id='hsnCodePurchaseOrder"
				+ id
				+ "'  style='width:90px;'></td>"
...
function closePOPopUp() {
	window.location.replace("inv_purchase_order_master.jsp");
	refreshPO();

	$("#purchaseOrderModuleModal").modal('hide');
}
...
function calculateTotalItemgstAmountAfterRemoveRowPurchaseOrder(id) {
	...
	//set otal tax amount added by rohit 06-07-2020
 ...
 * @param r
 */
function setParyMasterStateToEditPurchaseOrder(r) {
	var divContent = "";
	divContent = divContent
			+ "<select name='room Name' class='col-md-12'><option value='0'>---Select---</option>";
 ...
 * @author : Dayanand Khandekar
 * @date : 30-March-2020
 * @codeFor : search getPurchaseOrderDetailsById
 ******************************************************************************/
function getPurchaseOrderDetailsById() {
	...
	jQuery.ajax({
		...
		success : function(r) {
			...
			if (r.supplierName == null) {
				...
				setPurchaseOrderDataToTableById(r);
			}
 ...
 * @param r
 */
function setPurchaseOrderDataToTableById(r) {
	var htm = "";
	var index = 1;
	...
	htm = htm
			...
			+ '</td>'
			+ ' <td class="col-md-1 center">'
			+ getDateTimePurchaseOrder(r.createdDateTime)
			+ '</td>'
			+ ' <td class="col-md-1 center">'
			+ r.id
			+ '</td>'
			...
			if(r.isPoUsed == "N"){
				...
				htm = htm	+ ' <td class="col-md-1 center">'
				+ '	<button class="btn btn-xs btn-success editBodyPartMaster" disabled="disabled" data-toggle="modal" data-target="#purchaseOrderModuleModal"  onclick=editPurchaseOrder('
				+ r.id
				+ ')><i class="fa fa-edit"></i></button></td>';
			}
 ...
 * @returns
 */
function getDateTimePurchaseOrder(date){
	var datee;
...
function uploadPurchaseOrderDocuments(poMasterId) {
	...
	poDocumentUpload.lstPurchaseOrderDocUploadDto.push({
		id : poDocSlaveId,
	...
	data.append("poMasterId", poId);

	jQuery.ajax({
		async : true,
		type : "POST",
		enctype : 'multipart/form-data',
		url : "ehat/inventoryPurchaseOrder/uploadPurchaseOrderDocument",
		data : data,
		processData : false,
		contentType : false,
		catche : false,
...
function getUploadedDocuments(poId){
	...
	jQuery.ajax({
		async : true,
		type : "POST",
		data : {"poMasterId" : poId },
		url : "ehat/inventoryPurchaseOrder/getUploadedDocuments",
		success : function(response) {
			if(response !=null && response !="" && response.lstPurchaseOrderDocUploadDto !=null){
				var fileName = "";
				//fileName.replace(/^\[(.+)\]$/,'$1')
				
				for ( var i = 0; i < response.lstPurchaseOrderDocUploadDto.length; i++) {
					count++;
					fileName = response.lstPurchaseOrderDocUploadDto[i].imagePath;
					htm = htm
					+ '<tr class="newAdded"> '
					+ ' <td class="col-md-1 center">'
					+ count
					...
					+ '" >'
					+ response.lstPurchaseOrderDocUploadDto[i].imagePath
					+ '</td>'
					+ ' <td class="col-md-1 center" id="commentDocumentUploadId' + count
					+ '" >'
					+ response.lstPurchaseOrderDocUploadDto[i].note
					+ '</td>'
					+ ' <td class="col-md-1 center" id="uploadedDateDocumentUploadId' + count
					+ '" >'
					+ getDateTimePurchaseOrder(response.lstPurchaseOrderDocUploadDto[i].createdDate)
					+ '</td>'
					//view button
					+ ' <td class="col-md-1 center"><button id="viewDocumentUploadId'+count+'" value="'+JSON.parse(response.lstPurchaseOrderDocUploadDto[i].imagePath)+'"  type="button" onclick="viewUploadedDocument(this.value)" ><i class="fa fa-eye" title="View Document"></i></button>'
					+ '</td>'
					
					+ '</tr>';
				}
				$('#uploadedDocumentPoBody').html(htm);
			}
		}
...
function viewUploadedDocument(document){
	if(document ==null || document =="" || document ==undefined){
		...
		$('#viewDocumentPo').attr("src","");
		$('#viewDocumentPo').attr("src","ehat/inventoryPurchaseOrder/readDocuments?poMasterId="+poId+"&fileName="+document);
...
function getPurchaseOrderSeries() {
	var inputs = [];
		...
		.ajax({
			...
			success: function (response) {
				for (var i = 0; i < response.lstdocMasterDocNumFinancialYearDto.length; i++) {
					...
					if (response.lstdocMasterDocNumFinancialYearDto[i].docNumberingId == 2) {
