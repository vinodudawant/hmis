	...
	if(type=="fromTable"){
		for ( var i = 0; i < r.length; i++) {
			htm = htm + '<tr> ' + ' <td>' + index + '</td>'
					...
					+ ' <td>' + r[i].marathiInstruction + '</td>'
					+ ' <td>' + r[i].referTo + '</td>'					
					+ '<td><button class="btn btn-success btn-xs" onclick=onEditInst('+r[i].id+')><i class="fa fa-edit"></i></button></td>'
					+ '<td><button class="btn btn-danger btn-xs" onclick=onDeleteInst('+r[i].id+')><i class="fa fa-trash-o"></i></button></td>'
