...
function setHistoryTemplate(id){
	...
	var temp = '<div class="tab-pane active" id="History">'
			...
			+ '<button onclick="saveIVFHistory()" title="Save History " data-placement="left" data-toggle="tooltip" id="saveAddIpdHistory" class="btn btn-xs btn-success editUserAccess" style="margin-left: 2px;">'
...
var rowCount = $('#historyTable1 tbody tr').length;
	...
	htm = htm
			...
			+ "<td style='width: 50.96%; height: 21.5px;'  ><div class='col-md-12-1' style='margin-top:5%' ><div class='col-md-6-1' style='margin-top:-3%'><input type='range'  min='0' max='100' class='defaultSlider' id='defaultSlider_"+rowCount+" ' style='font-size: 11px; width:120%;' name='duration' " 
				...
				+ "<input type ='text' class='col-sm-2-1' style='margin-left:125%; margin-top:-7.3%; font-weight:bold;' name='qty"+parseInt(rowCount)+ " '   id='qty"+parseInt(rowCount) +"' onkeyup='changeSliderInHistory("+rowCount+")' /> "
   ...
   if(docId.length>0){
	 ...
	 var inputs = [];
		...
		jQuery.ajax({
			async : false,
			type : "GET",
			url : "ehat/ivfhistory/deleteHistorySalve",
 ...
 * @author   :HM00054
 * @Date     :23-12-2021
 * @Code     :this method used for saveIVFHistory
 * *****/
...
inputs.push('patientId=' + patientId);
	...
	jQuery.ajax({
		...
		type : "POST",
		data : str + "&reqType=AJAX",
		url : "ehat/ivfhistory/saveIVFHistory",
		timeout : 1000 * 60 * 5,
		catche : false,
		error : function() {
			alert("error");
 ...
 * @author   :HM00054
 * @Date     :23-12-2021
 * @Code     :this method used for getIVFHistory
 * *****/
function getIVFHistory(){
...
//var treatmentId=$("#tr_Id").val();
	...
	if(ivftreatmentId == null || ivftreatmentId == undefined || ivftreatmentId == "")
		{
	...
	var inputs = [];
	inputs.push('ivftreatmentId=' + encodeURIComponent(ivftreatmentId));
	...
	jQuery.ajax({
		...
		type : "POST",
		data : str + "&reqType=AJAX",
		url : "ehat/ivfhistory/getIVFHistory",
		timeout : 1000 * 60 * 5,
		cache : false,
		error : function() {
			alert('error');
...
//	var billId=$("#billNo").text();    
    ...
    window.open("ivf_history_print.jsp?billId="+billId+"&treatId="+treatId+"&patId="+patId+"&recId="+recId+"&pendFlag="+pendFlag+"&ivftreatmentId="+ivftreatmentId);
