...
function displayHideDiv(type) {
	...
	if (type == 'home') {
		...
		$("#tabledivDigo").show();
		$("#diagosForm").hide();
		fetchComplaint('onload');// done by Akshata
	...
	if (callfrom == 'hometab') {
		...
		for ( var i = 0; i < r.length; i++) {
			htm = htm
					...
					+ '<td ><button class="btn btn-danger btn-xs" onclick=onDeletePrescription('
...
function setmmToCoverSheet(r, callfrom) {
	if (callfrom == 'hometab') {
		...
		for ( var i = 0; i < r.length; i++) {
			...
			htm = htm + '<tr> ' + ' <td>' + index + '</td>' + ' <td>'
					+ r[i].patient_weight + '</td>' + ' <td>' + r[i].patient_height + '</td>'
					+ ' <td>' + r[i].patient_bmi + '</td>' + ' <td>' + r[i].patient_bsa
					+ '</td>' + ' <td>' + r[i].date + '</td>' + '</tr>';
			index++;
		}
		if (r.length == 0) {
			htm = htm
					+ "<tr class='text-center'><td colspan='6' class='center text-danger'>Sorry No Records To Display</td></tr>";
		}
		...
		for ( var i = 0; i < r.length; i++) {
			htm = htm
					...
					+ r[i].patient_bmi_id + ')><i class="fa fa-trash-o"></i></button></td>'
					+ '</tr>';
			index++;
		}
...
function onEditMeasurements(id) {
	...
	jQuery.ajax({
		...
		success : function(r) {
			...
			for ( var i = 0; i < r.length; i++) {
				$("#measurementsMasterId").val(r[i].patient_bmi_id);
				$("#date").val(r[i].date);
				$("#wg").val(r[i].patient_weight);
				$("#height").val(r[i].patient_height);
				$("#hcim").val(r[i].patient_headcim);
				$("#bmi").val(r[i].patient_bmi);
				$("#bsa").val(r[i].patient_bsa);
...
function onSaveVitals() {
	...
	if (type == "second") {
		...
		inputs.push('patient_id=' + patientId);
		inputs.push('patient_treat_id=' + treatmentId);
		inputs.push('patient_bmi_id=' + $("#measurementsMasterId").val());
		inputs.push('date=' + date);
		inputs.push('patient_weight=' + wg);
		inputs.push('patient_height=' + height);
		inputs.push('patient_headcim=' + hcim);
		inputs.push('patient_bmi=' + bmi);
		inputs.push('patient_bsa=' + bsa);
		inputs.push('finalAgeInMonths=' + "0");
		inputs.push('patient_treat_count=' + 0);
...
function openclosemodal(){
	var newcallfrom = $("#callfromforprvTrtmnt").val();
	var inputs = [];
	if(newcallfrom=='privioustreatmentVise'){
		var prvtreatmentId = $("#priviousTrtmntId").val();
		var currenttid = $("#treatmentId").val();
		inputs.push('priviousTretamentId=' + prvtreatmentId);
		inputs.push('treatmentId=' + currenttid);
		inputs.push('callfrom=' + 'entiretreatment');
		$("#mdfortreatment").modal('hide');
		var str = inputs.join('&');
	...
	if (type == 'foreditdel') {
		...
		for ( var i = 0; i < r.length; i++) {
			htm = htm + '<tr> ' + ' <td>'
					...
					+ ' <td>' + r[i].allergyName + '</td>'

					+ ' <td>' + r[i].allergyTypeName
					+ '</td>'

					+ ' <td>' + r[i].allergyReactName
					+ '</td>'
					...
					+ '<td><input  name="allPopuptablecheckbox"  value="'
		...
		for ( var i = 0; i < r.length; i++) {
			htm = htm + '<tr> ' + ' <td  class=" col-md-1">'
					+ index + '</td>'

					+ ' <td class=" col-md-3">' + r[i].allergyName
					+ '</td>'

					+ ' <td class=" col-md-3">' + r[i].fos
					+ '</td>'
...
function onSaveSxAdvice() {
	...
	var pattern = /^(\d{1,2})\/(\d{1,2})\/(\d{4})$/;
	var arrayDate = ddprocdate.match(pattern);
	var d3 = new Date(arrayDate[3], arrayDate[2] - 1, arrayDate[1]);
	var d2=d3.getTime();
	...
	if (callfrom == 'hometab') {
		...
		for ( var i = 0; i < r.length; i++) {
			htm = htm
					...
					+ r[i].proceduerDate
					+ '</td>'
					+ '<td><button class="btn btn-success btn-xs" onclick=onEditSxForm('
					+ r[i].id
					+ ')><i class="fa fa-edit"></i></button></td>'
					+ '<td><button class="btn btn-danger btn-xs" onclick=ondeleteSxForm('
					+ r[i].id + ')><i class="fa fa-trash-o"></i></button></td>'
...
function setDatatoGeneralAdvice(r) {
	...
	for ( var i = 0; i < r.length; i++) {
		htm = htm + '<tr> ' + ' <td>' + index + '</td>'
				+ ' <td>' + r[i].instruction_name + '</td>'
				+ ' <td>' + $("#loggedinUserName").val()
				+ '</td>' + ' <td>'
...
function ongetDropdownsData(){
	getAllOrganMaster();
	getListOfSpecialzationById();
	onresetTemplatemaster();
}

/*Template Making start*/

function savecepTemplate(){
	var docspecialityid= $("#docspeciality").val();
	var cporgansid = $("#cporgans").val();
	var cptempname =$("#cptempname").val();
	var cptype=$("input[name='ceptype']:checked").val();
	 var favflag = $('#favflag').is(":checked");
	 var cptemplateId =$("#cptemplateId").val();
	 var docspecialityName = $("#docspeciality").find(':selected').attr('data-name');
	 var cporgansName = $("#cporgans").find(':selected').attr('data-name');
	 var treatmentId="";
	 var callfrom = $("#callfromforprvTrtmnt").val();
	 if (callfrom == 'privioustreatmentVise') {
			treatmentId = $("#priviousTrtmntId").val();
	 ...
	 if(docspecialityid==0){
		 alertify.error("please select specailty");
		 return false;
	 }
	 else if(cptempname=="" || cptempname==undefined || cptempname == null){
	 ...
	 var data ={
			 ...
			 "id":cptemplateId,
			 "templateName":cptempname,
			 "specailzationId":docspecialityid,
			 "organId":cporgansid,
			 "favFlag":favflag,
			 "type":cptype,
			 "spcName":docspecialityName,
			 "orgName":cporgansName,
			"treatmentId":treatmentId,
	 ...
	 jQuery.ajax({
			async : false,
			type : "POST",
			url : "ehat/coversheet/savetemplate",
			data : JSON.stringify(data),
			dataType : 'text',
			contentType : 'application/json; charset=utf-8',
			error : function() {
...
function onresetTemplatemaster(){
	getTemplateList();
	 $("#cporgans").val(0);
	 $("#cporgans").select2();
	$("#cptempname").val("");
	$('#favflag').prop('checked',false);
...
function getListOfSpecialzation(){
	var inputs = [];
	...
	jQuery.ajax({
		async	: false,
		type: "POST",
		url : "ehat/coversheet/hospcialization",
		data: str + "&reqType=AJAX",
		catche : false,
		error : function() {
			alert('error');
	...
	for ( var i = 0; i < r.length; i++) {
		alistTemp = alistTemp + "<option value=" + r[i].specialisationId
				+ " data-name='"+ r[i].specializationName+"'>"
				+ r[i].specializationName + "</option>";
	}
	$("#front_page_specialty").html(alistTemp);
	$("#docspeciality").html(alistTemp);
	$("#docspeciality").select2();
	$("#docspecialityaccessTemplate").html(alistTemp);
	$("#docspecialityaccessTemplate").select2();
		}
	});
}

function getListOfSpecialzationById(){
	var inputs = [];
	inputs.push('callfrom='+"byid");
	var str = inputs.join('&');
	jQuery.ajax({
		async	: false,
		type: "POST",
		...
		success : function(r) {
			...
			$("#front_page_specialty").val(r[0].specialisationId);
			$("#docspeciality").val(r[0].specialisationId);
			$("#docspeciality").select2();
			
			//setdataspecalitytoaccessTemp(r);
			$("#docspecialityaccessTemplate").val(r[0].specialisationId);
			$("#docspecialityaccessTemplate").select2();
		}
	});
}
...
function getTemplateList(){
	jQuery.ajax({
		async	: false,
		type: "POST",
		url : "ehat/coversheet/getcpTemps",
		data:"&reqType=AJAX",
		catche : false,
		error : function() {
			alert('error');
...
function setDatatoTmpModal(r){
	...
	for ( var i = 0; i < r.length; i++) {
		htm = htm
				...
				+ '<td><button class="btn btn-success btn-xs" onclick=onEditModalTmp('
				+ r[i].id
				+ ')><i class="fa fa-edit"></i></button></td>'
				+ '<td><button class="btn btn-danger btn-xs" onclick=onDeleteModalTmp('
				+ r[i].id + ')><i class="fa fa-trash-o"></i></button></td>'
	...
	$("#popup_template_body").html(htm);
}

function onEditModalTmp(id){
	var inputs = [];
	inputs.push('id=' + id);
	var str = inputs.join('&');
	jQuery.ajax({
		async : false,
		type : "POST",
		url : "ehat/coversheet/getcpTempsById",
		data : str + "&reqType=AJAX",
		catche : false,
		error : function() {
			alert('error');
		...
		success : function(r) {
			for ( var i = 0; i < r.length; i++) {
				$("#cptemplateId").val(r[i].id);
				 $("#cporgans").val(r[i].organId);
				 $("#cporgans").select2();
				 $("#docspeciality").val(r[i].specailzationId);
					$("#docspeciality").select2();
				$("#cptempname").val(r[i].templateName);
...
function onDeleteModalTmp(id){
	...
	jQuery.ajax({
		async : false,
		type : "POST",
		url : "ehat/coversheet/deleteTemplate",
		data : str + "&reqType=AJAX",
		catche : false,
		error : function() {
			alert('error');
