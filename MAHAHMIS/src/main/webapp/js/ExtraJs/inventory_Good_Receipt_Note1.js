			...
			jQuery
					.ajax({
						...
						success : function(r) {
							...
							totalAmount();
						/*	$("#txtPurchaseQuotationTaxAmount_"+idValue).val(0);*/
							}
...
function viewGRNMasterDetails(partyId) {
	...
	for ( var rowCount = 0; rowCount < objPurchase.ltinvetorypurchasecommonmaster.length; rowCount++) {
		...
		if (objPurchase.ltinvetorypurchasecommonmaster[rowCount].inv_batch_stock_master_doc_no == partyId) {
			...
			$("#txtempAmt").attr('readonly', false);
			$("#txtexGstper").attr('readonly', false);
			$("#txtexGstamt").attr('readonly', false);
			...
			.ajax({
				...
				success : function(r) {
					...
					for ( var Count = 0; Count < pobj1.ltinvetorypurchaseorderitemmaster.length; Count++) {
				    	...
				    	$("#txtPurchaseQuotationBaseAmount"+srNumber).hide();
		             	var hostate = $("#hosState").val();
		             	if(batchstateid > 0){
...
function pendingAmount(id, rowCount) {
	...
	/* this used for calculation the base AMT 19sep2016 */
	 ...
	 totalAmountforParialGRN(1,rowCount);
	 totalVatAmtnEW(id, rowCount);
	 rowAmtCalNEW(id, rowCount);
	// totalVatAmt(id, rowCount);
	// rowAmtCal(id, rowCount);
	 calcuatSumforTradeDisAmtforPartialGRN();
	 calcuatSumforitemQTYforPartialGRN();
...
function toCreateDivGRN() {
	...
	//	auto("txtPurchaseQuotationItemName_" + rowCount, "onload");
		$("#txtPurchaseQuotationUnitPrice"+rowCount).val(0).hide();
		$("#txtPurchaseQuotationTrdeDiscountPercentage"+rowCount).val(0).hide();
		$("#txtPurchaseQuotationTrdeDiscountInRupess"+rowCount).val(0).hide();
		$("#txtPurchaseQuotationTrdeDiscountAmt"+rowCount).val(0).hide();
		$("#txtPurchaseQuotationTaxAmount_"+rowCount).val(0).hide();
		$("#txtPurchaseQuotationTaxCode_"+rowCount).val(0).hide();
		$("#txtGRNTaxAmtinRs"+rowCount).val(0).hide();		
		$("#txtPurchaseQuotationRowAmount"+rowCount).val(0).hide();		
		$("#txtPurchaseQuotationFactor2"+rowCount).val(0).hide();		
		$("#txtPurchaseQuotationFactor3"+rowCount).val(0).hide();		
		$("#txtPurchaseQuotationFactor4"+rowCount).val(0).hide();		
    	$("#txtPurchaseQuotationBaseAmount"+rowCount).val(0).hide();
   ...
   //end	
	...
	else
	{
		var finaltxtPurchaseGrnDocSeries =txtPurchaseQuotationDocSeries +"No"+":"+txtPurchaseQuotationDocNo;
			...
			txtGRNDeliverychallanNumber1 =txtGRNDeliverychallanNumber + txtGRNDeliverychallanNumber1;
			txtGRNPurchaseInvoiceNumber1 = txtGRNPurchaseInvoiceNumber + txtGRNPurchaseInvoiceNumber1;
	}
...
/* 	alert("Please enter reference number");
	...
	inputs.push('txtGRNPurchaseInvoiceNumber1='	+ txtGRNPurchaseInvoiceNumber1);
	inputs.push('txtGRNDeliverychallanNumber='	+ txtGRNDeliverychallanNumber);
	inputs.push('txtGRNDeliverychallanNumber1='	+ txtGRNDeliverychallanNumber1);
	 ...
	 for( var rowCount = 0; rowCount < objPurchase.ltinvetorypurchasecommonmaster.length; rowCount++){
		 if(objPurchase.ltinvetorypurchasecommonmaster[rowCount].inv_batch_stock_master_form_Name == "GRN" ||objPurchase.ltinvetorypurchasecommonmaster[rowCount].inv_batch_stock_master_form_Name == "Goods Reciept"){
			 htm = htm
	           ...
	           + "<td><button id='btnEdit2' class='btn btn-xs btn-success' type='button'  data-toggle='modal' data-target='#Sales_Quotation_Form' onclick='viewGRNMasterDetailschalaln("+ objPurchase.ltinvetorypurchasecommonmaster[rowCount].inv_batch_stock_master_doc_no +")' value='EDIT'><i class='fa fa-eye View'></i></button></td>"
				...
				for ( var rowCount = 0; rowCount < objPurchase.ltinvetorypurchasecommonmaster.length; rowCount++) {
					...
					if (objPurchase.ltinvetorypurchasecommonmaster[rowCount].inv_batch_stock_master_doc_no == partyId) {
						...
						/*var txtPurchaseQuotationSupplierCode = $('#txtVendorCodePO').val(myObj.inv_batch_stock_master_Supplier_Id);
				 		...
				 		window.open("Inventory_Grn_printNEW.jsp?txtVendorCode="+txtVendorCode+"&partyId="+partyId+"&txtGRNDocSeries="+txtGRNDocSeries+"&txtGRNDeliveryDate="+txtGRNDeliveryDate+"&txtGrnDate="+txtGrnDate+"&txtOrderNO="+txtOrderNO +"&taxapp="+taxapp +"&bill="+"PURCHASE");
			}});
					...
					for ( var Count = 0; Count < pobj1.ltinvetorypurchaseorderitemmaster.length; Count++) {
						...
						.append(
								"<tr id='deleterow"
										...
										+ "'  onblur='totalAmount(this.id,"
										+ srNumber
										+ ")' onkeypress='return validateNumbers(event);'> <input type='hidden' id='txtPurchaseQuotationChangingItemQty"
										+ srNumber
						...
						if(state ==hostate || state == hostate || state == hostate)
						{
							...
							//$("#txtPurchaseQuotationTaxCode_"+idValue).val(sumofRate);
							$("#txtPurchaseQuotationTaxAmount_"+srNumber).val("0.0").hide();
							
						}else{
							
							$("#txtPurchaseQuotationTaxCode_"+srNumber).val("0.0").hide();
...
if(quantity=="" || quantity==null){
	quantity = $('#txtPurchaseQuotationChangingItemQty' + rowCount).val();
}
		...
		if(txtVendorCodePO !="" || txtVendorCodePO !=null || txtVendorCodePO !=undefined){
			...
			for ( var i = 0; i < objDocument.ltinvetorypurchaseordermaster.length; i++) {
				if(txtPurchaseOrderList != objDocument.ltinvetorypurchaseordermaster[i].inv_purchase_order_master_doc_no){
					if (objDocument.ltinvetorypurchaseordermaster[i].inv_purchase_order_master_Supplier_Id == txtVendorCodePO) {
...
/*									+ " <td style='display: none'><select class='form-control input-SmallText'  multiple='multiple' onchange ='taxcalculation(this.id," + srNumber + ")' id='txtPurchaseQuotationTaxCode_"+srNumber+ "' > <option selected=selected >" + pobj1.ltinvetorypurchaseorderitemmaster[Count].inv_batch_stock_item_tax_code + "</option>  </select></td> <td style='display: none'><input type='text'  readonly='' class='form-control input-SmallText' id='txtPurchaseQuotationTaxAmount_"
									...
									+ "<td style='display: none'><input type='text'  style='width:100px;' class='form-control input-SmallText' id='txtGRNTaxGSTPercent"
									+ srNumber
									+ "' value='"
									+ pobj1.ltinvetorypurchaseorderitemmaster[Count].inv_batch_stock_item_tax_amount
									+ "' ></td>"
									+ "<td style='display: none'><input type='text'  style='width:100px;' class='form-control input-SmallText' id='txtGRNTaxIGSTPercent"
									+ srNumber
									+ "' value='"
									+ pobj1.ltinvetorypurchaseorderitemmaster[Count].inv_batch_stock_item_tax_amount
									+ "'  ></td>"
...
function viewGRNMasterDetailschalaln(partyId){
	

	$("#txtiw").text("Purchase Information");
	$("#saveGRN").hide();
	$("#removebtn").hide();
	/*$('#taxcodeGst').css('display','none');
	$('#taxcodeIGst').css('display','none');*/
...
//	$('#Sales_Quotation_Form').find('input, text').attr("readonly", "readonly");
	...
	for ( var rowCount = 0; rowCount < objPurchase.ltinvetorypurchasecommonmaster.length; rowCount++) {
		...
		if (objPurchase.ltinvetorypurchasecommonmaster[rowCount].inv_batch_stock_master_doc_no == partyId) {
			...
			var txtGRNPurchaseInvoiceNumber = $("#txtGRNPurchaseInvoiceNumber").val(objPurchase.ltinvetorypurchasecommonmaster[rowCount].inv_batch_stock_fixpurchaseinvoice);
			var txtGRNDeliverychallanNumber = $("#txtGRNDeliverychallanNumber").val(objPurchase.ltinvetorypurchasecommonmaster[rowCount].inv_batch_stock_fixchallan);
			 $("#txtGRNPurchaseInvoiceNumber1").val(objPurchase.ltinvetorypurchasecommonmaster[rowCount].inv_batch_stock_master_purchase_invoice_number);
			 $("#txtGRNDeliverychallanNumber1").val(objPurchase.ltinvetorypurchasecommonmaster[rowCount].inv_batch_stock_master_purchase_delivery_challan_number);
		...
		//	if(objPurchase.ltinvetorypurchasecommonmaster[rowCount].txtagchallan=="Y"){
				...
				$("#chkapndchllan").attr('readonly', false);
				$("#saveGRN").show();
				$("#txtGRNDeliverychallanNumber1").attr('readonly', true);
				$("#txtGRNPurchaseInvoiceNumber1").attr('readonly', true);
				$("#txtGRNDeliverychallanNumber").css("display", "none");
				$("#txtGRNPurchaseInvoiceNumber").css("display", "none");
				$("#txtGRNPurchaseInvoiceNumber1").css("margin-left", "-29px");
				$("#txtGRNPurchaseInvoiceNumber1").css("margin-top", "-2px");
				$("#txtGRNDeliverychallanNumber1").css("margin-left", "-16px");
				$("#txtGRNDeliverychallanNumber1").css("margin-top", "1px");
			//}
		//end	
			//add by paras for edit supplier state	 
			if(objPurchase.ltinvetorypurchasecommonmaster[rowCount].inv_SupplierState == 0 || objPurchase.ltinvetorypurchasecommonmaster[rowCount].inv_SupplierState ==null || objPurchase.ltinvetorypurchasecommonmaster[rowCount].inv_SupplierState ==undefined ){
						...
						.append(
								"<tr id='deleterow"
									...
									/*	+ " <td style='display: none'><select class='form-control input-SmallText'  multiple='multiple' onchange ='taxcalculation(this.id," + srNumber + ")' id='txtPurchaseQuotationTaxCode_"+srNumber+ "' > <option selected=selected >" + pobj1.ltinvetorypurchaseorderitemmaster[Count].inv_batch_stock_item_tax_amount + "</option>  </select></td> <td style='display: none'><input type='text'  readonly='' class='form-control input-SmallText' id='txtPurchaseQuotationTaxAmount_"
										...
										"<td><input type='text' readonly='readonly' class='form-control input-SmallText' id='txtPurchaseExpiryDate_"+ srNumber	+"' onclick ='getMfgandexpyDate(this.id,"+srNumber+ ")'; style='float:left;' value='"+pobj1.ltinvetorypurchaseorderitemmaster[Count].inv_batch_stock_item_exp_date+"'></td>" + "</tr>");
							//alert(pobj1.ltinvetorypurchaseorderitemmaster[Count].inv_batch_stock_item_tax_amount_rupess);
						$("#txtGRNTaxGSTPercent"+srNumber).css("dispaly", "none");
						$("#txtGRNTaxIGSTPercent"+srNumber).css("dispaly", "none");
		             	var hostate = $("#hosState").val();
		             	if(batchstateid > 0){
		             		if(batchstateid ==hostate || batchstateid == hostate || batchstateid == hostate)
							{
