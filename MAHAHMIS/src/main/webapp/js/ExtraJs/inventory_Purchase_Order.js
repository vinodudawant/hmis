						...
						"<tr id='deleterow"
								...
								+ "' readonly='' style='width:100px;'></td> "*/
								 +"<td><input type='text' class='typeahead form-control input-SmallText'  autocomplete='off'  id='txtPurchaseQuotationTaxCodePO_"
									+ rowCount
									+ "' onkeypress='autotaxCodeforItem(this.id,onchange)'  style='width:80px;'></td>"
		...
		$("#ItemInfoTablePO > tbody")
				.append(
						...
						/* +"<td><select style='width:160px;' class='form-control input-SmallText' onclick='multipletaxCalculation(this.id," + rowCount + ")'  multiple='multiple' onchange ='taxcalculation(this.id," + rowCount + ")' id='txtPurchaseQuotationTaxCodePO_"
								...
								+ "<td><input type='text' class='form-control input-SmallText' onkeypress='return validateNumbers(event);' id='txtPurchaseQuotationFactor3"
								+ rowCount
								+ "' onkeypress='return validateNumbers(event);'maxlength='5' style='width:60px;' value='0'><label id='txtPurchaseQuotationFactor3UOM"+rowCount+"' ></label></td> "
								...
								+ rowCount
								+ "' onkeypress='return validateNumbers(event);' maxlength='5' style='width:60px;' value='0'><label id='txtPurchaseQuotationFactor4UOM"+rowCount+"' ></label></td> "
								+ " <td><input type='text' class='form-control input-SmallText' id='txtPurchaseQuotationActualQuantity"
								+ rowCount
								+ "' onblur='pendingAmount(this.id,"
								+ rowCount
...
function viewPurchaseMasterDetails(partyId) {
	...
	if(orderObj.inv_SupplierState == 0 || orderObj.inv_SupplierState ==null || orderObj.inv_SupplierState==undefined ){
		 
	}else{
		 $("#hoseditState").val(orderObj.inv_SupplierState);
	}
	//end
					...
					for ( var Count = 0; Count < pobj1.ltinvetorypurchasecommonitemmaster.length; Count++) {
								...
								.append(
										"<tr id='deleterow"
											...
											/*	+"<td><select class='form-control input-SmallText'  multiple='multiple' onchange ='taxcalculation(this.id," + srNumber + ")' id='txtPurchaseQuotationTaxCodePO_"+srNumber+ "' > <option selected=selected >" + pobj1.ltinvetorypurchasecommonitemmaster[Count].inv_purchase_common_item_tax_code +"</option>  </select></td> "
												...
												+"<td><input type='text' class='typeahead form-control input-SmallText'  autocomplete='off'  id='txtPurchaseQuotationTaxCodePO_"
												+ srNumber
												+ "' onkeypress='autotaxCodeforItem(this.id,onchange)'  style='width:80px;' value='"
												+ pobj1.ltinvetorypurchasecommonitemmaster[Count].inv_purchase_common_item_tax_code
												+ "'></td>"
						...
						//alert(state);
						    if(state == null    || state ==0 ){
						    	state = orderObj.inv_SupplierState;
						    }
...
if(myObj.inv_remark==null ||myObj.inv_remark=="null"){
	$("#txtGRNArermark").val("-");

}else{
	$("#txtGRNArermark").val(myObj.inv_remark);
}
//add by paras for edit supplier state	 
...
//print po new by paras suryawanshi
var SrNo = 1; 
...
var inventoryPurchaseOrderTemp = "<table class='table table-striped table-bordered header-fixed cf' style='margin: 10px;width: 97%; '>"
		...
		+ "<th style='height: 21.5px;' class='col-md-1 center'><div>Delete</div></th> <th style='height: 21.5px;' class='col-md-1 center'><div>Print</div></th>"
        + "</tr> </thead>"
		...
		+ "<td><button id='btnDelete' value='Delete' class='btn btn-xs btn-success' type='button' onclick=\"deletePurchaseMasterDetails({$T.ltinvetorypurchaseordermaster.inv_purchase_order_master_doc_no})\"><i class='fa fa-trash-o'></i></button></td>"
		+ "<td><button id='btnEdit2' class='btn btn-xs btn-success'  data-toggle='modal'   onclick=\"printPurchaseMasterDetailsOnEditNew({$T.ltinvetorypurchaseordermaster.inv_purchase_order_master_doc_no})\" value='EDIT'><i class='fa fa-print'></i></button></td>"
		+ "</tr>{#/if}{#/for}</table>";
...
function calculateFactoring(qty,rowCountPO,Type)
{
 ...
 else if((factQty1 != '' || factQty1 != 'undefined' || factQty1 != undefined) && (factQty2 != 'undefined' || factQty2 != '' ||  factQty2 != undefined) 
		 && (factQty3 != 'undefined' || factQty3 != '' || factQty3 != undefined) && (factQty4 != 'undefined' || factQty4 != '' || factQty4 !=undefined))
       {
		  if(qty != 0){
				  if(Type == "PurchaseOrder")
					{
							...
							// $('#txtPurchaseQuotationUnitPricePO' + rowCountPO).val(parseFloat(newUnitPrice));
							 ...
							 $('#txtPurchaseQuotationFactor3PO' + rowCountPO).val(fact3QTY);
							 ...
							 $('#txtPurchaseQuotationFactor2PO' + rowCountPO).val(fact2QTY);
							 ...
							 $('#txtPurchaseQuotationFactor1PO' + rowCountPO).val(fact1QTY);
					  ...
					  //alert(r.length);
						...
						if (r.length == 32) {
							...
							for ( var i = 0; i < ajaxResponse.ltInventoryItemMasterDTOs.length; i++) {
								// alert(ajaxResponse.ltinvetorypurchasecommonmaster[i].inv_purchase_common_master_Supplier_Name+"_"+ajaxResponse.ltinvetorypurchasecommonmaster[i].inv_purchase_common_master_doc_no);
								availableTags.push(ajaxResponse.ltInventoryItemMasterDTOs[i].item_name
												+ "_"
			...
			jQuery
					.ajax({
						...
						success : function(r) {
							...
							{
								$("#txtPurchaseQuotationTaxCodePO_"+idValue).val(finalrat);
						...
						/*	$("#txtPurchaseQuotationTaxAmount_"+idValue).val(0);*/
							...
							for ( var i = 0; i < ajaxResponse.inventoryitempurchaseandItemMasterDTOs.length; i++) {
								// alert(ajaxResponse.ltinvetorypurchasecommonmaster[i].inv_purchase_common_master_Supplier_Name+"_"+ajaxResponse.ltinvetorypurchasecommonmaster[i].inv_purchase_common_master_doc_no);
						//validation by paras
							var f1=ajaxResponse.inventoryitempurchaseandItemMasterDTOs[i].item_purchase_uom_factor1 ;
							var f2=ajaxResponse.inventoryitempurchaseandItemMasterDTOs[i].item_purchase_uom_factor2;
                            var f3=ajaxResponse.inventoryitempurchaseandItemMasterDTOs[i].item_purchase_uom_factor3;
                            var f4=ajaxResponse.inventoryitempurchaseandItemMasterDTOs[i].item_purchase_uom_factor4;
...
/*	if(idValue=="" || idValue== undefined){
	 ...
	 if(rowCount[0]=="txtPurchaseQuotationTaxCodePO_"){
...
//*multiplepurOrderPrint Author Vinod @Date 14/09/2016 */
 ...
 function printPurchaseMasterDetailsOnEditNew(partyid){
	 ...
	 jQuery.ajax({
			...
			var hostate = parseInt(response);
		 	...
		 	var str = "supplierscopy";
	    	window
		 				.open("Inventory_purchase_order_printNew.jsp?txtPurchaseOrderSupplierCode="
		 						+ txtPurchaseOrderSupplierCode
		 						+ "&partyId="
		 						+ purOrderPrintId
		 						+ "&txtPurchaseOrderDocSeries="
		 						...
		 						+ "&quotationNo="
		 						+ txtPurchaseOrderQuatationNo
		 						+ "&purOrderDate=" + purOrderDate +"&taxapp="+taxapp);
...
//*uncheckpurOrderPrint Author Vinod @Date 14/09/2016 */
 ...
 {
	   var txtPurchaseQuotationDocSeriesIsEdit = $("#txtPurchaseOrderDocSeriesIsEdit").val();
	    var partyId =$("#txtPurchaseOrderDocNoPRl").val();
				...
				//alert(state);
				  var hostate = $("#hosState").val();
					 if(state == hostate)
					{
						...
						$("#txtPurchaseQuotationTaxAmount_"+i).hide();
						$("#txtPurchaseQuotationTaxCodePO_"+i).show();
						...
						$("#txtPurchaseQuotationTaxAmount_"+i).show();
						$("#txtPurchaseQuotationTaxCodePO_"+i).hide();
