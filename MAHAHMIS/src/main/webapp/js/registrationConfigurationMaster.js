...
function fetchSuperCatogoiresSlaveReg_old(chargesMasterDto) {
	...
	if (chargesMasterDto == "" || chargesMasterDto == null || chargesMasterDto == undefined || isNaN(chargesMasterDto)) {
		...
		jQuery.ajax({
			...
			//url : "ehat/chargesSlave/fetchSuperCatogoires",
			url : "ehat/chargesSlave/fetchSuperCatogoiresSlaveReg",
			error : function() {
				alert('Network Issue!');
			},
			success : function(response) {
			//	alert(response.lstChargesSlave.length);
				...
				setDyanamicDivForListRegMaster('mulDynamicItem',response);
				
				if(response.lstChargesSlave.length > 0){
					
					//var liSize = $("#" + mulDynamicItem + " li").length;
					//var count = $("#" + mulDynamicItem + " li").size();
...
function getSponsorRecordsForReg(callFrom,sourceTypeId) {
	//alert("hi sponsor...");
...
function applytoallReg(){
	...
	if($("#chksp").is(":checked")){
		...
		var sponser1 =$("#listmstr_select_service").val();
		var sponser ="";
		sponser = $("#lisHcpe" + 0).val();
...
function setDyanamicDivForListRegMaster(setDiv,response) {
	var htm ="";
	for ( var i = 0; i < response.lstChargesSlave.length; i++) {
		var count =i;
		var name = response.lstChargesSlave[i].categoryName;
...
function fetchSuperCatogoiresSlaveReg(setDiv, getDiv,callform) {
	...
	if (liSize == 0) {
		...
		if (liSize == 1) {
			//fetchSubServiceByIdcom(masterid, selfId,callform, setDiv);
			fetchChargesSlaveListByIdRegNew(masterid, selfId,callform,setDiv);
		} else {			
				selfId = $("#lisHcpe" + (liSize - 1)).val();		
			//fetchSubServiceByIdcom(masterid, selfId,callform,setDiv);
				fetchChargesSlaveListByIdRegNew(masterid, selfId,callform,setDiv);
...
function fetchChargesSlaveListByIdRegNew(masterid, selfId,callform,setDiv) {
