	...
	if (r.listChartType.length > 0) {
		...
		for ( var i = 0; i < r.listChartType.length; i++) {
			...
			htm = htm
					...
					+ "<td> <span id='snum"+rowCount+"'>"+rowCount+"</span><input type='hidden' id='chartTempMasterId"+rowCount+"' value="+r.listChartType[i].idchartTypeTbl+"></td>"
					
					
					+"<td><input type='text'  class='form-control input-SmallText TextFont'  value='"
					+ r.listChartType[i].name
					+ "'  id='cname"
					+ (rowCount)
					+ "' ></td> "
					
					+ "<td ><input type='text' class='form-control input-SmallText TextFont'   value='"
					+ r.listChartType[i].fee
					+ "'    id='fee"
					+ (rowCount)
					+ "' ></td> "
					
					
					+ "<td><input type='checkbox' class='chkChartTemp'   value='"+rowCount+"'"
					
					+ " name='charttempdocid'   isNew='false' id="+r.listChartType[i].idchartTypeTbl+"></td>"
...
var rowCount = $('#fixed_header tbody tr').length;
	...
	htm = htm
			+ "<tr class='newChartTemp' id='count"+ rowCount+ "'>"
			
			
			+ "<td> <span id='snum"+rowCount+"'>"+rowCount+"</span><input type='hidden'   id='chartTempMasterId" + rowCount + "' value='"
			+ 0 + "' ></td>"
			...
			+ "<td><input type='text' class='form-control input-SmallText TextFont' id='fee"
			+ rowCount
			+ "' ></td> "
			
			+ "<td><input type='checkbox' class='chkChartTemp' id='checkbox" 
			+ parseInt(rowCount)
...
function deleteChartSlave(tableId,checkboxClass)
{
	...
	$("input[name='charttempdocid']:checked").each(function() {	
		
		var slaveId=$("#chartTempMasterId"+$(this).val()).val();
		
		if(slaveId >0){
	
			docId.push($("#chartTempMasterId"+$(this).val()).val());
		}
	});
   ...
   if(docId.length>0){
	 ...
	 var inputs = [];
		inputs.push('chartslaveid=' + docId);
		...
		jQuery.ajax({
			async : false,
			type : "POST",
			url : "ehat/tempchartcontroller/deletechartslave",
			data : str + "&reqType=AJAX",
			error : function() {
				alert('error');
			},
			success : function(response) {
				...
				alert(response);
				checkForChartTemp(tableId);
				checkChartTempSequenece(tableId);
				//getIvfCalenderInfo();
   ...
   * @codeFor	: For reorder srno after delete
    ************/
   function checkForChartTemp(tableId){
   ...
   * @codeFor	: For reorder index ids of componant after delete
    ************/
   function checkChartTempSequenece(tableId){
...
function saveChartSlaveName() {
	...
	var cType = $("#cType :selected").val();
	if (cType.trim() == null || cType.trim() == "" || cType.trim() == "Select" || cType == 0) {
		alert("Please select Chart Name.");
		return false;
	}
	...
	var rows = $('#fixed_header tbody tr.newChartTemp').length;
	
	for ( var i = 1; i <= rows; i++) {
		var chartTempMasterId = $("#chartTempMasterId" + i).val();
		...
		if(fee=="" || fee==undefined || fee==null || fee=="null"  ){
			alert("please enter fees");		
			$("#fee").focus();					
			return false;
		}	

		setChartTempInfp(lstChartTemp, chartTempMasterId,
				cname, fee, cType);
	}
	lstChartTemp = JSON.stringify(lstChartTemp);
	var inputs = [];
	inputs.push("objSKC="	+ encodeURIComponent(lstChartTemp));
	...
	jQuery.ajax({
		...
		type : "POST",
		data : str + "&reqType=AJAX",
		url : "ehat/tempchartcontroller/savechartslavename",
		timeout : 1000 * 60 * 5,
		catche : false,
		error : function() {
			alert("error");
...
function setChartTempInfp(lstChartTemp, chartTempMasterId,cname, fee, cType){
	lstChartTemp.listChartType.push({
		idchartTypeTbl : chartTempMasterId,
		name : cname,
		fee : fee,
		cType : cType,
...
function setWardType(r)
{
	...
	for ( var i = 0; i < r.lstChargesSlave.length; i++) 
	{
		...
		divContent = divContent + "<option value="+r.lstChargesSlave[i].categoryName+" data-value="+r.lstChargesSlave[i].categoryName+" data-id="+r.lstChargesSlave[i].slaveId+">"
...
function wardTypeName(id) {
	var value = $("#" + id).find(':selected').attr('data-value');
...
function wardName(id)
{
	...
	$("#wardHallSelect").select2("val", selfid);
...
{	
	jQuery.ajax({
		async : false,
		type : "POST",
		url : "ehat/wardtypecontroller/fetchhallname",
		timeout : 1000 * 60 * 5,
		cache : false,
		error : function() {
			alert('error');
...
function saveHallNameInfo()
{
	...
	//var  hName = $("#wardHallSelect").find(':selected').attr('data-name');
	var hName = $("#wardHallSelect").val();
	...
	jQuery.ajax({
		...
		type : "POST",
		data : str + "&reqType=AJAX",
		url : "ehat/wardtypecontroller/savehallinformation",
		timeout : 1000 * 60 * 5,
		cache : false,
		error : function() {
			alert('error');
...
function fetchHallInfo()
{
	jQuery.ajax({
		async : true,
		type : "POST",
		url : "ehat/wardtypecontroller/fetchhallinfo",
		timeout : 1000 * 60 * 5,
		cache : false,
		error : function() {
			alert('error');
...
function setHallInfo(r)
{
	...
	var index=1;
		...
		+ "<td class='col-md-5-1 center'>"+r.hl[i].hn+"</td>"
		+ " <td class='col-md-2-1 center'>"+r.hl[i].bn+"</td>"
		+ " <td class='col-md-2-1 center'>"
		+ "<button onclick='editHallType("+r.hl[i].hi+");'type='button' class='btn btn-xs btn-success'><i class='fa fa-edit'></i></button>"	
		+"</td>"
		+ " <td class='col-md-2-1 center'>"
		+ "<button onclick='deleteHallBeds("+r.hl[i].hi+");'type='button' class='btn btn-xs btn-success'><i class='fa fa-trash-o'></i></button>"	
		+"</td>"
		+ " <td class='col-md-2-1 center'>"
		+ "<button onclick='addHallType("+r.hl[i].hi+");'type='button' class='btn btn-xs btn-success'><i class='fa fa-plus'></i></button>"	
		+"</td>"
		+ " <td class='col-md-2-1 center'>"
		+ "<button onclick='deleteHallBed("+r.hl[i].hi+");'type='button' class='btn btn-xs btn-success'><i class='fa fa-trash-o'></i></button>"	
		+"</td>"
		+ '</tr>';
		}
...
function addWard() {
	...
	+"</div>"
	+"<div class='col-md-3 form-group'>"
	+"<label for=''>Ward Name:</label><input class='form-control' type='text' id='wardName' placeholder='ward Type Name'>"
	+"</div>"	
	+"<div class='col-md-3 form-group'>"
	...
	+"</div>"	
	+"<div class='col-md-3 form-group'>"
	+"<label for=''>Lease/ Bed Normal Rs:</label><input class='form-control' type='text' id='leasebednormal' name='' placeholder='Lease/ Bed Normal Rs'>"
	+"</div>"	
	+"</div>"
	+"<div class='row'>"
	+"<div class='col-md-3 form-group'>"
	+"<label for=''>Lease/ Bed Isolation Rs:</label><input class='form-control' type='text' id='leasebedisolation' name=' placeholder='Lease/ Bed Isolation Rs'>"
	+"</div>"	
	+"<div class='col-md-3 form-group'>"
	+"<label for=''>Number Of Beds:</label><input class='form-control' type='text' id='noofbeds' name='' placeholder='Number Of Beds'>"
	+"</div>"	
	+"<div class='col-md-3 form-group'>"
...
function editHallType(id) {
	...
	jQuery.ajax({
		...
		type : "POST",
		data : str + "&reqType=AJAX",
		url : "ehat/wardtypecontroller/edithalltype",
		timeout : 1000 * 60 * 5,
		cache : false,
		error : function() {
			alert('error');
		...
		success : function(r) {
			...
			// alert(r.chargesmasterslave.slaveId);
			$("#wardTypeSelect").select2('val',r.chargesmasterslave.slaveId);
			...
			$("#wardHallSelect").select2('val',r.hall_type);
			
			$('#noofbeds').attr("disabled", true); 
...
function addHallType(id)
{
	...
	jQuery.ajax({
		...
		type : "POST",
		data : str + "&reqType=AJAX",
		url : "ehat/wardtypecontroller/addHallType",
		timeout : 1000 * 60 * 5,
		cache : false,
		error : function() {
			alert('error');
...
function addBedHallInfo()
{
	...
	inputs.push('numberOfBed=' + no_of_bed);//how many bed you want add
	inputs.push('numberofbed=' + numberofbed);//total bed allready present in bed table
	...
	jQuery.ajax({
		...
		type : "POST",
		data : str + "&reqType=AJAX",
		//url : "ehat/wardtypecontroller/addbedhalltype",
		url : "ehat/wardtypecontroller/addbedinhall",
		timeout : 1000 * 60 * 5,
		cache : false,
		error : function() {
			alert('error');
...
function deleteHallBeds(id)
{
	...
	jQuery.ajax({
		...
		type : "POST",
		data : str + "&reqType=AJAX",
		url : "ehat/wardtypecontroller/deletehallbeds",
		timeout : 1000 * 60 * 5,
		cache : false,
		error : function() {
			alert('error');
...
function deleteHallBed(id)
{
	...
	jQuery.ajax({
		...
		type : "POST",
		data : str + "&reqType=AJAX",
		url : "ehat/wardtypecontroller/deletehallbedipd",
		timeout : 1000 * 60 * 5,
		cache : false,
		error : function() {
			alert('error');
...
function deletebedHalladmin(hall_ID,bed_ID)
{
	...
	jQuery.ajax({
		...
		type : "POST",
		data : str + "&reqType=AJAX",
		url : "ehat/wardtypecontroller/deletebedhalladmin",
		timeout : 1000 * 60 * 5,
		cache : false,
		error : function() {
			alert('error');
