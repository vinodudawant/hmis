...
function getDietTemplate(id){
	...
	var htm='';
	htm =htm
	+'<div style="margin-top: 0px; margin-bottom: 0px; " class="col-md-5-1">'
	+'<div style="margin-top: 9px;" class="col-md-2-1">'
	+'<b>From:</b>'
	+'</div>'
	+'<div style="margin-top: 9px;" class="col-md-5-1">'//change size by Annapurna
	+'	<input id="fromDate"'
	...
	+'</div>'
	+'</div> '
   +'<div style="margin-top: 0px; margin-bottom: 0px; " class="col-md-3-1">'
	+'<div style="margin-top: 9px;" class="col-md-3-1">'
	+'<b>To:</b>'
	+'</div>'
	+'<div style="margin-top: 9px;" class="col-md-9-1">'
	+'	<input id="toDate"'
	+'	class="form-control input-SmallText" type="text" value=""'
	...
	+' onclick="saveOPDDiet();">'
	+'<i class="fa fa-save"></i></i>Save</label>'
...
function setDietListForOPDDoctorStation(res){
	...
	for ( var i = 0; i < res.getListOfOPDDietDTO.length; i++) {
		...
		result = result + '<tr> <td callspan="5" > record not found';	
	}
...
function getPrefixIdByValue(){


	
	var inputs = [];

	inputs.push('deptName=' + "DEPT");//diet->diet opd
	inputs.push('value=' + "DIET");
	...
	jQuery.ajax({
		async : false,
		type : "GET",
		url : "ehat/opdhistory/getPrefixIdByValue",
		data : str + "&reqType=AJAX",
		timeout : 1000 * 60 * 5,
		catche : false,
		error : function() {
...
function getTemplateListByDepartmentId(){
	...
	var id=$("#dietValueId").val();
	var inputs = [];

	inputs.push('departmentId=' + id);//diet->diet opd
...
function setOPDBMIListForOPDDoctorStation(res) {
	...
	if (res.getListOfOPDBmiDTO.length > 0) {
		...
		for (var i = 0; i < res.getListOfOPDBmiDTO.length; i++) {
			...
			+ "<button class='btn btn-xs btn-success editUserAccess' onclick=editBMIBSAOPD("+res.getListOfOPDBmiDTO[i].opdBmiMasterId+") > <i class='fa fa-edit'></i> </button>"
           + '</td> '
