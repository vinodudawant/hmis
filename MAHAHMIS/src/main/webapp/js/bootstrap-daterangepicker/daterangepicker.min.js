!function(e) {
	var t = function(t, n, r) {
		...
		var o = '<div class="daterangepicker dropdown-menu">'
			...
			if (typeof n.ranges == "object") {
				for ( var u in n.ranges) {
					...
					if (this.minDate && f.isBefore(this.minDate)
							|| this.maxDate && a.isAfter(this.maxDate)) {
						continue
					}
					this.ranges[u] = [ a, f ]
				}
		...
		if (!i || typeof n.startDate == "undefined"
				&& typeof n.endDate == "undefined") {
	...
	t.prototype = {
		...
		move : function() {
			var t = {
				top : this.parentEl.offset().top
						- (this.parentEl.is("body") ? 0 : this.parentEl
								.scrollTop()),
			...
			if (this.opens == "left") {
				this.container.css({
					top : this.element.offset().top
							+ this.element.outerHeight() - t.top,
					right : e(window).width() - this.element.offset().left
							- this.element.outerWidth() - t.left,
					left : "auto"
				});
				...
				if (this.container.offset().left + this.container.outerWidth() > e(
						window).width()) {
					this.container.css({
						left : "auto",
						right : 0
		...
		enterRange : function(e) {
			...
			if (t == this.locale.customRangeLabel) {
				...
				var n = this.ranges[t];
				this.container.find("input[name=daterangepicker_start]").val(
						n[0].format(this.format));
				this.container.find("input[name=daterangepicker_end]").val(
						n[1].format(this.format))
			}
		},
		...
		enterDate : function(t) {
			...
			if (s.hasClass("left")) {
				this.container.find("input[name=daterangepicker_start]").val(
						this.leftCalendar.calendar[r][i].format(this.format))
			} else {
				this.container.find("input[name=daterangepicker_end]").val(
						this.rightCalendar.calendar[r][i].format(this.format))
			}
		},
		...
		updateCalendars : function() {
			...
			for ( var n in this.ranges) {
				if (this.timePicker) {
					if (this.startDate.isSame(this.ranges[n][0])
							&& this.endDate.isSame(this.ranges[n][1])) {
						e = false;
						this.container.find(".ranges li:eq(" + t + ")")
								.addClass("active")
					}
				} else {
					if (this.startDate.format("YYYY-MM-DD") == this.ranges[n][0]
							.format("YYYY-MM-DD")
							&& this.endDate.format("YYYY-MM-DD") == this.ranges[n][1]
									.format("YYYY-MM-DD")) {
						e = false;
						this.container.find(".ranges li:eq(" + t + ")")
								.addClass("active")
		...
		buildCalendar : function(e, t, n, r, i) {
			...
			for ( var c = 0, d = 0, v = 0; c < 42; c++, d++, p = moment(p).add(
					"hour", 24)) {
				if (c > 0 && d % 7 == 0) {
					d = 0;
					v++
		...
		renderDropdowns : function(e, t, n) {
			...
			for ( var u = 0; u < 12; u++) {
				if ((!s || u >= t.month()) && (!o || u <= n.month())) {
					i += "<option value='" + u + "'"
							+ (u === r ? " selected='selected'" : "") + ">"
							+ this.locale.monthNames[u] + "</option>"
				}
			}
			i += "</select>";
			var a = e.year();
		...
		renderCalendar : function(t, n, r, i) {
			...
			var o = this.locale.monthNames[t[1][1].month()]
					+ t[1][1].format(" YYYY");
			...
			for ( var u = 0; u < 6; u++) {
				...
				for ( var a = 0; a < 7; a++) {
					...
					if (r && t[u][a].isBefore(r) || i && t[u][a].isAfter(i)) {
						f = " off disabled "
					} else if (t[u][a].format("YYYY-MM-DD") == n
							.format("YYYY-MM-DD")) {
						f += " active ";
						if (t[u][a].format("YYYY-MM-DD") == this.startDate
								.format("YYYY-MM-DD")) {
							f += " start-date "
						}
						if (t[u][a].format("YYYY-MM-DD") == this.endDate
								.format("YYYY-MM-DD")) {
							f += " end-date "
						}
					} else if (t[u][a] >= this.startDate
							&& t[u][a] <= this.endDate) {
						f += " in-range ";
			...
			if (this.timePicker) {
				...
				for ( var d = c; d <= h; d++) {
					if (d == p) {
						s += '<option value="' + d + '" selected="selected">'
								+ d + "</option>"
					} else {
