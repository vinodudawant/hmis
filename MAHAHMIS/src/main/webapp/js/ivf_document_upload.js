...
function temForUploadDocumentOPD(id) {
	...
	var temp=  '<div class="tab-pane active" id="Upload_Document"><div class="box-body col-md-12-1" style="padding-top: 10px; padding-bottom: 0px">'
		...
		+ '	<button style=" margin-top: 3px; margin-left: -100px" class="btn btn-xs btn-success editUserAccess"  id="ifileUp" name="fileUp" onclick="saveIVFDocument()" type="button">Upload Document</button>'
...
function saveIVFDocument(){
	...
	data.append("ivftreatmentId", ivftreatmentId);

	jQuery.ajax({
		async:false,
		type : "POST",
		enctype: 'multipart/form-data',
		url : "ehat/ivfDocumentUpload/saveIVFDocument",
		data : data,
		processData : false,
		contentType : false,
		error : function() {
...
function getAllIVFDocuments(){
	...
	jQuery.ajax({
		async: false,
		type: "GET",
		url: "ehat/ivfDocumentUpload/getAllIVFDocuments",
...
function editIVFDocuments(documentId) {
	if(documentId !=undefined && documentId!=null && documentId!="" && documentId!="null"){
		...
		jQuery.ajax({
			async : false,
			type : "GET",
			url : "ehat/ivfDocumentUpload/editIVFDocuments",
			data : str + "&reqType=AJAX",
			error : function() {
				alert('error');
			},
...
function deleteIVFDocuments(documentId) {
	if(documentId !=undefined && documentId!=null && documentId!="" && documentId!="null"){
		var r = confirm("Are You Sure You Want To Delete Document");
		if (r == true) {
			jQuery.ajax({
				type : "POST",
				url : "ehat/ivfDocumentUpload/deleteIVFDocuments",
				data : {
		...
		for (var i = 0; i < result.lstDoctorDeskDocumentUploadDto.length; i++) {
			if (prevtre == "previousTreatmentOPDER") {
				divContent = divContent
						...
						+ (i + 1)
						+ "' class='btn btn-xs btn-success' style='margin-left: 6px;cursor: pointer;' onclick='editIVFDocuments("
						+ result.lstDoctorDeskDocumentUploadDto[i].documentId
						+ ")'><i class='fa fa fa-edit'></i></button></td>"*/
						+ "<td style='height: 21.5px; padding-left: 0px;' class='col-md-1-1 center'><button id='hiddenRv"
						+ (i + 1)
						+ "' class='btn btn-xs btn-success' style='margin-left: 6px;cursor: pointer;' onclick='viewIVFDocuments("
						...
						+ (i + 1)
						+ "' class='btn btn-xs btn-danger hidden' style='margin-center: 6px;cursor: pointer;' onclick='deleteIVFDocuments("
						+ result.lstDoctorDeskDocumentUploadDto[i].documentId
				...
				divContent = divContent
						...
						+ "<td style='height: 21.5px; padding-left: 0px;' class='col-md-1-1 center'><button id='hiddenR"
						+ (i + 1)
						+ "' class='btn btn-xs btn-danger deleteBtn' style='margin-center: 6px;cursor: pointer;' onclick='deleteIVFDocuments("
...
function viewIVFDocuments(rowNumber,documentId) {
	...
	if(document ==null || document =="" || document ==undefined){
		...
		$('#ViewDocumemnt').attr("src","ehat/ivfDocumentUpload/viewIVFDocuments?documentId="+documentId+"&fileName="+document);
