...
//	alert(pId);
	...
	var dateExpectedDischarge = $("#dateExpectedDischarge").val();d
...
function savedIPDDischargeProcess() {	
	...
	var txtstartTime10 = $("#txtstartTime10").val();
	...
	var staffresp10 = $("#staffresp10").val();
	...
	var checkbox10 = $('input:checkbox[id=checkbox10]');
...
/*	 var processdata01 = {
	...
	inputs.push("txtstartTime10=" + txtstartTime10);
			...
			inputs.push("staffresp10=" + staffresp10);
			...
			inputs.push("checkbox10=" + checkbox10);
    ...
    inputs.push('tId=' + tId);
		 var str = inputs.join('&');
			...
			success : function(response) {
						...
						$("#txtstartTime10").val(response.processdatalist[0].txtstartTime10);
						...
						$("#staffresp10").val(response.processdatalist[0].staffresp10);
						...
						$("#remark10").val(response.processdatalist[0].remark10);
						...
						(response.processdatalist[0].checkbox10 == "Y") ? ($('#checkbox10').attr('checked', true)): ($('#checkbox10').attr('checked', false));
...
function getCustomizeTemplatesIDDischarge(){
	...
	jQuery.ajax({
		...
		success : function(r) {			
			...
			CKEDITOR.instances['editor1'].setData(r.temp_data);
			$('#customizeTemplateName').val(r.temp_name);
		}
	});
...
function saveIPDDischargeSummaryTemplate(callFrom) {
	...
	if (callFrom == 'IPD') {
		...
		if(idIPDdischargeSummary==""){
			idIPDdischargeSummary=0;
		}
		...
		if (discharge_dateNew != "") {
			...
			var admsndate = $("#doa").text(); 
			var addt = admsndate.split("/");
			var datetime = addt[2].split(",");
			var addDate = new Date(datetime[0],addt[1]-1,addt[0]); 
		...
		jQuery
				.ajax({
					...
					success : function(r) {
						...
						//alert(ajaxResponse);
						var tobj = ajaxResponse;
						//eval('(' + ajaxResponse + ')');
						if (r != null) {
							var myObj = tobj;
							$("#idIPDdischargeSummary").val(myObj.idipdPatientDischargeSummary);
						...
						//dd=new Date(disDate);
							//alert(disDate);
							var	disDate=myObj.dischargeDate.replace(/\-/g,'/');
							disDate=disDate.split("_");
						 
							//alert(disDate[0]+"date");
							//alert(disDate[1]+"time");
							
							var cal_date = myObj.dischargeDate.split("-");
							//alert(cal_date);
							var new1=cal_date[2].split("_");
							//alert(new1);
							var mydate =new1[0]+"/"+cal_date[1]+"/"+cal_date[0];
							$("#discharge_date_note").val(mydate);
							$("#discharge_Time_note").val(myObj.dischargeTime);
							//$("#discharge_Time_note").val(disDate[1]);
							$("#selTempWiseSummary").val(myObj.idCustomizeTemplate);
							
							var dischargeDateTime = mydate+","+myObj.dischargeTime;
							$("#dod").text(dischargeDateTime);
							...
							.setData(myObj.tempData);
							//$('#customizeTemplateName').val(myObj.dischargeType);
							$("#discharge_Type").val(myObj.dischargeType);
						
							if(myObj.tempData != null || myObj.tempData != undefined || myObj.tempData != "" || myObj.tempData != "null")
								{
