...
function setHistoryTemplate(id){
	...
	var temp = '<div class="tab-pane active" id="History">'
			...
			+ '<input type="text" value="" readonly="readonly" name="mrn" id="mrnIpd" class="form-control input-SmallText">'
			+ '</div></div>'
			...
			+ '<th style="height: 21.5px; width: 25px;"><input type="button" value="+" onclick="createDivIPDHistory()"> <input type="button" value="-" onclick="removeChifComp(\'historyTable1\',\'chkhistory\')">'
			...
			+ '<textarea class="" cols="52" rows="3" id="chiefComplaintsTxt" style="margin-left: 3%;"></textarea></div>'

			+ '<div style="padding-left: 1%; margin-top: 2%;" class="col-md-10-1"><label class="TextFont">Negative History: </label>'
			+ '<textarea class="" cols="52" rows="3" id="clinicalFinding" style="margin-left: 3%;"></textarea></div>'
			...
			+ '<div style="margin-top: 5px;" class="col-sm-12-1" id="row_1"><hr style="height: 0px;border-top: 1px solid;margin: 1em 0;width:99%"></div>'
			...
			+ '</div></div> <div style="padding-right: 8px; margin-top: 3%;" class="col-md-12-1"> </div>'
			...
			+ '<textarea class="" cols="40" rows="3" id="provDia" style="margin-left: 1%;"></textarea></div>'
			+ '<div style="padding-left: 1.2%; id="divtempTreatId"  class="col-md-6-1"><label class="TextFont">Treatment Plan:</label>'
			+ '<textarea class="" cols="40" rows="3" id="treatPlan" style="margin-left: 7%;"></textarea></div></div>'
			+ '</div></div> </div>'
			
			+ '<div style="padding-right: 8px; margin-top: 1%;margin-left: 26px;" class="col-md-12-1">'
			+ '<div class="divide-10"></div><div class="col-md-6-1" ">'
			+ '<label class="TextFont" style="margin-right: 50px;">Habbits:</label>'
...
var rowCount = $('#historyTable1 tbody tr').length;
	...
	htm = htm
			...
			+ "<td style=' width: 33.96%; height: 21.5px; '><textarea rows='1' cols='51'  id='chiefComp"+parseInt(rowCount)+ "'  > </textarea></td>"
				
				
			+ "<td style='width: 50.49%; height: 21.5px;'  ><div class='col-md-12-1' style='margin-top:5%' ><div class='col-md-6-1' style='margin-top:-3%'><input type='range'  min='0' max='100' class='defaultSlider' id='defaultSlider_"+rowCount+" ' style='font-size: 11px; width:120%;' name='duration' " 
...
//	var historyId=0;
	...
	//added by vishant
	var provDiagnosis = $("#provDia").val();
	var treatmentPlan = $("#treatPlan").val();
	var habbits = $("#habbits").val();
...
$('#historyTable1 tr').each(function() {  
...
var inputs = [];
	...
	inputs.push('provisionalDiagno=' + encodeURIComponent(provDiagnosis));
	
	inputs.push('treatPlan=' + encodeURIComponent(treatmentPlan));
	
	inputs.push('habbits=' + encodeURIComponent(habbits));
...
var treatmentId=$("#tr_Id").val();
	...
	jQuery.ajax({
		...
		success : function(r) {
			...
			//$("#selCustomizeTemp").val(r.template)
			//$('#selCustomizeTemp option').eq(r.templateId).prop('selected', true);
...
//	var billId=$("#billNo").text();    
    ...
    window.open("ipd_history_print.jsp?billId="+billId+"&treatId="+treatId+"&patId="+patId+"&recId="+recId+"&pendFlag="+pendFlag);
 ...
 * @author   :HM00098
 * @Date     :08-08-2023
 * @Code     :this method used for IPD History patient info by template id
 * *****//*
function getPatientHistoryByTemplateId(){
	...
	var treatmentId=$("#tr_Id").val();
		...
		jQuery.ajax({
			...
			type : "POST",
			data : str + "&reqType=AJAX",
			url : "ehat/opdhistory/getPatientHistoryByTemplateId",
			timeout : 1000 * 60 * 5,
			cache : false,
			error : function() {
				alert('error');
