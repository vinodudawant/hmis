...
<body>
	<%
		...
		int unitId=1;
		ltTreatsAll = fs.getBuildTreatments(unitId, userId, patId, sponsorId);
		...
		if(ltTreatsAll.size() > 0){
			...
			if(cashPaidSpId != -10){
				...
				BillReceiptMasterDTO objBillForNo = new BillReceiptMasterDTO();
				objBillForNo.setPatientId(patId);
				objBillForNo.setListTreatDto(ltTreats);
				BillMasterAllTreat objBillTreat = fs.getBilledBillNo(objBillForNo);
				billedBillNo = objBillTreat.getBillMasterId();
			}			
			...
			if(finalbillIs.equals("finalBill")){
				...
				HeaderTable3.addCell(new Phrase("Bill No",subheader));
				HeaderTable3.addCell(new Phrase(" : "+billedBillNo,tabletext));
					...
					if(lstSponsors.size() > 0){ */
						...
						/* for(MultipleSponsorDto mulSpObj: lstSponsors){
							...
							if(spId == mulSpObj.getChargesSlaveId()){ */
									...
									if((listPharmacyDetailsOnBillingPrint.size() > 0 || lstNarcotics.size() > 0) && cashPaidSpId != -10){
										
										PdfPTable HeaderTable51 = new PdfPTable(3);
										int[] headerwidth51 = { 60, 50, 50 };
										...
										for (int k = 0; k < listBillDetail.size(); k++) {
											...
											if(listBillDetail.get(k).getCancle().equals("N")){						
												...
												pharmaDisc = listBillDetail.get(k).getConcessionPer();
												pharmaDiscAmt = (pharmaFinalAmt * pharmaDisc)/100;
												pharmaTotalAmt = (pharmaFinalAmt - pharmaDiscAmt);
									...
									HeaderTable5.addCell(new Phrase("       Date              Quantity         Rate             Amount", subheader)) ;								
									...
									for (int i = 0; i < listService.size(); i++) {
										...
										String sevName=listService.get(i).getServiceName();
											...
											if(PType.equals("Sponsor") && cashPaidSpId != -10){
											...
											if(serId == 3){
												...
												for (int k = 0; k < listBedIpdDto.size(); k++) {
													...
													if(listBedIpdDto.get(k).getCancle().equals("N")){
														...
														if(listBedIpdDto.get(k).getSubServiceId()!=0){
															...
															if((k%2)!=0){
																if(shraddha.equalsIgnoreCase("on")){
																	...
																	subServName="Medication Charges : "+listBedIpdDto.get(k-1).getBedHall();
																}else{
																	
																	subServName="Nursing : "+listBedIpdDto.get(k-1).getBedHall();
																}														
															}
															double rate = 0;
															double qty  = 0;
															...
															/* if((k%2)!=0){ */
																if(shraddha.equalsIgnoreCase("on")){
																	...
																	subServName="Medication Charges : "+listBedIpdDto.get(k).getBedHall();
																}else{
																	
																	subServName="Nursing : "+listBedIpdDto.get(k).getBedHall();
																}
												...
												for (int k = 0; k < listBillDetail.size(); k++) {
													...
													if(listBillDetail.get(k).getCancle().equals("N")){
														...
														String subServName = listBillDetail.get(k).getBedHall();
														//String subServName="";
														double rate = 0;
														double qty  = 0;
														double amt  = 0;
														...
														if(serId==2){
															...
															subServName="Consultation Charges";
															subServName = subServName + cghsId + Date;
							...
							if(obj3.getListBillReceiptMaster().size()>0){
								...
								for(int i=0;i< obj3.getListBillReceiptMaster().size();i++){
									...
									HeaderTable9.getDefaultCell().setHorizontalAlignment(Element.ALIGN_RIGHT);
									HeaderTable9.addCell(new Phrase(""+df2.format(obj3.getListBillReceiptMaster().get(i).getFirstPaid()), tabletext));
									HeaderTable9.getDefaultCell().setHorizontalAlignment(Element.ALIGN_RIGHT);
