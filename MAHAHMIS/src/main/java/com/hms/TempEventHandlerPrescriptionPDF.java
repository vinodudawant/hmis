...
import com.hms.doctordesk.controller.OPDHistoryController;
...
public class TempEventHandlerPrescriptionPDF extends PdfPageEventHelper implements PdfPageEvent {
...
//	Font font;
	...
	public void onStartPage(PdfWriter writer, Document document) {
		...
		try {
			...
			OPDHistoryService uss2=(ApplicationContextUtils.getApplicationContext()).getBean(OPDHistoryService.class);
			 OPDHistoryMasterDTO historyobj=  uss2.getOPDHistory(treatmentId);
			  List<OPDBmiMasterDTO> bmiList = uss2.getOPDBMIListByTreatmentId(treatmentId);
			
			
			
			
		List<EhatViewPatientBedDetailsIpdDto> listBedIpdDto2 =null;	
		if(printType.equalsIgnoreCase("DischargeSummery")) {	
			
			
			IpdBillService fetchServlist=(ApplicationContextUtils.getApplicationContext()).getBean(IpdBillService.class);	
			
			listBedIpdDto2=fetchServlist.getPatientBedBill(treatmentId,3);
		}
			...
			if (headerFlag.equalsIgnoreCase("No")) {
				...
				if(patientId.equalsIgnoreCase("UHID")){
					patientDetailsHeader.addCell(new Phrase(""+patientId,subheader));
					patientDetailsHeader.addCell(new Phrase(": "+rtd.getCenterPatientId(),tabletext));
				}else{
					patientDetailsHeader.addCell(new Phrase(""+patientId, subheader));
		...
		if(printType.equalsIgnoreCase("IPDPrint")) {	
			...
			patientDetailsHeader.addCell(new Phrase(": " + regDate, tabletext));
				...
				if(listBedIpdDto2.size()>0 && listBedIpdDto2.get(0).gethName()!=null){	
					patientDetailsHeader.addCell(new Phrase("Ward", subheader));
					patientDetailsHeader.addCell(new Phrase(": " + listBedIpdDto2.get(0).gethName(), tabletext));
				   }
				else {
					patientDetailsHeader.addCell(new Phrase("Ward", subheader));
					patientDetailsHeader.addCell(new Phrase(":", tabletext));
					...
					patientDetailsHeader.addCell(new Phrase("Discharge Date ", subheader));
					patientDetailsHeader.addCell(new Phrase(": "+dischargedate, tabletext));
					  
					patientDetailsHeader.addCell(new Phrase("Discharge Type", subheader));
					patientDetailsHeader.addCell(new Phrase(": " + dischargeType, tabletext));
					patientDetailsHeader.addCell(new Phrase("Res. Address  ", subheader));
					patientDetailsHeader.addCell(new Phrase(": " + rtd.getAddress(), tabletext));
				...
				if(printType.equalsIgnoreCase("IPDPrintPrescription")) {
					
					String addresspatient =(String) request.getAttribute("patientAdd");
				
					String perPatientAdd = (String) request.getAttribute("perPatientAdd");
						
					patientDetailsHeader.addCell(new Phrase("Res. Address ",subheader));
					patientDetailsHeader.addCell(new Phrase(" : "+addresspatient,	tabletext));
					
					if(perPatientAdd !="" || perPatientAdd != "" && !perPatientAdd.equals("undefined") && perPatientAdd != null && perPatientAdd !=null)
					{
						patientDetailsHeader.addCell(new Phrase("Per. Address ",subheader));
						patientDetailsHeader.addCell(new Phrase(" : "+perPatientAdd,	tabletext));
					}
				}	
				...
				if(nabhLogoPrint.equalsIgnoreCase("on")) {
					 ...
					 * if (imgNabh == null) {
					 * 
					 * HeaderTable1.addCell(new Phrase("", header)); } else {
					 * 
				...
				double BMI=0;
				if(rtd.getHeight() > 0 && rtd.getWeight() > 0 ) {
				double heightInCM = (rtd.getHeight() / 100);
				BMI = rtd.getWeight() / Math.pow(heightInCM, 2);
				}
				...
				patientDetailsHeader.addCell(new Phrase("Phone", subheader));
				patientDetailsHeader.addCell(new Phrase(": " + rtd.getMobile(), tabletext));
				patientDetailsHeader.addCell(new Phrase(" Date", subheader));
				patientDetailsHeader.addCell(new Phrase(": " + regDate, tabletext));
						      
				patientDetailsHeader.addCell(new Phrase("Mrn ", subheader));
				patientDetailsHeader.addCell(new Phrase(": " + rtd.getMrnno(), tabletext));
				patientDetailsHeader.addCell(new Phrase("", subheader));
				patientDetailsHeader.addCell(new Phrase("", subheader));
				...
				patientDetailsHeader.addCell(new Phrase("BMI: " , subheader));
				/* if(bmiList.size() > 0) {
					 
						patientDetailsHeader.addCell(new Phrase(""+bmiList.get(bmiList.size()-1).getBmi(), tabletext));
				 }else {
					 patientDetailsHeader.addCell(new Phrase("", tabletext));
				 }*/
				 patientDetailsHeader.addCell(new Phrase(""+BMI, tabletext));
						patientDetailsHeader.addCell(new Phrase("", tabletext));
						patientDetailsHeader.addCell(new Phrase("", tabletext));
				...
				if(historyobj !=null) {
				patientDetailsHeader.addCell(new Phrase("Pulse: "  , subheader));
				patientDetailsHeader.addCell(new Phrase(": " +historyobj.getPulse() , tabletext));
				patientDetailsHeader.addCell(new Phrase("BP: " , subheader));
				patientDetailsHeader.addCell(new Phrase(""+historyobj.getBp(), tabletext));
				
				patientDetailsHeader.addCell(new Phrase("Complaints: " , subheader));
				patientDetailsHeader.addCell(new Phrase(": " +historyobj.getChiefComplaints(), tabletext));
				patientDetailsHeader.addCell(new Phrase("", tabletext));
				patientDetailsHeader.addCell(new Phrase("", tabletext));
				}
